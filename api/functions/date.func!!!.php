...
***************************************************************************************************** */
	...
	function dateToDBCheckUS($_date){
		if($_date == "")
		{
			return "NULL";	
		}
...
//		$_date = $arrDate[0]."-".$arrDate[1]."-".$arrDate[2];
	...
	function datetimeToDBUS($_datetime){
		if($_datetime == "")
		{
			return "NULL";	
		}
//		$arrDate = explode("-", $_date);
//		$_date = $arrDate[0]."-".$arrDate[1]."-".$arrDate[2];
		return "TO_DATE('".$_datetime."', 'YYYY-MM-DD:HH24:MI')";
		//return "'".$_date."'";
	}
	...
	function convertToJamMenit($_time, $format = '%02d:%02d') {
		if ($_time < 1) {
			return "0 mnt";
		}
		
		if(floor($_time/60) > 10)
			$hours = sprintf("%02d jam", floor($_time/60));	
		else
			$hours = sprintf("%01d jam", floor($_time/60));	
		
		if($_time%60 > 10)
			$minutes = sprintf("%02d mnt", $_time%60);
		else
			$minutes = sprintf("%01d mnt", $_time%60);
	...
	function convertMenitToJamMenit($_time)
	{
		if($_time == "00:00:00")
			return "0 mnt";
		...
		return (int)$arrTime[0].' jam'.' '.(int)$arrTime[1].' mnt';
	}
