  ...
  class Dokumen extends Entity{ 
    ...
    **/ 
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order=" ORDER BY DOKUMEN_ID ASC ")
	{
		$str = "
		SELECT 
		DOKUMEN_ID, NAMA, KETERANGAN, WAJIB, FORMAT, STATUS_AKTIF, PENAMAAN_FILE
		, CASE WHEN STATUS_AKTIF = '1' THEN 'Aktif' ELSE 'Tidak Aktif' END STATUS_AKTIF_INFO
		FROM dokumen A
		WHERE 1 = 1
		"; 
	...
	function selectByParamsMonitoring($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order=" ORDER BY DOKUMEN_ID ASC ")
	{
		$str = "
		SELECT
		A.DOKUMEN_ID, A.NAMA, A.KETERANGAN, WAJIB, FORMAT
		, CASE WAJIB WHEN '1' THEN 'Wajib' WHEN '0' THEN 'Tidak Wajib' END WAJIB_INFO
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(DOKUMEN_ID) AS ROWCOUNT FROM dokumen
		        WHERE DOKUMEN_ID IS NOT NULL ".$statement; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key LIKE '%$val%' ";
