  ...
  class Bidang extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		...
		$this->setField("BIDANG_ID", $this->getNextId("BIDANG_ID","pds_rekrutmen.BIDANG")); 		

		$str = "
				INSERT INTO pds_rekrutmen.BIDANG
						(BIDANG_ID, KODE, NAMA, KETERANGAN, LAST_CREATE_USER,
						 LAST_CREATE_DATE)
	...
	function delete()
	{
        $str = "DELETE FROM pds_rekrutmen.BIDANG
                WHERE 
    ...
    **/ 
	...
	{
		$str = "
				SELECT BIDANG_ID, KODE, NAMA, KETERANGAN, LAST_CREATE_USER, LAST_CREATE_DATE,
					   LAST_UPDATE_USER, LAST_UPDATE_DATE
				  FROM pds_rekrutmen.BIDANG A
				  WHERE 1 = 1
				"; 
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(BIDANG_ID) AS ROWCOUNT FROM pds_rekrutmen.BIDANG A
	...
	{
		$str = "SELECT COUNT(BIDANG_ID) AS ROWCOUNT FROM pds_rekrutmen.BIDANG
		        WHERE BIDANG_ID IS NOT NULL ".$statement; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key LIKE '%$val%' ";
