  ...
  class UserGroup extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		...
		$this->setField("USER_GROUP_ID", $this->getAdminNextId("USER_GROUP_ID","USER_GROUP"));

		$str = "
					INSERT INTO USER_GROUP (
					   USER_GROUP_ID, NAMA, AKSES_ADM_REKRUTMEN_ID)
 			  	VALUES (
    ...
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order=" ORDER BY A.NAMA ASC")
	{
		$str = "
					SELECT USER_GROUP_ID, A.NAMA, KETERANGAN, 
					   ...
					   E.NAMA AKSES_APP_PENGHASILAN, 
					   F.NAMA AKSES_APP_PRESENSI,
					   ...
					   AKSES_APP_TAGIHAN AKSES_APP_TAGIHAN_ID,
					   CASE
						  WHEN AKSES_APP_TAGIHAN = '1'
							 THEN 'YA'
						  ELSE 'TIDAK'
					   END AKSES_APP_TAGIHAN
					   , A.AKSES_ADM_REKRUTMEN_ID, S.NAMA AKSES_ADM_REKRUTMEN
				  FROM USER_GROUP A 
					   LEFT JOIN AKSES_APP_KEPEGAWAIAN D
					   ON A.AKSES_APP_KEPEGAWAIAN_ID = D.AKSES_APP_KEPEGAWAIAN_ID
					   LEFT JOIN AKSES_APP_PENGHASILAN E
					   ON A.AKSES_APP_PENGHASILAN_ID = E.AKSES_APP_PENGHASILAN_ID
					   LEFT JOIN AKSES_APP_PRESENSI F
					   ON A.AKSES_APP_PRESENSI_ID = F.AKSES_APP_PRESENSI_ID
					   LEFT JOIN AKSES_APP_PENILAIAN G
					   ON A.AKSES_APP_PENILAIAN_ID = G.AKSES_APP_PENILAIAN_ID
					   LEFT JOIN AKSES_ADM_INTRANET J
					   ON A.AKSES_ADM_INTRANET_ID = J.AKSES_ADM_INTRANET_ID
					   LEFT JOIN AKSES_APP_SURVEY K
					   ON A.AKSES_APP_SURVEY_ID = K.AKSES_APP_SURVEY_ID
					   LEFT JOIN AKSES_APP_INVENTARIS N
					   ...
					   LEFT JOIN AKSES_APP_HUKUM P ON A.AKSES_APP_HUKUM_ID =
																		  P.AKSES_APP_HUKUM_ID
					   LEFT JOIN AKSES_APP_NOTIFIKASI Q
					   ON A.AKSES_APP_NOTIFIKASI_ID = Q.AKSES_APP_NOTIFIKASI_ID
					   LEFT JOIN AKSES_APP_OPERASIONAL R
					   ON A.AKSES_APP_OPERASIONAL_ID = R.AKSES_APP_OPERASIONAL_ID
					   
					   LEFT JOIN AKSES_ADM_REKRUTMEN S
					   ON A.AKSES_ADM_REKRUTMEN_ID = S.AKSES_ADM_REKRUTMEN_ID
