  ...
  ***/
  ...
  class PelamarLowongan extends Entity{ 
    ...
    **/ 
	...
	{
		$str = "
				SELECT A.LOWONGAN_ID, B.KODE, C.NAMA JABATAN, B.PENEMPATAN, TANGGAL_KIRIM, 
				CASE WHEN D.PELAMAR_ID IS NULL THEN 'Tidak' ELSE 'Ya' END SHORTLIST FROM pds_rekrutmen.PELAMAR_LOWONGAN A 
				INNER JOIN pds_rekrutmen.LOWONGAN B ON A.LOWONGAN_ID = B.LOWONGAN_ID
				INNER JOIN pds_rekrutmen.JABATAN C ON B.JABATAN_ID = C.JABATAN_ID
				LEFT JOIN pds_rekrutmen.PELAMAR_LOWONGAN_SHORTLIST D ON A.PELAMAR_ID = D.PELAMAR_ID AND A.LOWONGAN_ID = D.LOWONGAN_ID
		 		WHERE 1 = 1
				".$statement; 
	...
	{
		$str = "
				SELECT A.LOWONGAN_ID, A.KODE, B.NAMA JABATAN, A.PENEMPATAN, A.TANGGAL, A.TANGGAL_AKHIR, C.TANGGAL_KIRIM FROM
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(PELAMAR_LOWONGAN_ID) AS ROWCOUNT FROM pds_rekrutmen.PELAMAR_LOWONGAN
		        WHERE PELAMAR_LOWONGAN_ID IS NOT NULL ".$statement; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = '$val' ";
	...
	{
		//LEFT JOIN pds_rekrutmen.PELAMAR_LOWONGAN B ON A.PELAMAR_ID = B.PELAMAR_ID AND B.TANGGAL_KIRIM IS NOT NULL
