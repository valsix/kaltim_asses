...
class Entity{
	...
	*/
	function getAdminNextId($idName,$tableName,$filter=""){
		...
		if($filter != "")
			...
			if($this->rowCount>0){
				$tempField= strtolower("MAXVALUE");
				$temp = $this->rowResult[0][$tempField] + 1; //next key*/
	...
	function getNextId($idName,$tableName,$filter=""){
		...
		/*
		//$str = "SELECT MAX($idName) as maxvalue FROM $tableName ";
		$str = " SELECT pds_rekrutmen.RANDOM_INTEGER(1, 1000000000) as MAXVALUE ";
		//if($filter != "")
			//$str = $str . " WHERE $filter";				
			...
			if($this->rowCount>0){
				$tempField= strtolower("MAXVALUE");
				$temp = $this->rowResult[0][$tempField] + 1;
			}else{ //belum ada recordnya
        		$temp = 1;
      		}
		//echo $str;
	...
	function getNextUrutId($idName,$tableName,$filter=""){
		$str = "SELECT MAX($idName) as maxvalue FROM $tableName ";
		//$str = " SELECT MAX) pds_rekrutmen.RANDOM_INTEGER(1, 1000000000) as MAXVALUE ";
		//if($filter != "")
			//$str = $str . " WHERE $filter";				
