...
***************************************************************************************************** */
	...
	function dateToDBCheck2($_date){
		...
		$arrDate = explode("-", $_date);
		$_date = $arrDate[2]."-".generateZeroDate($arrDate[1],2)."-".generateZeroDate($arrDate[0], 2);
		return "TO_DATE('".$_date."', 'DD-MM-YYYY')";
	}
	...
	function getNameDay($number) {
		$number = (int)$number;
		$arrMonth = array("1"=>"Minggu", "2"=>"Senin", "3"=>"Selasa", "4"=>"Rabu", "5"=>"Kamis", 
						  "6"=>"Jumat", "7"=>"Sabtu");
		return $arrMonth[$number];
	}
	...
	// date input : database
	function getFormattedDateTime($_date, $showTime=true, $extended=false)
	{
		$_date = explode(" ", $_date);
		...
		if($extended == true)
		$date = $arrDate[2].' '.getNameMonthExt($_month).' '.$arrDate[0];
		else
		$date = $arrDate[2].' '.$arrMonth[$_month].' '.$arrDate[0];
