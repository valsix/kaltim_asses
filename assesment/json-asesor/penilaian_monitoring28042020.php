...
// print_r($reqPenilaianPotensiDeskripsi);exit();
...
// print_r($reqPenilaianKompetensiBolehSimpan);exit();
...
$tempPotensiStrength = "";
$tempPotensiWeaknes = "";
$tempPotensiKesimpulan = "";
$tempPotensiSaranPengembangan = "";
$tempPotensiSaranPenempatan = "";
$tempPotensiId = "";
...
for($i=0; $i < count($reqPenilaianPotensiDetilId); $i++)
{
	...
	else
	{
		...
		// if (!empty($reqPenilaianPotensiBukti[$i]) && empty($reqPenilaianPotensiDeskripsi[$i]))
		// {
		//  	$set_detil->setField('BUKTI', setQuote($reqPenilaianPotensiBukti[$i]));
		// }
		// if (!empty($reqPenilaianPotensiDeskripsi[$i]) && empty($reqPenilaianPotensiBukti[$i]))
		//  {
		// 	$set_detil->setField('BUKTI', setQuote($reqPenilaianPotensiDeskripsi[$i]));
		//  }
		 $set_detil->setField('BUKTI', setQuote($reqPenilaianPotensiBukti[$i]));
		...
		if (!empty($reqPenilaianPotensiStrength[$i]))
		{
			$tempPotensiStrength= $reqPenilaianPotensiStrength[$i];
		}
		if (!empty($reqPenilaianPotensiWeaknes[$i]))
		{
			$tempPotensiWeaknes= $reqPenilaianPotensiWeaknes[$i];
		}
		if (!empty($reqPenilaianPotensiKesimpulan[$i]))
		{
			$tempPotensiKesimpulan= $reqPenilaianPotensiKesimpulan[$i];
		}
		if (!empty($reqPenilaianPotensiSaranPengembangan[$i]))
		{
			$tempPotensiSaranPengembangan= $reqPenilaianPotensiSaranPengembangan[$i];
		}
		if (!empty($reqPenilaianPotensiSaranPenempatan[$i]))
		{
			$tempPotensiSaranPenempatan= $reqPenilaianPotensiSaranPenempatan[$i];
		}
		if (!empty($reqPenilaianPotensiId[$i]))
		{
			$tempPotensiId= $reqPenilaianPotensiId[$i];
		}   
...
// echo $tempPotensiSaranPengembangan;exit;

// echo $tempPotensiStrength;exit;
$set_text= new JadwalPegawaiDetil();
$set_text->setField('CATATAN_STRENGTH', setQuote($tempPotensiStrength));
$set_text->setField('CATATAN_WEAKNES', setQuote($tempPotensiWeaknes));
$set_text->setField('KESIMPULAN', setQuote($tempPotensiKesimpulan));
$set_text->setField('SARAN_PENGEMBANGAN', setQuote($tempPotensiSaranPengembangan));
$set_text->setField('SARAN_PENEMPATAN', setQuote($tempPotensiSaranPenempatan));
$set_text->setField('PENILAIAN_ID',$tempPotensiId);
...
for($i=0; $i < count($reqPenilaianKompetensiDetilId); $i++)
{
	if($reqPenilaianKompetensiBolehSimpan[$i] == "1")
	{
		...
		else
		{
			...
			$set_detil->setField('CATATAN', setQuote($reqPenilaianKompetensiCatatan[$i]));
			$set_detil->setField('BUKTI', setQuote($reqPenilaianKompetensiBukti[$i]));
