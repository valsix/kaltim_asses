...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",
		"sAjaxSource": "../json-skp/kinerjaku_json.php?reqId=<?=$reqId?>&reqBulan=<?=$tempBulanAktif?>&reqTahun=<?=$tempTahunAktif?>",
		...
		/* Click event handler */
		  ...
		  $('#btnEdit').on('click', function () {
			  opUrl    = 'kinerjaku_add.php?reqMode=update&reqId='+anSelectedId+'&reqBulan='+anSelectedBulan+'&reqTahun='+anSelectedTahun;
		  ...
		  $('#btnCetak').on('click', function () {
			  if(anSelectedData == "")
				  return false;	
			  window.top.OpenDHTML('kinerjaku_cetak_pdf.php?&reqId='+anSelectedId+'&reqBulan='+anSelectedBulan+'&reqTahun='+anSelectedTahun, 'Sasaran Kerja Pegawai');	
		  });
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						var jqxhr = $.get( "delete.php?reqMode=kinerjaku&id="+anSelectedId, function() {
							$.messager.progress('close');
						})
						.done(function() {
							$.messager.progress('close');
							oTable.fnReloadAjax("../json-skp/kinerjaku_json.php?reqSearch=" + $("#reqStatus").val() + "&reqId=<?=$reqId?>");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
		  ...
		  $("#reqBulan,#reqTahun").change(function() { 
			  ...
			  oTable.fnReloadAjax("../json-skp/kinerjaku_json.php?reqId=<?=$reqId?>&reqBulan="+reqBulan+"&reqTahun="+reqTahun);
		  });
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <div id="header-tna">Data <span>Kinerjaku dan SKP</span></div>
    <div id="bluemenu" class="bluetabs" style="background:url(css/media/bluetab.gif)">    
    <ul>
          <li>
          	<?php /*?><a href="#" title="Isi/Edit Formulir" id="btnEdit"><img src="../WEB/images/icn_edit.png" width="15" height="15"/>&nbsp; Isi/Edit Formulir</a> 
            <a href="#" id="btnCetak" title="Cetak"> Cetak</a><?php */?>  
          </li>
    </ul>
    </div>
