...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",
		"sAjaxSource": "../json-skp/pelaporan_skp_realisasi_json.php",
		...
		/* Click event handler */
		  ...
		  $('#example tbody').on( 'click', 'tr', function () {
			  ...
			  anSelectedBulan = element[2];
			  anSelectedTahun = element[3];
		  ...
		  $('#btnEdit').on('click', function () {
			   ...
			   if(anSelectedStatus == "PROSES ENTRI")
				{
				  window.top.OpenDHTML('pelaporan_skp_realisasi_add.php?reqMode=update&reqId='+anSelectedId+'&reqBulan='+anSelectedBulan+'&reqTahun='+anSelectedTahun, 'Sasaran Kerja Pegawai');	
				}	
				else
				{
				  alert("Data yang telah di posting tidak dapat diubah.");
				  return false;																
				}
		  });
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						var jqxhr = $.get( "delete.php?reqMode=pelaporan_skp_realisasi&id="+anSelectedId, function() {
							$.messager.progress('close');
						})
						.done(function() {
							$.messager.progress('close');
							oTable.fnReloadAjax("../json-skp/pelaporan_skp_realisasi_json.php?reqSearch=" + $("#reqStatus").val() + "&reqId=<?=$reqId?>");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
	...
	function postingPencapaian(pegawaiId, bulan, tahun)
	{
		if(confirm("Posting pencapaian SKP periode terpilih?"))	
		{

			$.getJSON('../json-skp/pelaporan_skp_realisasi_posting_json.php?reqId='+pegawaiId+'&reqBulan='+bulan+'&reqTahun='+tahun,
			function(data){
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <div id="header-tna">Data <span>Realisasi Pencapaian SKP</span></div>
    ...
    <ul>
          <li>
			...
			?>
           		<a href="#" id="btnEdit" title="Realisasi"> Realisasi</a> 
			<?
			}else{
			?>
            <? 
    ...
    <thead>
        <tr>
            ...
            <th width="200px">Jumlah Target</th>   
            <th width="200px">Jumlah Pencapaian</th>  
