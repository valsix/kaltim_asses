...
$training->selectByParams(array(), -1,-1, " AND 1=2");
...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-silat/diklat_analisa_kompetensi_bendel_json.php?reqSatkerId=<?=$reqId?>&reqPersen=0&reqTahun=<?=$reqTahun?>&reqEselon=<?=$reqEselon?>",
		...
		/* Click event handler */
		  ...
		  var anSelectedData = '';
		  var anSelectedKodeUnker= anSelectedAtributId = anSelectedTrainingId= '';
		  ...
		  $('#example tbody').on( 'click', 'tr', function () {
			  ...
			  //anSelectedKodeUnker = element[element.length-3];
			  anSelectedKodeUnker = "<?=$reqId?>";
			  anSelectedTrainingId = element[element.length-5];
			  anSelectedAtributId = element[element.length-2];
			  anSelectedPegawaiId = element[0];
			  anSelectedPegawaiNipBaru = element[3];
			  anSelectedPegawaiNama = element[4];
			  //alert(anSelectedKodeUnker);
		  });
		  ...
		  $('#btnUbahData').on('click', function () {
			  ...
			  opUrl    = 'diklat_analisa_kompetensi.php?reqAtributId='+anSelectedAtributId+"&reqId="+anSelectedKodeUnker+"&reqTrainingId="+anSelectedTrainingId;
		  ...
		  $("#btnCari").on("click", function () {
			...
			oTable.fnReloadAjax("../json-silat/diklat_analisa_kompetensi_bendel_json.php?reqSatkerId=<?=$reqId?>&reqPersen=0&reqTrainingId="+reqTrainingId+"&reqTahun="+reqTahun);
		  });
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <div id="header-tna">Analisis <span>Kebutuhan Diklat</span></div>
    ...
    <ul>
          <li>
          	...
          	<li><a href="#" title="Ubah" id="btnUbahData"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;View</a></li>
    ...
    <div id="bar-status">
    	...
    	<?
        while($training->nextRow())
		{
		?>
        <option value="<?=$training->getField("TRAINING_ID")?>"><?=$training->getField("NAMA")?></option>
        <?
		}
        ?>
        </select>
		   <?php /*?><input id='reqId' type="hidden" />
           <input type="hidden" id="reqPersen" />
           <select id="reqPersen">
           ...
           <select id="reqEselon">
                <option value="" <? if($reqEselon == "") echo "selected";?>>Semua</option>
                <option value="1" <? if($reqEselon == "1") echo "selected";?>>I</option>
                <option value="2" <? if($reqEselon == "2") echo "selected";?>>II</option>
                <option value="3" <? if($reqEselon == "3") echo "selected";?>>III</option>
                <option value="4" <? if($reqEselon == "4") echo "selected";?>>IV</option>
                <option value="5" <? if($reqEselon == "5") echo "selected";?>>Staf & Fungsional</option>
           </select>
           &nbsp;&nbsp;
           <label>Total Pegawai Rekomendasi : <?=$jumlah_pegawai?></label>
           <label> ||   Total Rekomendasi Diklat : <?=$jumlah_pegawai2?></label><?php */?>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="50px">Analisa</th>
            <th width="50px">Rekomendasi Training</th>
            <th width="20px">Jumlah Pegawai Rekomendasi</th>
        </tr>
    </thead>
    </table>
