...
$set= new Kelautan();
$set->selectByParamsMonitoringAnalisaPegawai(array("D1.ATRIBUT_ID"=>$reqAtributId, "F1.TRAINING_ID"=>$reqTrainingId),-1,-1);
$set->firstRow();
//$tempNama= $set->getField("NAMA_ATRIBUT_PARENT_NAMA")."-".$set->getField("ATRIBUT_NAMA").", Training: ".$set->getField("TRAINING_NAMA");
$tempNama= $set->getField("ATRIBUT_NAMA").", Training: ".$set->getField("TRAINING_NAMA");
unset($set);
...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",
		"sAjaxSource": "../json-silat/diklat_analisa_kompetensi_json.php?reqTrainingId=<?=$reqTrainingId?>&reqAtributId=<?=$reqAtributId?>&reqId=<?=$reqId?>",
		...
		/* Click event handler */
		  ...
		  $('#btnUbahData').on('click', function () {
			  window.location.href = 'diklat_analisa_kompetensi_bendel.php?reqId=<?=$reqId?>&reqTahun=<?=$reqTahun?>&reqPersen=<?=$reqPersen?>';
		  });
		  
		  $('#btnLihatData').on('click', function () {
			  if(anSelectedData == "")
				  return false;
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						.done(function() {
							$.messager.progress('close');
							oTable.fnReloadAjax("../json/diklat_analisa_kompetensi_json.php?reqSearch=" + $("#reqStatus").val() + "&reqId=<?=$reqId?>");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
		  ...
		  $("#reqPencarian").keyup(function(e) { 
			...
			if(code==13)
			{
				...
				oTable.fnReloadAjax("../json-silat/diklat_analisa_kompetensi_json.php?reqTrainingId=<?=$reqTrainingId?>&reqAtributId=<?=$reqAtributId?>&reqId=<?=$reqId?>&sSearch="+reqPencarian);
			}
		  });
		  
		  $('#btnExcel').on('click', function () {
			  var url= 'diklat_analisa_kompetensi_excel.php?reqTrainingId=<?=$reqTrainingId?>&reqAtributId=<?=$reqAtributId?>&reqId=<?=$reqId?>';
		  ...
		  $('#btnPdf').on('click', function () {
			  var url= 'diklat_analisa_kompetensi_pdf.php?reqTrainingId=<?=$reqTrainingId?>&reqAtributId=<?=$reqAtributId?>&reqId=<?=$reqId?>';
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <div id="header-tna">Detil Analisa <span>(<?=$tempNama?>)</span></div>
    <div id="bluemenu" class="bluetabs" style="background:url(css/media/bluetab.gif)">    
    <ul>
          <li>
          	<a href="#" title="Detail Data" id="btnUbahData"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;Kembali</a>
            <a href="#" title="Detail Data" id="btnLihatData"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;Detail Data</a>
            <li><a href="#" title="Cetak" rel="dropmenu2_b"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;Cetak</a></li>
    ...
    <div id="dropmenu2_b" class="dropmenudiv_b" style="width: 250px;">
        ...
        <a href="#" title="Pdf" id="btnPdf">Pdf</a>
    </div>
