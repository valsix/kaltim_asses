...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		columnDefs: [{ className: 'never', targets: [ 0, 1, 8, 13, 14, 16] }],
		"sScrollY": ($(window).height() - <?=$tinggi?>),
		"sScrollX": "100%",
		"sScrollXInner": "160%",
		"sAjaxSource": "../json-silat/pegawai_beasiswa_json.php?reqId=<?=$reqId?>&reqCari=<?=$reqCari?>",
		...
		/* Click event handler */
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						.done(function() {
							$.messager.progress('close');
							oTable.fnReloadAjax("../json/pegawai_beasiswa_json.php?reqSearch=" + $("#reqStatus").val() + "&reqId=<?=$reqId?>");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
		  ...
		  $("#reqJenis,#reqTahun").change(function() { 
		  	var reqJenis= reqTahun= "";
			reqJenis= $("#reqJenis").val();
			reqTahun= $("#reqTahun").val();
			oTable.fnReloadAjax("../json-silat/pegawai_beasiswa_json.php?reqJenis="+reqJenis+"&reqTahun="+reqTahun);
		  });
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <div id="header-tna">Data <span>Beasiswa Pegawai</span></div>
