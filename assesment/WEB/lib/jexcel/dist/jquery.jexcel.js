...
function getnextdata(coldata, rowdata, mode)
{
    if($('#'+coldata+'-'+rowdata).css('display') == 'none')
    // if($('#reqAccountJexcel > #'+coldata+'-'+rowdata).css('display') == 'none')
    // if($('.jexcel>div>.selected>table>tbody>tr>td.c'+coldata+'.r'+rowdata).css('display') == 'none')

    {
        if(mode == "kanan")
        {
            coldata= parseInt(coldata) + 1;
            return getnextdata(coldata, rowdata, mode);
        }
        else if(mode == "kiri")
        {
            coldata= parseInt(coldata) - 1;
    ...
    {
        // console.log("func:"+coldata+", "+rowdata);
...
var methods = {
            ...
            $.fn.jexcel.mouseDownControls = function (e) {
                ...
                if (e.which != 3) {
                    ...
                    if (e.target.id == 'jexcel_corner') {
                        ...
                        if($($.fn.jexcel.selectedCell).hasClass('readonlyvaluetrue'))
                        {
                            return false;
                        }
                        ...
                        if ($(jexcelTable).length > 0) {
                            ...
                            if ($(e.target).parents('.jexcel-content').length) {
                                ...
                                // if ($(target).length && $(target).prop('id')) {
                                    ...
                                    // if ($(e.target).parent().is('.jexcel_label')) {
                                    if ($(e.target).is('.jexcel_label')) {
                                        ...
                                        if ($.fn.jexcel.defaults[$.fn.jexcel.current].rowDrag == true && $(e.target).outerWidth() - e.offsetX < 8) {
                                            ...
                                            if ($.fn.jexcel.selectedRow && (e.shiftKey || e.ctrlKey)) {
                                                ...
                                                // noteone tambahan detil load function pada row selected
                                                if (typeof window.loaddetilexcel === 'function')
                                                {
                                                    reqIRowIdtable= $.fn.jexcel.current;
                                                    dindex= String(d);
                                                    dindex= dindex.replace("row,", "");
                                                    dindex= parseInt(dindex) + 1;
                                                    var reqIRowIdval = $('#' + $.fn.jexcel.current).jexcel('getValue', "A"+dindex);

                                                    if(reqIRowIdval !== "")
                                                        window.loaddetilexcel(reqIRowIdtable, reqIRowIdval);
                                                }
                        ...
                        if (e.target.id == 'jexcel_corner') {
                            ...
                            if ($(target).is('.highlight')) {
                                if($($.fn.jexcel.selectedCell).hasClass('readonlyvaluetrue'))
                                {
                                    ...
                                    reqIRowIdtable= $.fn.jexcel.current;
                                    reqCallMode= $.fn.jexcel.defaults[$.fn.jexcel.current].columns[columnId[0]].type;
                                    // console.log(reqIRowIdtable+"--"+reqCallMode);return false;

                                    arrcolumnId= String(columnId);
                                    arrcolumnId= arrcolumnId.split(',');
                                    // coltable= parseInt(arrcolumnId[0]) + 1;
                                    // rowtable= parseInt(arrcolumnId[1]);
                                    // coltable= codeexceltochar(parseInt(arrcolumnId[0]));
                                    coltable= parseInt(arrcolumnId[0]);
                                    rowtable= parseInt(arrcolumnId[1]) + 1;
                                    
                                    if (typeof window.changedetilexcel === 'function')
                                    {
                                        // coltable= "a";
                                        // rowtable= "b";
                                        // valuetable= "5";
                                        window.changedetilexcel(reqIRowIdtable, reqCallMode, coltable, rowtable);
                                    }
                                }
                                else
                                {
                        ...
                        if (e.which == 37) {
                            ...
                            if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                                if (e.ctrlKey) {
                                    ...
                                    if ($.fn.jexcel('col') > 0) {
                                        ...
                                        cell = $($.fn.jexcel.selectedCell).prevAll('td:visible:first');
                                        // console.log($.fn.jexcel.selectedCell);

                                        coldata= (parseInt(columnId[0]) - 1);
                                        rowdata= columnId[1];
                                        coldatanext= getnextdata(coldata, rowdata, 'kiri');

                                        // var inputs = $($.fn.jexcel.selectedCell).parent().find(':td:visible');
                                        // inputs.eq( inputs.index(this)- 1 ).focus();

                                        // // console.log(coldatanext);
                                        // cell = $($.fn.jexcel.selectedCell).parent().find('#'+coldatanext);
                                    }
                                }
                                ...
                                //             // scrollLeft: ($(window).width() + $(window).scrollLeft()) - ($(cell).offset().left)
                                //             scrollRight: $(cell).offset().right
                                //         }, 1000);
                                //     }
                                // }
                                // else
                                ...
                                /*if(cell !== null)
                                {
                                    if(cell.length > 0)
                                    {
                                        geserscroll= $(cell).offset().left - $(cell).width();
                                        // geserscroll= $(cell).offset().left;
                                        // geserscroll= $(cell).offset().left - ( $(cell).width() + $(cell).scrollLeft() );
                                        // console.log(geserscroll);

                                        // if(geserscroll > 0)
                                        // {
                                            // $('html, body').stop(true).animate({
                                            $('#'+$.fn.jexcel.current).stop(true).animate({
                                                // scrollLeft: ($(window).width() + $(window).scrollLeft()) - ($(cell).offset().left)
                                                // scrollLeft: $(cell).offset().left - 40
                                                scrollLeft: geserscroll
                                            }, 1000);
                                        // }
                                ...
                                console.log(cell.position().left);
                                console.log($(cell).width());
                                console.log("posisicell:"+posisicell);

                                if($(cell).width() <= 148)
                                {
                                    if(posisicell !== $(cell).width())
                                    {
                                        // oneleren
                                        $('#'+$.fn.jexcel.current).stop(true).animate({
                                            // scrollLeft: $("#"+coldata+"-"+rowdata).position().left - 200
                                            scrollLeft: cell.position().left - 148
                                        }, 1000);
                                        posisicell= $(cell).width();
                                    }
                                }
                            ...
                            if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                                if (e.ctrlKey) {
                                    ...
                                    // noteone
                                    // console.log("noteone-next");
                                    // cell = $($.fn.jexcel.selectedCell).next();

                                    // cell = $($.fn.jexcel.selectedCell).nextAll('td:visible').focus();
                                    cell = $($.fn.jexcel.selectedCell).nextAll('td:visible:first');


                                    // console.log(cell.attr('id'));

                                    // $("#"+cell.attr('id')).focus();
                                    // infocellid= cell.attr('id');
                                    // document.getElementById(infocellid).scrollIntoView();
                                    ...
                                    coldatanext= getnextdata(coldata, rowdata, 'kanan');
                                    // // console.log(coldatanext);
                                    // cell = $($.fn.jexcel.selectedCell).parent().find('#'+coldatanext);
                                    ...
                                    // if ( $($.fn.jexcel.selectedCell).next().offset() != undefined)
                                    //     $('html,body').animate({ scrollTop:$($.fn.jexcel.selectedCell).next().offset().top});
                                }
                                
                                // var elmnt = document.getElementById(coldatanext);
                                // var txt = "Height with padding: " + elmnt.clientHeight + "px<br>";
                                // txt += "Width with padding: " + elmnt.clientWidth + "px<br>";
                                // txt += "Width with padding: " + $(cell).width() + "px";
                                ...
                                // console.log(txt);
                                leftOff = ($(window).width() - $(cell).width())/2;
                                var offs = getScrollXY();

                                // console.log( $("#"+$.fn.jexcel.current).outerWidth()+";;"+elmnt.clientWidth+"xx"+offs[0]+"**"+offs[1]+"--"+window.pageYOffset +"-"+window.pageXOffset +"-"+leftOff +"-"+ $(window).width()+"--"+$(cell).scrollLeft()+"--"+$(cell).width()+"--"+$(cell).offset().left );

                                // if($(cell).width() == 78)
                                // if(elmnt.clientWidth == 98 || elmnt.clientWidth == 118)
                                // {
                                    // geserscroll= $(cell).offset().left - ( $(cell).width() + $(cell).scrollLeft() );
                                    // geserscroll= $(cell).offset().left - $(cell).width();
                                    // geserscroll= $(cell).offset().left - ( $(cell).width() + $(cell).scrollLeft() + (elmnt.clientWidth * 2) );

                                    // geserscroll= window.pageYOffset;

                                    // geserscroll= $("#"+$.fn.jexcel.current).outerWidth();
                                ...
                                // }
                                // console.log($(cell).offset().left +"--"+$(cell).width()+"--"+$(cell).scrollLeft());
                                ...
                                // console.log(getPosition(cell));
                                // console.log($("#"+coldata+"-"+rowdata).position());
                                ...
                                if($(cell).width() <= 115)
                                {
                                    if(posisicell !== $(cell).width())
                                    {
                                        ...
                                        $('#'+$.fn.jexcel.current).stop(true).animate({
                                            // scrollLeft: $("#"+coldata+"-"+rowdata).position().left - 200
                                            scrollLeft: cell.position().left - 115
                                ...
                                /*$('#'+$.fn.jexcel.current).stop(true).animate({
                                    // scrollLeft: $("#"+coldata+"-"+rowdata).offset().top
                                    scrollLeft: $("#"+coldata+"-"+rowdata).offset().left - 200
                                }, 1000);
                                $("#"+coldata+"-"+rowdata).focus();*/
                            ...
                            if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                                ...
                                //     // $('html, body').stop(true).animate({
                                //     $('#'+$.fn.jexcel.current).stop(true).animate({
                                //          scrollTop: $(cell).offset().top - 40
                                //     }, 1000);
                                // }

                                /*$('#'+$.fn.jexcel.current).stop(true).animate({
                                    // scrollLeft: $("#"+coldata+"-"+rowdata).position().left - 200
                                    // scrollTop: cell.position().top + 4
                                    scrollTop: $(cell).offset().top - 40
                                }, 1000);*/
                            ...
                            // Tab
                            // noteone
                            // $($.fn.jexcel.selectedCell).nextAll('td:visible').focus();
                            ...
                            if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                               if (e.shiftKey) {
                                   if ($.fn.jexcel('col') > 0) {
                                       ...
                                       cell = $($.fn.jexcel.selectedCell).prevAll('td:visible:first');
                                       $('#'+$.fn.jexcel.current).stop(true).animate({
                                        scrollLeft: cell.position().left - 200
                                       }, 1000);
                                   }
                               } else {
                                    // noteone
                                   // cell = $($.fn.jexcel.selectedCell).next();
                                   // cell = $($.fn.jexcel.selectedCell).nextAll('td:visible').focus();
                                   cell = $($.fn.jexcel.selectedCell).nextAll('td:visible:first').focus();
                                   // cell = $($.fn.jexcel.selectedCell).nextAll('td:visible:first');
                                   $('#'+$.fn.jexcel.current).stop(true).animate({
                                    scrollLeft: cell.position().left - 200
                                   }, 1000);
                            ...
                            if (e.metaKey && ! e.shiftKey && ! e.ctrlKey) {
                                ...
                                if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                                    if (e.which == 65) {
                                        ...
                                        e.preventDefault();
                                    } else if (e.which == 81 || e.which == 32) {
                                        // console.log("a");
                                     //
                                    // f1
                                    // console.log($('#' + $.fn.jexcel.current).find('.highlight'));
                                    // $('#' + $.fn.jexcel.current).jexcel('setValue', $('#' + $.fn.jexcel.current).find('.highlight'), '');
                                    ...
                                    // {
                                        reqIRowIdtable= $.fn.jexcel.current;
                                        reqCallMode= $.fn.jexcel.defaults[$.fn.jexcel.current].columns[columnId[0]].type;
                                        // console.log(reqIRowIdtable+"--"+reqCallMode);
                                        arrcolumnId= String(columnId);
                                        arrcolumnId= arrcolumnId.split(',');
                                        // coltable= parseInt(arrcolumnId[0]) + 1;
                                        // rowtable= parseInt(arrcolumnId[1]);
                                    ...
                                    // console.log("--"+$.fn.jexcel.defaults[$.fn.jexcel.current].columns[columnId[0]].type);
                                        e.preventDefault();
    ...
    openEditor : function(cell, empty, e) {
        ...
        // Readonly
        if ($(cell).hasClass('readonly') == true || $(cell).hasClass('readonlyvaluetrue') == true) {
            // Do nothing
        } else {
            // Holder
    ...
    closeEditor : function(cell, save) {
        ...
        if (save == true) {
            ...
            if (typeof window.changedetilexcel === 'function')
            {
                // console.log($.fn.jexcel.current);return false;
                reqIRowIdtable= $.fn.jexcel.current;
                window.changedetilexcel(reqIRowIdtable, position[0], position[1], value);
            }
    ...
    setValue : function(cell, value, force) {
        ...
        if ($(cell).length) {
            ...
            $.each(cell, function(k, v) {
                ...
                // console.log("oldValue:"+ $.fn.jexcel.defaults[id].data[position[1]][position[0]]);
                // console.log("oldValue1:"+ $.fn.jexcel.records[position[1]][position[0]].innerHTML;
        ...
        if ($(v.cell).hasClass('readonly') == true && force == false) {
            ...
            if (options.columns[position[0]].editor) {
                ...
                if (options.columns[position[0]].type == 'checkbox' || options.columns[position[0]].type == 'radio') {
                    ...
                    if (value.substr(0,1) == '=') {
                        ...
                        // console.log("formulavalue:"+value);
                    } else {
            ...
            if ($.fn.jexcel.ignoreEvents != true) {
                ...
                if (value.substr(0,1) == '=') {
                    ...
                    console.log("executeFormula:"+v.col + '-' + v.row);
                }
    ...
    deleteColumn : function(columnNumber, numOfColumns) {
        ...
        if (options.allowDeleteColumn == true) {
            ...
            if (options.data[0].length > 1) {
                ...
                // $('#reqJexcel').jexcel('setValue', 'O0', "");
                // $('#reqJexcel').jexcel('setValue', 'O1', "");
                // $('#' + $.fn.jexcel.current).jexcel('setValue', $('#' + $.fn.jexcel.current).find('.highlight'), '');
    ...
    deleteRow : function(rowNumber, numOfRows) {
        ...
        if (options.allowDeleteRow == true) {
            ...
            // addfunctionsebelumhapus
            if (typeof(options.onbeforedeleterow) == 'function') {
                // console.log("sdasd");
                if (! options.onbeforedeleterow($(this), rowNumber, numOfRows)) {
            ...
            if (options.data.length > 1) {
                ...
                var id = $(this).prop('id');
                $('#'+id).jexcel('insertRow', 1, 2);
                $('#'+id).jexcel('deleteRow', 0, 1);
                $(this).jexcel('setValue', 'C'+rowNumber, "1");
                ...
                /*for (var col = 0; col < numOfColumns; col++) {
                    $(this).jexcel('setValue', codeexceltochar(col)+rowNumber, "");
                }*/

                // $('#reqJexcel').jexcel('setValue', 'H0', "");
                // $('#reqJexcel').jexcel('setValue', 'H1', "");
    ...
    executeFormula : function (cellId) {
        ...
        if (value === null || value == undefined || (''+value) == 'NaN' || typeof value === 'function') {
            ...
            value = '<input type="hidden" value="' + value + '">' + value;
            // Remove any error class
            $(cell).removeClass('error');
            // Update cell content
            $(cell).html(value);
            // console.log("valexecuteFormula:"+value);
    ...
    createCell : function(i, j) {
        ...
        if (options.columns[i].readOnly == "valuetrue") {
            $(td).addClass('readonlyvaluetrue');
            // $(td).addClass('readonlyvaluetrue', 'readonly');
            // $(td).addClass('readonly', 'readonly');
        }
    ...
    destroy : function () {
        ...
        if ($.fn.jexcel.defaults != undefined) 
        {
