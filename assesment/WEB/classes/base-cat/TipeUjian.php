  ...
  ***/
  ...
  class TipeUjian extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		...
		$str = "INSERT INTO cat.tipe_ujian (
				   TIPE_UJIAN_ID, TIPE, LAST_CREATE_DATE, LAST_CREATE_USER, KATEGORI) 
				VALUES (
    ...
    **/ 
	...
	{
		$str = "SELECT TIPE_UJIAN_ID, TIPE, WAKTU, TOTAL_SOAL, KETERANGAN, KATEGORI, ID,
				CASE WHEN KATEGORI = '1' THEN 'POTENSI' WHEN KATEGORI = '2' THEN 'KOMPETENSI' ELSE '-' END KATEGORI_NAMA
				FROM cat.tipe_ujian A WHERE 1=1"; 
	...
	function selectByParamsJumlahSoal($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		...
		SELECT
			...
			, CASE WHEN A.ID LIKE '01%' OR A.ID LIKE '02%' OR A.TIPE_UJIAN_ID = 7 OR A.TIPE_UJIAN_ID = 17 OR A.TIPE_UJIAN_ID = 41 OR A.TIPE_UJIAN_ID = 42 OR A.TIPE_UJIAN_ID = 45 OR A.TIPE_UJIAN_ID = 47 OR A.TIPE_UJIAN_ID = 48 OR A.TIPE_UJIAN_ID = 49 THEN 1 ELSE 0 END TIPE_READONLY
			, CASE WHEN A.ID LIKE '01%' OR A.ID LIKE '02%' OR A.TIPE_UJIAN_ID = 7 OR A.TIPE_UJIAN_ID = 17 OR A.TIPE_UJIAN_ID = 41 OR A.TIPE_UJIAN_ID = 42 OR A.TIPE_UJIAN_ID = 45 OR A.TIPE_UJIAN_ID = 47 OR A.TIPE_UJIAN_ID = 48 OR A.TIPE_UJIAN_ID = 49 THEN COALESCE(A.TOTAL_SOAL,0) 
    ...
    **/ 
    ...
    function selectByParamsInfoBankSoal($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder= "ORDER BY A.ID")
	{
		$str = "
		SELECT
		A.TIPE_UJIAN_ID
		, CASE WHEN A.PARENT_ID = '0' THEN A.TIPE ELSE B.TIPE || ' - ' || A.TIPE END TIPE_NAMA
		FROM cat.tipe_ujian A
    ...
    function selectByParamsBankSoal($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder= "ORDER BY A.ID")
	{
		$str = "
		SELECT
		A.TIPE_UJIAN_ID
		...
		(
			...
			FROM
			(
			SELECT TIPE_UJIAN_ID FROM cat.bank_soal A WHERE 1=1 GROUP BY TIPE_UJIAN_ID
			) X WHERE A.TIPE_UJIAN_ID = X.TIPE_UJIAN_ID
		)"; 
    ...
    function getCountByParamsBankSoal($paramsArray=array(), $statement="")
	{
		$str = "
