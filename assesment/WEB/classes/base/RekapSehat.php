...
***************************************************************************************************** */
  ...
  ***/
  ...
  class RekapSehat extends Entity{ 
   ...
   function selectByParamsMonitoringRekapLain6r($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $statementdetil='', $sorder="", $norma="")
	{
		$str = "
		SELECT
				HSL.JADWAL_TES_ID, UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, B.PEGAWAI_ID, B.FORMULA_ASSESMENT_ID
...
function selectByParamsJawabanPegawaiTKD6RMonitoring($paramsArray=array(),$limit=-1,$from=-1, $parseid, $statement='', $sOrder= "ORDER BY ja.NOMOR_URUT_GENERATE asc", $norma='')
	{
		$str = "
		SELECT
		Aaa.PEGAWAI_ID,aa.NIP_BARU, aa.nama NAMA_PEGAWAI, ja.NOMOR_URUT_GENERATE, x.JUMLAH_SOAL,
			coalesce(xx.JUMLAH_BENAR,0)  JUMLAH_BENAR".$norma."
		FROM
		(
			SELECT * FROM cat.ujian_pegawai_daftar A
		) Aaa
		left join simpeg.pegawai aa on aa.pegawai_id=Aaa.pegawai_id
		INNER JOIN formula_assesment_ujian_tahap_bank_soal bB ON Aaa.FORMULA_ASSESMENT_ID = bB.FORMULA_ASSESMENT_ID
	LEFT JOIN
			(
				SELECT a.NO_URUT NOMOR_URUT_GENERATE, A.PEGAWAI_ID, A.LAST_UPDATE_DATE
				FROM jadwal_awal_tes_simulasi_pegawai A
				INNER JOIN jadwal_tes B ON JADWAL_AWAL_TES_SIMULASI_ID = JADWAL_TES_ID
				WHERE JADWAL_AWAL_TES_SIMULASI_ID =".$parseid."
			) JA ON JA.PEGAWAI_ID = aaa.PEGAWAI_ID
		INNER JOIN
		(
			SELECT A.FORMULA_ASSESMENT_ID, A.JUMLAH_SOAL_UJIAN_TAHAP JUMLAH_SOAL
			FROM formula_assesment_ujian_tahap a
			INNER JOIN cat.tipe_ujian TU ON TU.TIPE_UJIAN_ID = A.TIPE_UJIAN_ID and tu.TIPE_UJIAN_ID=73
			GROUP BY A.FORMULA_ASSESMENT_ID, A.JUMLAH_SOAL_UJIAN_TAHAP
		) x ON bB.FORMULA_ASSESMENT_ID = x.FORMULA_ASSESMENT_ID
		left join(
			SELECT
			A.PEGAWAI_ID,
				count(a.PEGAWAI_ID)/2 JUMLAH_BENAR 
			FROM
			(
				SELECT * FROM cat.ujian_pegawai_daftar A
			) A
			...
			INNER JOIN
			(
				SELECT A.PEGAWAI_ID, A.BANK_SOAL_ID, b.LAST_CREATE_USER jawaban_1, c.LAST_CREATE_USER jawaban_2 ,d.jawaban cekjawaban_1,e.jawaban cekjawaban_2
				FROM cat_pegawai.ujian_pegawai_".$parseid." A
				left join cat_pegawai.ujian_pegawai_".$parseid." b on b.pegawai_id = a.pegawai_id and A.BANK_SOAL_ID=b.BANK_SOAL_ID and b.bank_soal_pilihan_id=1 and a.bank_soal_pilihan_id is not null
				left join cat_pegawai.ujian_pegawai_".$parseid." c on c.pegawai_id = a.pegawai_id and A.BANK_SOAL_ID=c.BANK_SOAL_ID and c.bank_soal_pilihan_id=2 and a.bank_soal_pilihan_id is not null
				left join 
				...
				WHERE 1=1  
				group by A.PEGAWAI_ID, A.UJIAN_ID, A.BANK_SOAL_ID, A.TIPE_UJIAN_ID, A.URUT, b.LAST_CREATE_USER, c.LAST_CREATE_USER, c.bank_soal_pilihan_id, b.bank_soal_pilihan_id,d.jawaban,e.jawaban
			)  UP ON A.PEGAWAI_ID = UP.PEGAWAI_ID AND B.BANK_SOAL_ID = UP.BANK_SOAL_ID		 
			WHERE 1=1
			AND A.JADWAL_TES_ID = ".$parseid."
			AND B.TIPE_UJIAN_ID IN (73)
			and (case when up.jawaban_1= cekjawaban_1 and up.jawaban_2= cekjawaban_2 then 1 else 0 end)=1 "
			.$statement." group by A.PEGAWAI_ID
			)xx on aaa.pegawai_id=xx.pegawai_id
			WHERE 1=1
			AND aaA.JADWAL_TES_ID = ".$parseid."
			AND bb.TIPE_UJIAN_ID IN (73) 
			group by 		Aaa.PEGAWAI_ID,aa.NIP_BARU, aa.nama, ja.NOMOR_URUT_GENERATE, x.JUMLAH_SOAL,xx.jumlah_benar"; 
