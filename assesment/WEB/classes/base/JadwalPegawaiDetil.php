    ...
    **/ 
    ...
    function updatePenilaianText()
	{
		...
		SET
			...
			PROFIL_KEPRIBADIAN= '".$this->getField("PROFIL_KEPRIBADIAN")."',
			KESESUAIAN_RUMPUN= '".$this->getField("KESESUAIAN_RUMPUN")."',
    ...
    function selectByParamsPenilaianLain($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
	{
		$str = "
		SELECT
			STRIP_TAGS(A.CATATAN_STRENGTH) CATATAN_STRENGTH, STRIP_TAGS(A.CATATAN_WEAKNES) CATATAN_WEAKNES, STRIP_TAGS(A.KESIMPULAN) KESIMPULAN, 
			STRIP_TAGS(A.SARAN_PENGEMBANGAN) SARAN_PENGEMBANGAN, STRIP_TAGS(A.SARAN_PENEMPATAN) SARAN_PENEMPATAN
		FROM penilaian A
		WHERE ASPEK_ID = 1
		".$statement;
		// AND A.ASESOR_ID = 25 AND A.JADWAL_TES_ID = 25
