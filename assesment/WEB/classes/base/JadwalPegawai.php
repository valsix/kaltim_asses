...
***************************************************************************************************** */
  ...
  ***/
  ...
  class JadwalPegawai extends Entity{ 
    ...
    **/
	...
	function prosesJadwalAbsen()
	{
        $str = "
        SELECT P_JADWAL_ABSEN(".$this->getField("JADWAL_TES_ID").", ".$this->getField("PEGAWAI_ID").", '".$this->getField("LAST_CREATE_USER")."')
		"; 
		$this->query = $str;
		// echo $str;exit();
	...
	function insert()
	{
		...
		$this->setField("JADWAL_PEGAWAI_ID", $this->getNextId("JADWAL_PEGAWAI_ID","jadwal_pegawai")); 

		$str = "INSERT INTO jadwal_pegawai (
				...
				// echo $str; exit;
		$this->id= $this->getField("JADWAL_PEGAWAI_ID");
		$this->query= $str;
		return $this->execQuery($str);
    }
	...
	function delete()
	{
        $str = "DELETE FROM jadwal_pegawai
                WHERE 
    ...
    **/ 
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY JADWAL_PEGAWAI_ID ASC")
	{
		$str = "SELECT JADWAL_PEGAWAI_ID, JADWAL_ASESOR_ID, PEGAWAI_ID, PENGGALIAN_ID, NILAI, KETERANGAN
				FROM jadwal_pegawai WHERE 1=1"; 
	...
	// {
	// 	$str = "
	// 	SELECT A.JADWAL_PEGAWAI_ID, A.PEGAWAI_ID, A1.NAMA PEGAWAI_NAMA, A1.NIP_BARU PEGAWAI_NIP
	// 	, B1.KODE PEGAWAI_GOL, C1.NAMA PEGAWAI_ESELON, A1.LAST_JABATAN PEGAWAI_JAB_STRUKTURAL
	// 	, E.TANGGAL_TES, D1.SATKER_ID SATKER_TES_ID, f.link_foto,a1.LAST_ESELON_ID LAST_ESELON_ID
	// 	, D1.NAMA SATKER
	// 	, A.JADWAL_ASESOR_ID
	...
	// 	INNER JOIN SIMPEG.UPLOAD_FILE F ON A.PEGAWAI_ID = F.pegawai_ID
 ...
 //    }
    ...
    function selectByParamsPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
	{
		...
		LEFT JOIN jadwal_asesor C ON A.JADWAL_ASESOR_ID = C.JADWAL_ASESOR_ID
		LEFT JOIN jadwal_acara D ON C.JADWAL_ACARA_ID = D.JADWAL_ACARA_ID
		LEFT JOIN jadwal_tes E ON D.JADWAL_TES_ID = E.JADWAL_TES_ID
		LEFT JOIN SIMPEG.UPLOAD_FILE F ON A.PEGAWAI_ID = F.pegawai_ID
		LEFT JOIN ".$this->db.".pangkat B1 ON A1.LAST_PANGKAT_ID = B1.PANGKAT_ID
		LEFT JOIN ".$this->db.".eselon C1 ON A1.LAST_ESELON_ID = C1.ESELON_ID
		LEFT JOIN ".$this->db.".satker D1 ON A1.SATKER_ID = D1.SATKER_ID
		WHERE 1=1
	...
	function selectByParamsJadwalPegawaiInfo($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
	{
		...
		, E.STATUS_PENILAIAN, A.JADWAL_ASESOR_ID, A.PEGAWAI_ID
		FROM jadwal_pegawai A
	...
	function selectByParamsJadwalPegawaiInfoAspek1($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
	{
		$str = "
		SELECT D.NAMA NAMA_ASESI, 'Potensi' METODE, E.TANGGAL_TES, F.NAMA NAMA_PEGAWAI, F.LAST_JABATAN JABATAN_INI_TES, G.NAMA SATUAN_KERJA_INI_TES, E.JADWAL_TES_ID
		, E.STATUS_PENILAIAN, A.JADWAL_ASESOR_POTENSI_ID JADWAL_ASESOR_ID, A.PEGAWAI_ID
	...
	function selectByParamsMonitoring($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY B.PUKUL1, D.NAMA, B.KETERANGAN ASC")
	{
		$str = "
		SELECT 
			A.JADWAL_ASESOR_ID, A.JADWAL_ACARA_ID, A.ASESOR_ID, A.KETERANGAN AS KETERANGAN_JADWAL, C.NAMA, B.KETERANGAN AS KETERANGAN_ACARA
			, C.NAMA ASESOR_NAMA, B.PUKUL1, B.PUKUL2
			, D.PENGGALIAN_ID, D.NAMA PENGGALIAN_NAMA, D.KODE PENGGALIAN_KODE
			, CASE WHEN A.BATAS_PEGAWAI = JUMLAH THEN 1 ELSE 0 END CHECK_TERPENUHI
		...
		(
			SELECT 
			A.JADWAL_ASESOR_ID JASID, COUNT(1) JUMLAH
			FROM jadwal_pegawai A
			WHERE 1=1
			GROUP BY A.JADWAL_ASESOR_ID
		) XY ON A.JADWAL_ASESOR_ID = JASID
		WHERE 1=1 "; 
	...
	function selectByParamsJadwalPegawaiPenggalianDetil($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY B.NAMA")
	{
		$str = "SELECT A.PEGAWAI_ID, A.PENGGALIAN_ID, B.NAMA PENGGALIAN_NAMA, B.KODE PENGGALIAN_KODE, TO_CHAR(C.TANGGAL_TES, 'YYYY') TAHUN_JADWAL
				...
				INNER JOIN penggalian B ON A.PENGGALIAN_ID = B.PENGGALIAN_ID
				INNER JOIN jadwal_tes C ON A.ID_JADWAL = C.JADWAL_TES_ID
				INNER JOIN jadwal_pegawai_detil D ON A.JADWAL_PEGAWAI_ID = D.JADWAL_PEGAWAI_ID
				WHERE 1=1
				"; 
		// AND A.PEGAWAI_ID = 182 AND TO_CHAR(C.TANGGAL_TES, 'YYYY') = '2016'
		while(list($key,$val) = each($paramsArray))
	...
	function selectByParamsAsesorPenilaianDetil($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY A.ATRIBUT_ID, A.INDIKATOR_ID, A.LEVEL_ID")
	{
		$str = "SELECT A.ATRIBUT_ID, A.INDIKATOR_ID, A.LEVEL_ID, C.ASESOR_ID
				FROM jadwal_pegawai_detil A
				INNER JOIN jadwal_pegawai B ON A.JADWAL_PEGAWAI_ID = B.JADWAL_PEGAWAI_ID
				INNER JOIN jadwal_asesor C ON B.JADWAL_ASESOR_ID = C.JADWAL_ASESOR_ID
				WHERE 1=1 "; 
	...
	function selectByParamsAsesorNilai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY B.ASESOR_ID, A.PEGAWAI_ID, F.ATRIBUT_ID")
	{
		$str = "SELECT 
				...
				INNER JOIN
				(
					SELECT A.PEGAWAI_ID, A.JADWAL_PEGAWAI_ID, G.LEVEL_ID, COUNT(1) JUMLAH_LEVEL
					FROM jadwal_pegawai A
					INNER JOIN jadwal_asesor B ON A.JADWAL_ASESOR_ID = B.JADWAL_ASESOR_ID
					INNER JOIN jadwal_tes C ON B.JADWAL_TES_ID = C.JADWAL_TES_ID
					INNER JOIN formula_atribut D ON C.FORMULA_ESELON_ID = D.FORMULA_ESELON_ID
					...
					INNER JOIN indikator_penilaian G ON G.LEVEL_ID = E.LEVEL_ID
					WHERE 1=1
					GROUP BY A.PEGAWAI_ID, A.JADWAL_PEGAWAI_ID, G.LEVEL_ID
				) G1 ON G1.LEVEL_ID = G.LEVEL_ID AND A.JADWAL_PEGAWAI_ID = G1.JADWAL_PEGAWAI_ID AND G1.PEGAWAI_ID = A.PEGAWAI_ID
				WHERE 1=1
				"; 
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(1) AS ROWCOUNT FROM jadwal_pegawai WHERE 1=1 ".$statement;
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = '$val' ";
		}
