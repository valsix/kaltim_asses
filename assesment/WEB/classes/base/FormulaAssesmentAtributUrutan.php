  ...
  class FormulaAssesmentAtributUrutan extends Entity{ 
    ...
    **/
	...
	function delete()
	{
        ...
        and FORMULA_ESELON_ID = ".$this->getField("FORMULA_ESELON_ID")."
        ";
    ...
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $formulaid, $statement='', $sOrder="ORDER BY UR.URUT,A.ASPEK_ID, A.ATRIBUT_ID asc")
	{
		$str = "
		SELECT
		...
		(
			...
			WHERE 1=1 AND A.FORMULA_ESELON_ID = ".$formulaid."
			UNION ALL
		...
		LEFT JOIN 
		( 
			SELECT * FROM formula_assesment_atribut_urutan WHERE FORMULA_ESELON_ID = ".$formulaid."
		) UR ON A.ATRIBUT_ID = UR.ATRIBUT_ID AND A.PERMEN_ID = UR.PERMEN_ID
		WHERE 1=1 and a.aspek_id=1
		AND EXISTS
		(
			SELECT 1
			FROM
			(
				...
				FROM formula_atribut
				WHERE 1=1 AND FORMULA_ESELON_ID =".$formulaid."
