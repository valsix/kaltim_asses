...
***************************************************************************************************** */
  ...
  ***/
  ...
  class RekapCetak extends Entity{ 
    ...
    **/
	...
	function selectByParamsMonitoringCfitHasilRekap($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $statementdetil='', $sorder="")
	{
		...
		(
			SELECT
				UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, B.PEGAWAI_ID, B.ujian_id
				, A.NAMA NAMA_PEGAWAI, A.NIP_BARU, A.NO_PESERTA
			...
			(
				SELECT
				A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				...
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					FROM
					(
						SELECT A.*
						FROM
						(
							...
							(
								SELECT
								A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.TIPE_UJIAN_ID, SUBSTR(TU.ID,1,2) ID, A.BANK_SOAL_ID
								...
								GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.TIPE_UJIAN_ID, SUBSTR(TU.ID,1,2), A.BANK_SOAL_ID
							) A
							INNER JOIN
							(
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) A
				LEFT JOIN
				(
					SELECT
						A.UJIAN_ID D_JTI, A.PEGAWAI_ID D_PID
						, A.ujian_id D_FAI, A.PARENT_ID D_ID, COUNT(A.PEGAWAI_ID) D_BN
					FROM
					(
						SELECT
						A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
						...
						WHERE 1=1
						GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
					...
					WHERE GRADE_PROSENTASE = 100
					AND A.ID = '0101'
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.PARENT_ID
				) A1 ON A.UJIAN_ID = A1.D_JTI AND A.PEGAWAI_ID = A1.D_PID AND A.ujian_id = A1.D_FAI AND A.ID = A1.D_ID
				LEFT JOIN
				(
					SELECT
						A.UJIAN_ID D_JTI, A.PEGAWAI_ID D_PID
					...
					AND A.ID = '0102'
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.PARENT_ID
				) A2 ON A.UJIAN_ID = A2.D_JTI AND A.PEGAWAI_ID = A2.D_PID AND A.ujian_id = A2.D_FAI AND A.ID = A2.D_ID
				LEFT JOIN
				(
					SELECT
						A.UJIAN_ID D_JTI, A.PEGAWAI_ID D_PID
					...
					AND A.ID = '0103'
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.PARENT_ID
				) A3 ON A.UJIAN_ID = A3.D_JTI AND A.PEGAWAI_ID = A3.D_PID AND A.ujian_id = A3.D_FAI AND A.ID = A3.D_ID
				LEFT JOIN
				(
					SELECT
						A.UJIAN_ID D_JTI, A.PEGAWAI_ID D_PID
					...
					AND A.ID = '0104'
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.PARENT_ID
				) A4 ON A.UJIAN_ID = A4.D_JTI AND A.PEGAWAI_ID = A4.D_PID AND A.ujian_id = A4.D_FAI AND A.ID = A4.D_ID
				INNER JOIN
				(
					SELECT A.UJIAN_TAHAP_ID, B.ID, A.ujian_id, A.UJIAN_ID, A.TIPE_UJIAN_ID
					FROM cat.ujian_tahap A
					INNER JOIN cat.tipe_ujian B ON A.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
					WHERE 1=1
					AND PARENT_ID = '0'
				) C ON A.ujian_id = C.ujian_id AND A.ID = C.ID AND A.UJIAN_ID = C.UJIAN_ID
				INNER JOIN
				(
					SELECT A.ujian_id, A.UJIAN_ID, SUBSTR(TU.ID,1,2) ID, SUM(A.JUMLAH_SOAL_UJIAN_TAHAP) JUMLAH_SOAL
					FROM cat.ujian_tahap A
					INNER JOIN cat.tipe_ujian TU ON TU.TIPE_UJIAN_ID = A.TIPE_UJIAN_ID
					GROUP BY A.ujian_id, A.UJIAN_ID, SUBSTR(TU.ID,1,2)
				) D ON A.ujian_id = D.ujian_id AND A.ID = D.ID AND A.UJIAN_ID = D.UJIAN_ID
				WHERE 1=1
		...
		(
			SELECT
				X.ujian_id, UJIAN_TAHAP_ID, SUM(1) AS JUMLAH_SOAL
			FROM cat.ujian_bank_soal X
			GROUP BY X.ujian_id, UJIAN_TAHAP_ID
		) B ON A.ujian_id = B.ujian_id AND B.UJIAN_TAHAP_ID = A.UJIAN_TAHAP_ID
		WHERE 1=1
    ...
    function getCountByParamsMonitoringCfitHasilRekapA($paramsArray=array(), $jadwaltesid, $tipeujianid, $statement="", $statementdetil="")
	{
		...
		(
			...
			(
				...
				INNER JOIN
				(
					SELECT A.UJIAN_TAHAP_ID, B.ID, A.ujian_id, A.TIPE_UJIAN_ID
					FROM cat.ujian_tahap A
					INNER JOIN cat.tipe_ujian B ON A.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
					WHERE 1=1
					AND PARENT_ID = '0'
				) C ON A.ujian_id = C.ujian_id AND A.ID = C.ID
				INNER JOIN
				(
					SELECT A.ujian_id, A.UJIAN_ID, SUBSTR(TU.ID,1,2) ID, SUM(A.JUMLAH_SOAL_UJIAN_TAHAP) JUMLAH_SOAL
					FROM cat.ujian_tahap A
    ...
    function selectByParamsMonitoringCfitHasilRekapBak($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $statementdetil='', $sorder="")
	{
		$str = "
		SELECT A.*
		, CASE
    ...
    function selectByParamsMonitoringPapiHasilBak1($paramsArray=array(),$limit=-1,$from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		$str = "
		SELECT
    ...
    function selectByParamsMonitoringKraepelin($paramsArray=array(),$limit=-1,$from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		...
		";
			for($x=1; $x <= 50; $x++)
			{
				...
				$str .= ", SUM(CASE WHEN A.X_DATA = ".$x." THEN COALESCE(B.Y_MIN_DATA,0) END) Y_MIN_DATA".$x;
				$str .= ", SUM(CASE WHEN A.X_DATA = ".$x." THEN COALESCE(BARIS_JUMLAH_SALAH,0) END) BARIS_JUMLAH_SALAH".$x;
				$str .= ", SUM(CASE WHEN A.X_DATA = ".$x." THEN COALESCE(BARIS_JUMLAH_BENAR,0) END) BARIS_JUMLAH_BENAR".$x;
			}
		...
		$str .= "
			...
			LEFT JOIN
			(
				SELECT XX.*, BARIS_JUMLAH_SALAH, BARIS_JUMLAH_BENAR
				FROM
				(
					SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, XX.X_DATA, MAX(XX.Y_DATA) Y_DATA, MIN(XX.Y_DATA) Y_MIN_DATA
				...
				LEFT JOIN
				(
					SELECT B.UJIAN_ID, B.PEGAWAI_ID, B.X_DATA, COUNT(1) BARIS_JUMLAH_SALAH
					FROM cat.KRAEPELIN_JAWAB A
					INNER JOIN
					(
						SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, X_DATA, Y_DATA, NILAI
						FROM cat_pegawai.UJIAN_KRAEPELIN_".$jadwaltesid." XX
						WHERE 1=1
					) B ON A.X_DATA = B.X_DATA AND A.Y_DATA = B.Y_DATA AND A.NILAI != B.NILAI
					WHERE 1=1 and a.pakai_kraepelin_id = 1
					GROUP BY B.UJIAN_ID, B.PEGAWAI_ID, B.X_DATA
					ORDER BY B.UJIAN_ID, B.PEGAWAI_ID, B.X_DATA
				) B1 ON XX.UJIAN_ID = B1.UJIAN_ID AND XX.PEGAWAI_ID = B1.PEGAWAI_ID AND XX.X_DATA = B1.X_DATA
				LEFT JOIN
				(
					SELECT B.UJIAN_ID, B.PEGAWAI_ID, B.X_DATA, COUNT(1) BARIS_JUMLAH_BENAR
					FROM cat.KRAEPELIN_JAWAB A
					INNER JOIN
					(
						SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, X_DATA, Y_DATA, NILAI
					...
					GROUP BY B.UJIAN_ID, B.PEGAWAI_ID, B.X_DATA
					ORDER BY B.UJIAN_ID, B.PEGAWAI_ID, B.X_DATA
				) B2 ON XX.UJIAN_ID = B2.UJIAN_ID AND XX.PEGAWAI_ID = B2.PEGAWAI_ID AND XX.X_DATA = B2.X_DATA
			) B ON A.X_DATA = B.X_DATA
			GROUP BY B.PEGAWAI_ID
		) XX ON XX.PEGAWAI_ID = B.PEGAWAI_ID
		LEFT JOIN
		(
			SELECT XX.UJIAN_ID, XX.PEGAWAI_ID
			...
			(
				...
				(
					...
					AND XX.NILAI IS NOT NULL
					GROUP BY XX.UJIAN_ID, XX.PEGAWAI_ID, XX.X_DATA
				) X WHERE XX.UJIAN_ID = X.UJIAN_ID AND XX.PEGAWAI_ID = X.PEGAWAI_ID AND XX.X_DATA = X.X_DATA AND XX.Y_DATA <= X.Y_DATA
			)
			GROUP BY XX.UJIAN_ID, XX.PEGAWAI_ID
		) JL ON JL.UJIAN_ID = B.UJIAN_ID AND JL.PEGAWAI_ID = B.PEGAWAI_ID
		LEFT JOIN
		(
			SELECT B.UJIAN_ID, B.PEGAWAI_ID, COUNT(1) JUMLAH_SALAH
			FROM cat.KRAEPELIN_JAWAB A
			INNER JOIN
			(
				SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, X_DATA, Y_DATA, NILAI
			...
			WHERE 1=1 and a.pakai_kraepelin_id = 1
			GROUP BY B.UJIAN_ID, B.PEGAWAI_ID
		) JS ON JS.UJIAN_ID = B.UJIAN_ID AND JS.PEGAWAI_ID = B.PEGAWAI_ID
		LEFT JOIN
		(
			SELECT B.UJIAN_ID, B.PEGAWAI_ID, COUNT(1) JUMLAH_BENAR
			FROM cat.KRAEPELIN_JAWAB A
			INNER JOIN
			(
				SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, X_DATA, Y_DATA, NILAI
			...
			WHERE 1=1 and a.pakai_kraepelin_id = 1
			GROUP BY B.UJIAN_ID, B.PEGAWAI_ID
		) JB ON JB.UJIAN_ID = B.UJIAN_ID AND JB.PEGAWAI_ID = B.PEGAWAI_ID
		LEFT JOIN
		(
			SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, MAX(XX.Y_DATA) Y_MAX_DATA, MIN(XX.Y_DATA) Y_MIN_DATA
			FROM cat_pegawai.UJIAN_KRAEPELIN_".$jadwaltesid." XX
			WHERE 1=1
			AND XX.NILAI IS NOT NULL
			GROUP BY XX.UJIAN_ID, XX.PEGAWAI_ID
			ORDER BY XX.UJIAN_ID, XX.PEGAWAI_ID
		) MM ON MM.UJIAN_ID = B.UJIAN_ID AND MM.PEGAWAI_ID = B.PEGAWAI_ID
		LEFT JOIN
		(
			SELECT XX.UJIAN_ID, XX.PEGAWAI_ID
			, COALESCE(Y_MAX_DATA, 0) Y_MAX_DATA
			, COALESCE(Y_MIN_DATA, 0) Y_MIN_DATA
			FROM
			(
				SELECT XX.UJIAN_ID, XX.PEGAWAI_ID
				FROM cat_pegawai.UJIAN_KRAEPELIN_".$jadwaltesid." XX
			...
			LEFT JOIN
			(
				SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MAX_DATA
				FROM
				(
					SELECT
					XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MAX_DATA, XX.JUMLAH_MAX_DATA
					, ROW_NUMBER () OVER ( PARTITION BY XX.UJIAN_ID, XX.PEGAWAI_ID ORDER BY XX.JUMLAH_MAX_DATA DESC) URUT
					FROM
					(
						SELECT
						XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MAX_DATA, COUNT(XX.Y_MAX_DATA) JUMLAH_MAX_DATA
						FROM
						(
							SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, XX.X_DATA, MAX(XX.Y_DATA) Y_MAX_DATA, MIN(XX.Y_DATA) Y_MIN_DATA
							FROM cat_pegawai.UJIAN_KRAEPELIN_".$jadwaltesid." XX
							WHERE 1=1
							AND XX.NILAI IS NOT NULL
							GROUP BY XX.UJIAN_ID, XX.PEGAWAI_ID, XX.X_DATA
						...
						GROUP BY XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MAX_DATA
					) XX
					ORDER BY XX.UJIAN_ID, XX.PEGAWAI_ID, XX.JUMLAH_MAX_DATA DESC
				) XX
				WHERE URUT = 1
			) XX1 ON XX.UJIAN_ID = XX1.UJIAN_ID AND XX.PEGAWAI_ID = XX1.PEGAWAI_ID
			LEFT JOIN
			(
				SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MIN_DATA
				FROM
				(
					SELECT
					XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MIN_DATA, XX.JUMLAH_MIN_DATA
					, ROW_NUMBER () OVER ( PARTITION BY XX.UJIAN_ID, XX.PEGAWAI_ID ORDER BY XX.JUMLAH_MIN_DATA DESC) URUT
					FROM
					(
						SELECT
						XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MIN_DATA, COUNT(XX.Y_MIN_DATA) JUMLAH_MIN_DATA
						FROM
						(
							SELECT XX.UJIAN_ID, XX.PEGAWAI_ID, XX.X_DATA, MAX(XX.Y_DATA) Y_MAX_DATA, MIN(XX.Y_DATA) Y_MIN_DATA
							FROM cat_pegawai.UJIAN_KRAEPELIN_".$jadwaltesid." XX
						...
						GROUP BY XX.UJIAN_ID, XX.PEGAWAI_ID, XX.Y_MIN_DATA
					) XX
					ORDER BY XX.UJIAN_ID, XX.PEGAWAI_ID, XX.JUMLAH_MIN_DATA DESC
				) XX
				WHERE URUT = 1
			) XX2 ON XX.UJIAN_ID = XX2.UJIAN_ID AND XX.PEGAWAI_ID = XX2.PEGAWAI_ID
			WHERE 1=1
		) MM1 ON MM1.UJIAN_ID = B.UJIAN_ID AND MM1.PEGAWAI_ID = B.PEGAWAI_ID
		WHERE 1=1
		";
	...
	function getCountByParamsMonitoringKraepelin($paramsArray=array(), $jadwaltesid, $statement="")
	{
		...
		WHERE 1=1
		AND B.UJIAN_ID = ".$jadwaltesid.$statement;
    ...
    function selectByParamsMonitoringKraepelinBak($paramsArray=array(),$limit=-1,$from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		...
		(
			...
			INNER JOIN jadwal_tes B ON JADWAL_AWAL_TES_SIMULASI_ID = UJIAN_ID
			WHERE JADWAL_AWAL_TES_SIMULASI_ID = ".$jadwaltesid."
		) JA ON JA.PEGAWAI_ID = A.PEGAWAI_ID
		WHERE 1=1
		AND B.UJIAN_ID = ".$jadwaltesid."
		";
    ...
    function selectByParamsJadwalAcara($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY A.UJIAN_ID, A.PUKUL1, PD.ASESOR_ID, NOMOR_URUT_GENERATE")
	{
		$str = "
		SELECT 
			A.UJIAN_ID, JT.TANGGAL_TES
			, 
			CASE WHEN JA.LAST_UPDATE_DATE IS NULL THEN '' ELSE GENERATEZERO(CAST(JA.NOMOR_URUT AS TEXT), 2) END NOMOR_URUT_GENERATE
			, COALESCE(B.NAMA, 'Potensi') PENGGALIAN_NAMA, A.PUKUL1, A.PUKUL2
			, PD.* 
		FROM jadwal_acara A
		INNER JOIN jadwal_tes JT ON A.UJIAN_ID = JT.UJIAN_ID
		...
		) JA ON JA.PEGAWAI_ID = PD.PEGAWAI_ID AND A.UJIAN_ID = JA.JADWAL_AWAL_TES_SIMULASI_ID
		WHERE 1=1
		"; 
	...
	function selectByParamsMonitoringEppsHasilBak($paramsArray=array(),$limit=-1,$from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		$str = "
		SELECT
			UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, B.PEGAWAI_ID, B.ujian_id
		...
		";
			...
			for($r=1; $r <= 15; $r++)
			{
				...
				for($y=0; $y < 15; $y++)
				{
					...
					if($kolom == 1)
					{
						...
						// echo $tempKondisi."<br/>";
						if(
						$tempKondisi == 1 || $tempKondisi == 26 || $tempKondisi == 51 ||
						$tempKondisi == 7 || $tempKondisi == 32 || $tempKondisi == 57 ||
						$tempKondisi == 13 || $tempKondisi == 38 || $tempKondisi == 63 ||
						$tempKondisi == 19 || $tempKondisi == 44 || $tempKondisi == 69 ||
						$tempKondisi == 25 || $tempKondisi == 50 || $tempKondisi == 75
						)
							continue;
					}
					elseif($kolom == 2)
					{
						...
						// echo $tempKondisi."<br/>";
						if(
						$tempKondisi == 101 || $tempKondisi == 107 || $tempKondisi == 113 ||
						$tempKondisi == 119 || $tempKondisi == 125
						)
							continue;
					}
					elseif($kolom == 3)
					...
					$tempValue= $separator." SUM(CASE WHEN XX.URUT = ".$x." + (".$y." * 5) AND Y.GRADE_A = 1 THEN 1 ELSE 0 END) ";
			...
			for($r=0; $r < 15; $r++)
			{
				...
				for($y=0; $y < 15; $y++)
				{
					...
					if(
					$tempValueKondisi == 1 || $tempValueKondisi == 26 || $tempValueKondisi == 51 ||
					$tempValueKondisi == 7 || $tempValueKondisi == 32 || $tempValueKondisi == 57 ||
					$tempValueKondisi == 13 || $tempValueKondisi == 38 || $tempValueKondisi == 63 ||
					$tempValueKondisi == 19 || $tempValueKondisi == 44 || $tempValueKondisi == 69 ||
					$tempValueKondisi == 25 || $tempValueKondisi == 50 || $tempValueKondisi == 75 ||
					$tempValueKondisi == 101 || $tempValueKondisi == 107 || $tempValueKondisi == 113 ||
					$tempValueKondisi == 119 || $tempValueKondisi == 125 ||
					$tempValueKondisi == 151 || $tempValueKondisi == 201 || $tempValueKondisi == 157 ||
					$tempValueKondisi == 207 || $tempValueKondisi == 163 || $tempValueKondisi == 213 ||
					$tempValueKondisi == 169 || $tempValueKondisi == 219 || $tempValueKondisi == 175 || $tempValueKondisi == 225
					)
						continue;

					$tempValue= $separator." SUM(CASE WHEN XX.URUT = ".$tempValueKondisi."  AND Y.GRADE_B = 1 THEN 1 ELSE 0 END) ";
			...
			WHERE 1=1
			GROUP BY XX.PEGAWAI_ID, XX.UJIAN_ID, XX.ujian_id
	...
	function selectByParamsMonitoringEppsHasil($paramsArray=array(),$limit=-1,$from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		...
		// SELECT
		// XX.PEGAWAI_ID, XX.UJIAN_ID
		// , SUM(CASE WHEN XX.URUT = 1 OR XX.URUT = 151 THEN Y.GRADE_A END) ROW1,
		// SUM(CASE WHEN XX.URUT = 7 OR XX.URUT = 157 THEN Y.GRADE_A END) ROW2,
		// SUM(CASE WHEN XX.URUT = 13 OR XX.URUT = 163 THEN Y.GRADE_A END) ROW3,
		// SUM(CASE WHEN XX.URUT = 19 OR XX.URUT = 169 THEN Y.GRADE_A END) ROW4,
		// SUM(CASE WHEN XX.URUT = 25 OR XX.URUT = 175 THEN Y.GRADE_A END) ROW5,
		// SUM(CASE WHEN XX.URUT = 26 OR XX.URUT = 101 THEN Y.GRADE_A END) ROW6,
		// SUM(CASE WHEN XX.URUT = 32 OR XX.URUT = 107 THEN Y.GRADE_A END) ROW7,
		// SUM(CASE WHEN XX.URUT = 38 OR XX.URUT = 113 THEN Y.GRADE_A END) ROW8,
		// SUM(CASE WHEN XX.URUT = 44 OR XX.URUT = 119 THEN Y.GRADE_A END) ROW9,
		// SUM(CASE WHEN XX.URUT = 50 OR XX.URUT = 125 THEN Y.GRADE_A END) ROW10,
		// SUM(CASE WHEN XX.URUT = 51 OR XX.URUT = 201 THEN Y.GRADE_A END) ROW11,
		// SUM(CASE WHEN XX.URUT = 57 OR XX.URUT = 207 THEN Y.GRADE_A END) ROW12,
		// SUM(CASE WHEN XX.URUT = 63 OR XX.URUT = 213 THEN Y.GRADE_A END) ROW13,
		// SUM(CASE WHEN XX.URUT = 69 OR XX.URUT = 219 THEN Y.GRADE_A END) ROW14,
		// SUM(CASE WHEN XX.URUT = 75 OR XX.URUT = 225 THEN Y.GRADE_A END) ROW15
		// FROM cat_pegawai.ujian_pegawai_".$jadwaltesid." XX
		// INNER JOIN cat.epps_pilihan Y ON Y.SOAL_EPPS_ID = XX.BANK_SOAL_ID AND Y.EPPS_PILIHAN_ID = XX.BANK_SOAL_PILIHAN_ID
		// WHERE 1=1
		// AND XX.TIPE_UJIAN_ID = 17
		// AND URUT IN (1, 151, 7, 157, 13, 163, 19, 169, 25, 175, 26, 101, 32, 107, 38, 113, 44, 119, 50, 125, 51, 201, 57, 207, 63, 213, 69, 219, 75, 225)
		// GROUP BY XX.PEGAWAI_ID, XX.UJIAN_ID, XX.ujian_id
    ...
    function selectByParamsMonitoringIst($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		$str = "
		...
		(
			...
			(
				SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				...
				(
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) SE ON A.UJIAN_ID = SE.UJIAN_ID AND A.PEGAWAI_ID = SE.PEGAWAI_ID AND A.ujian_id = SE.ujian_id
				LEFT JOIN
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) WA ON A.UJIAN_ID = WA.UJIAN_ID AND A.PEGAWAI_ID = WA.PEGAWAI_ID AND A.ujian_id = WA.ujian_id
				LEFT JOIN
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) AN ON A.UJIAN_ID = AN.UJIAN_ID AND A.PEGAWAI_ID = AN.PEGAWAI_ID AND A.ujian_id = AN.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
					...
					(
						SELECT
						A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(B.NILAI) NILAI
				...
				) GE ON A.UJIAN_ID = GE.UJIAN_ID AND A.PEGAWAI_ID = GE.PEGAWAI_ID AND A.ujian_id = GE.ujian_id
				LEFT JOIN
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) ME ON A.UJIAN_ID = ME.UJIAN_ID AND A.PEGAWAI_ID = ME.PEGAWAI_ID AND A.ujian_id = ME.ujian_id
				LEFT JOIN
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) RA ON A.UJIAN_ID = RA.UJIAN_ID AND A.PEGAWAI_ID = RA.PEGAWAI_ID AND A.ujian_id = RA.ujian_id
				LEFT JOIN
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) ZR ON A.UJIAN_ID = ZR.UJIAN_ID AND A.PEGAWAI_ID = ZR.PEGAWAI_ID AND A.ujian_id = ZR.ujian_id
				LEFT JOIN
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) FA ON A.UJIAN_ID = FA.UJIAN_ID AND A.PEGAWAI_ID = FA.PEGAWAI_ID AND A.ujian_id = FA.ujian_id
				LEFT JOIN
				(
					SELECT 
					A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
					...
					WHERE 1=1
					GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.ID
				) WU ON A.UJIAN_ID = WU.UJIAN_ID AND A.PEGAWAI_ID = WU.PEGAWAI_ID AND A.ujian_id = WU.ujian_id
				WHERE 1=1
				".$statement."
			) A
		) A
    ...
    function getCountByParamsMonitoringIst($paramsArray=array(), $jadwaltesid, $statement="")
	{
		...
		(
			SELECT
			A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.NAMA_PEGAWAI, A.NIP_BARU
			FROM
			(
				SELECT
				UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, B.PEGAWAI_ID, B.ujian_id
	...
	function selectByParamsMonitoringPauli($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		...
		(
			...
			, CASE WHEN COALESCE(NULLIF(A.PENDIDIKAN, ''), NULL) IS NULL THEN 'S1' ELSE PENDIDIKAN END PEGAWAI_PENDIDIKAN
			, A.NAMA NAMA_PEGAWAI, A.EMAIL NIP_BARU
			FROM cat.ujian_pegawai_daftar B
			INNER JOIN simpeg.pegawai A ON B.PEGAWAI_ID = A.PEGAWAI_ID
			WHERE 1=1
		...
		(
			...
			(
				SELECT
				A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.NOMOR
				, KOLOM1 + KOLOM2 + KOLOM3 JUMLAH
				FROM cat_pegawai.ujian_pauli_tanda_".$jadwaltesid." A
				WHERE 1=1
			) A
			GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
		) RW ON A.UJIAN_ID = RW.UJIAN_ID AND A.PEGAWAI_ID = RW.PEGAWAI_ID AND A.ujian_id = RW.ujian_id
		WHERE 1=1
		".$statement;
    ...
    function selectByParamsMonitoringPf16($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		$str = "
		SELECT
			A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, A.NO_PESERTA
			, A.NAMA_PEGAWAI, A.NIP_BARU, A.EMAIL, a.totalx, a.sudahx
		...
		(
			...
			(
				SELECT A.*
				, 105 TOTALx, COALESCE(JML.jumlah_sudah,0) SUDAHx
				...
				(
					SELECT
					UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, B.PEGAWAI_ID, B.ujian_id, A.NO_PESERTA
					, A.JENIS_KELAMIN PEGAWAI_JENIS_KELAMIN
					, CASE WHEN COALESCE(NULLIF(C.KODE, ''), NULL) IS NULL THEN 'S1' ELSE PENDIDIKAN END PEGAWAI_PENDIDIKAN
					, A.NAMA NAMA_PEGAWAI, A.EMAIL, A.NIP_BARU
					FROM cat.ujian_pegawai_daftar B
					INNER JOIN simpeg.pegawai A ON B.PEGAWAI_ID = A.PEGAWAI_ID
					LEFT JOIN simpeg.pendidikan C ON A.PENDIDIKAN = TO_CHAR(C.PENDIDIKAN_ID,'9999')
					WHERE 1=1
				) A
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					FROM cat_pegawai.ujian_pegawai_".$jadwaltesid." A
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_MD ON A.UJIAN_ID = RW_MD.UJIAN_ID AND A.PEGAWAI_ID = RW_MD.PEGAWAI_ID AND A.ujian_id = RW_MD.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_A ON A.UJIAN_ID = RW_A.UJIAN_ID AND A.PEGAWAI_ID = RW_A.PEGAWAI_ID AND A.ujian_id = RW_A.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_B ON A.UJIAN_ID = RW_B.UJIAN_ID AND A.PEGAWAI_ID = RW_B.PEGAWAI_ID AND A.ujian_id = RW_B.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_C ON A.UJIAN_ID = RW_C.UJIAN_ID AND A.PEGAWAI_ID = RW_C.PEGAWAI_ID AND A.ujian_id = RW_C.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_E ON A.UJIAN_ID = RW_E.UJIAN_ID AND A.PEGAWAI_ID = RW_E.PEGAWAI_ID AND A.ujian_id = RW_E.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_F ON A.UJIAN_ID = RW_F.UJIAN_ID AND A.PEGAWAI_ID = RW_F.PEGAWAI_ID AND A.ujian_id = RW_F.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_G ON A.UJIAN_ID = RW_G.UJIAN_ID AND A.PEGAWAI_ID = RW_G.PEGAWAI_ID AND A.ujian_id = RW_G.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_H ON A.UJIAN_ID = RW_H.UJIAN_ID AND A.PEGAWAI_ID = RW_H.PEGAWAI_ID AND A.ujian_id = RW_H.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_I ON A.UJIAN_ID = RW_I.UJIAN_ID AND A.PEGAWAI_ID = RW_I.PEGAWAI_ID AND A.ujian_id = RW_I.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_L ON A.UJIAN_ID = RW_L.UJIAN_ID AND A.PEGAWAI_ID = RW_L.PEGAWAI_ID AND A.ujian_id = RW_L.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_M ON A.UJIAN_ID = RW_M.UJIAN_ID AND A.PEGAWAI_ID = RW_M.PEGAWAI_ID AND A.ujian_id = RW_M.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_N ON A.UJIAN_ID = RW_N.UJIAN_ID AND A.PEGAWAI_ID = RW_N.PEGAWAI_ID AND A.ujian_id = RW_N.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_O ON A.UJIAN_ID = RW_O.UJIAN_ID AND A.PEGAWAI_ID = RW_O.PEGAWAI_ID AND A.ujian_id = RW_O.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_Q1 ON A.UJIAN_ID = RW_Q1.UJIAN_ID AND A.PEGAWAI_ID = RW_Q1.PEGAWAI_ID AND A.ujian_id = RW_Q1.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_Q2 ON A.UJIAN_ID = RW_Q2.UJIAN_ID AND A.PEGAWAI_ID = RW_Q2.PEGAWAI_ID AND A.ujian_id = RW_Q2.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_Q3 ON A.UJIAN_ID = RW_Q3.UJIAN_ID AND A.PEGAWAI_ID = RW_Q3.PEGAWAI_ID AND A.ujian_id = RW_Q3.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, SUM(GRADE_PROSENTASE) NILAI
					...
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				) RW_Q4 ON A.UJIAN_ID = RW_Q4.UJIAN_ID AND A.PEGAWAI_ID = RW_Q4.PEGAWAI_ID AND A.ujian_id = RW_Q4.ujian_id
				LEFT JOIN
				(
					SELECT
					A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id, count(A.BANK_SOAL_PILIHAN_ID) jumlah_sudah
					FROM cat_pegawai.ujian_pegawai_".$jadwaltesid." A
					INNER JOIN cat.bank_soal C ON A.BANK_SOAL_ID = C.BANK_SOAL_ID
					WHERE C.tipe_ujian_id = 40 
					GROUP BY A.UJIAN_PEGAWAI_DAFTAR_ID, A.UJIAN_ID, A.PEGAWAI_ID, A.ujian_id
				)JML ON A.UJIAN_ID = JML.UJIAN_ID AND A.PEGAWAI_ID = JML.PEGAWAI_ID AND A.ujian_id = JML.ujian_id
				WHERE 1=1
			) A
		) A
		WHERE 1=1
	...
	{
		$arrayData= array(
			array("WHERE"=>"1, 8, 15, 22, 29, 36, 43, 50, 57, 64", "BAGI"=>10, "DIMENSI_KIRI"=>"E", "DIMENSI_KANAN"=>"I", "DIMENSI_KIRI_VALUE"=>5, "DIMENSI_KANAN_VALUE"=>9)
			, array("WHERE"=>"2, 3, 9, 10, 16, 17, 23, 24, 30, 31, 37, 38, 44, 45, 51, 52, 58, 59, 65, 66", "BAGI"=>20, "DIMENSI_KIRI"=>"S", "DIMENSI_KANAN"=>"N", "DIMENSI_KIRI_VALUE"=>19, "DIMENSI_KANAN_VALUE"=>14)
			, array("WHERE"=>"4, 5, 11, 12, 18, 19, 25, 26, 32, 33, 39, 40, 46, 47, 53, 54, 60, 61, 67, 68", "BAGI"=>20, "DIMENSI_KIRI"=>"T", "DIMENSI_KANAN"=>"F", "DIMENSI_KIRI_VALUE"=>20, "DIMENSI_KANAN_VALUE"=>6)
			, array("WHERE"=>"6, 7, 13, 14, 20, 21, 27, 28, 34, 35, 41, 42, 48, 49, 55, 56, 62, 63, 69, 70", "BAGI"=>20, "DIMENSI_KIRI"=>"J", "DIMENSI_KANAN"=>"P", "DIMENSI_KIRI_VALUE"=>10, "DIMENSI_KANAN_VALUE"=>16)
		);
		...
		(
			...
			, A.JENIS_KELAMIN PEGAWAI_JENIS_KELAMIN
			, CASE WHEN COALESCE(NULLIF(A.PENDIDIKAN, ''), NULL) IS NULL THEN 'S1' ELSE PENDIDIKAN END PEGAWAI_PENDIDIKAN
			, A.NAMA NAMA_PEGAWAI, A.EMAIL NIP_BARU, A.NO_PESERTA
			FROM cat.ujian_pegawai_daftar B
			INNER JOIN simpeg.pegawai A ON B.PEGAWAI_ID = A.PEGAWAI_ID
			WHERE 1=1
		) A
    ...
    function selectByParamsMonitoringDisc($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		...
		(
			SELECT
			UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, B.PEGAWAI_ID, B.ujian_id LOWONGAN_ID
			, A.JENIS_KELAMIN PEGAWAI_JENIS_KELAMIN
			, A.NAMA NAMA_PEGAWAI, A.EMAIL, A.NIP_BARU
			FROM cat.ujian_pegawai_daftar B
			INNER JOIN simpeg.pegawai A ON B.PEGAWAI_ID = A.PEGAWAI_ID
		...
		";
			...
			";
				...
				$str .= " 
				SELECT
				A.UJIAN_ID, A.PEGAWAI_ID, A.UJIAN_TAHAP_ID, A.NILAI_P_1,jadwal_tes_id
				";
				for($y=1; $y<=2; $y++)
				{
					$ylabel= $y - 1;
		 		...
		 		";
					...
					";
						...
						$str .= " SELECT A.* ";
							...
							(
								SELECT
								A.UJIAN_ID, A.PEGAWAI_ID, A.UJIAN_TAHAP_ID,jadwal_tes_id
							";
							...
							GROUP BY A.UJIAN_ID, A.PEGAWAI_ID, A.UJIAN_TAHAP_ID,jadwal_tes_id
							) A
						) A
				) A
			) A
	...
	function selectByParamsMMPI($paramsArray=array(),$limit=-1,$from=-1, $reqId, $statement='', $sOrder="")
	{
		$str = "
		SELECT
			A.UJIAN_ID, B.UJIAN_BANK_SOAL_ID, B.BANK_SOAL_ID, D.MMPI_PILIHAN_ID BANK_SOAL_PILIHAN_ID, F.NAMA PEGAWAI_NAMA, C.PERTANYAAN, D.JAWABAN
			, A.STATUS_SETUJU, A.UJIAN_PEGAWAI_DAFTAR_ID, C.TIPE_UJIAN_ID TIPE_SOAL,
			CASE
			WHEN D.JAWABAN = CONCAT('<div class=\"icon minus\"></div>') THEN '-'
			WHEN D.JAWABAN = CONCAT('<div class=\"icon plus\"></div>') THEN '+'
			END JAWABAN_INFO
		FROM cat.ujian_pegawai_daftar A
		INNER JOIN cat.ujian_bank_soal B ON A.ujian_id = B.ujian_id AND A.UJIAN_ID = B.UJIAN_ID
		...
		INNER JOIN cat_pegawai.ujian_pegawai_".$reqId." E ON E.BANK_SOAL_PILIHAN_ID = D.MMPI_PILIHAN_ID AND A.PEGAWAI_ID = E.PEGAWAI_ID
		INNER JOIN simpeg.pegawai F ON F.PEGAWAI_ID = E.PEGAWAI_ID
		WHERE 1=1 "; 
		...
		$str .= $statement." ORDER BY A.UJIAN_ID, B.BANK_SOAL_ID, D.MMPI_PILIHAN_ID ".$sOrder;
		$this->query = $str;
				
		return $this->selectLimit($str,$limit,$from); 
    }

    function getCountByParamsMMPI($paramsArray=array(), $statement="")
    {
    	$str = "
    	SELECT COUNT(1) AS ROWCOUNT 
    	FROM MMPI_SOAL A
		INNER JOIN cat.MMPI_PILIHAN B ON B.MMPI_SOAL_ID = A.MMPI_SOAL_ID
    	WHERE 1=1
    	"; 
    	while(list($key,$val)=each($paramsArray))
    	{
    ...
    function selectByParamsDataMmpi($paramsArray=array(),$limit=-1,$from=-1, $jadwaltesid, $statement='', $sOrder="ORDER BY B.BANK_SOAL_ID ASC")
	{
		$str = "
		SELECT
		A.PEGAWAI_ID, B.PERTANYAAN, B.INFO_TANDA
		FROM cat.ujian_pegawai_daftar A
		LEFT JOIN
		(
			SELECT A.UJIAN_ID, C.PEGAWAI_ID, A.BANK_SOAL_ID, B.PERTANYAAN, INFO_TANDA
			FROM cat.ujian_bank_soal A
			INNER JOIN cat.mmpi_soal B ON A.BANK_SOAL_ID = B.MMPI_SOAL_ID
			INNER JOIN
			(
				SELECT A.UJIAN_ID, A.PEGAWAI_ID, B.MMPI_SOAL_ID, CASE WHEN B.GRADE_A = 1 THEN 1 ELSE 0 END INFO_TANDA
				FROM cat_pegawai.ujian_pegawai_".$jadwaltesid." A
				INNER JOIN cat.mmpi_pilihan B ON A.BANK_SOAL_PILIHAN_ID = B.MMPI_PILIHAN_ID
				WHERE TIPE_UJIAN_ID = 66
			) C ON B.MMPI_SOAL_ID = C.MMPI_SOAL_ID AND A.UJIAN_ID = C.UJIAN_ID
			WHERE A.TIPE_UJIAN_ID = 66
    ...
    function selectByParamsRekapMmpi($paramsArray=array(),$limit=-1,$from=-1, $jadwaltesid, $statement='', $sOrder="")
	{
		$str = "
		SELECT
		A.PEGAWAI_ID, c.NO_PESERTA, C.NAMA NAMA_PEGAWAI, C.EMAIL NIP_BARU, COALESCE(PLUS,0) PLUS, COALESCE(NEGATIP,0) NEGATIP
		FROM cat.ujian_pegawai_daftar A
		LEFT JOIN
		(
			SELECT A.PEGAWAI_ID, A.TIPE_UJIAN_ID, SUM(GRADE_A) PLUS, SUM(GRADE_B) NEGATIP
			FROM cat_pegawai.ujian_pegawai_".$jadwaltesid." A
			INNER JOIN cat.mmpi_pilihan B ON B.MMPI_PILIHAN_ID = A.BANK_SOAL_PILIHAN_ID
			WHERE 1=1 AND A.TIPE_UJIAN_ID = 66
			GROUP BY A.PEGAWAI_ID, A.TIPE_UJIAN_ID
    ...
    function getCountByParamsRekapMmpi($paramsArray=array(), $jadwaltesid, $statement="")
    {
    	$str = "
    	SELECT COUNT(1) AS ROWCOUNT 
    	FROM cat.ujian_pegawai_daftar A
    ...
    function selectByParamsInfoPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
	{
		...
		SELECT
			...
			, cat.NORMA_UMUR(B.UJIAN_ID, B.PEGAWAI_ID) PEGAWAI_UMUR_NORMA
			, A.NAMA NAMA_PEGAWAI, A.EMAIL NIP_BARU, A.JENIS_KELAMIN
		FROM cat.ujian_pegawai_daftar B
    ...
    function selectByParamsMonitoringBigFive($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		...
		(
			SELECT
			UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, B.PEGAWAI_ID, B.ujian_id JADWAL_TES_ID
			, A.JENIS_KELAMIN PEGAWAI_JENIS_KELAMIN
    ...
    function selectByParamsMonitoringHolland($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $statementDetil='', $sorder="")
	{
		...
		LEFT JOIN
		(
			SELECT  A.ujian_id JADWAL_TES_ID, A.PEGAWAI_ID
		...
		) HSL ON HSL.PEGAWAI_ID = B.PEGAWAI_ID AND HSL.JADWAL_TES_ID = JADWAL_TES_ID
		LEFT JOIN
		(
			SELECT ROW_NUMBER() OVER(ORDER BY A.LAST_UPDATE_DATE) NOMOR_URUT, A.PEGAWAI_ID, A.LAST_UPDATE_DATE
			FROM jadwal_awal_tes_simulasi_pegawai A
...
function getCountByParamsMonitoringHolland($paramsArray=array(), $jadwaltesid, $statement="")
	...
	function getCountByParamsMonitoringBelbin($paramsArray=array(), $jadwaltesid, $statement="")
	{
		$str = "
		SELECT COUNT(1) AS ROWCOUNT
		FROM
  ...
  function selectByParamsMonitoringBelbin($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		$arrdata= infobelbin();
		...
		{
			$infoid= $arrdata[$i]["id"];
			$str.=", COALESCE(B".$infoid.".TOTAL_NILAI,0) TOTAL_NILAI".$infoid;
		}
		...
		for($i=0; $i < count($arrdata); $i++)
		{
			...
			(
				SELECT
				A.ujian_id, A.PEGAWAI_ID
				, SUM(CAST(A.LAST_CREATE_USER AS NUMERIC)) TOTAL_NILAI
				FROM cat_pegawai.ujian_pegawai_".$jadwaltesid." A
				INNER JOIN cat.bank_soal B1 ON A.BANK_SOAL_ID = B1.BANK_SOAL_ID
				INNER JOIN cat.bank_soal_pilihan B2 ON A.BANK_SOAL_PILIHAN_ID = B2.BANK_SOAL_PILIHAN_ID
				WHERE 1=1 AND A.TIPE_UJIAN_ID = 77
				AND COALESCE(NULLIF(A.LAST_CREATE_USER, ''), NULL) IS NOT NULL
				AND
				(
			";
				$arrdetildata= infobelbinget($infoid);
				// print_r($arrdetildata);exit;
				for($x=0; $x < count($arrdetildata); $x++)
				{
					$infosoal= $arrdetildata[$x]["soal"];
					$infojawaban= $arrdetildata[$x]["jawaban"];

					if($x > 0)
						$str.=" OR ";

					$str.="(B1.PATH_SOAL = '".$infosoal."' AND B2.GRADE_PROSENTASE = ".$infojawaban.")";
				}
			$str.="
				)
				GROUP BY A.ujian_id, A.PEGAWAI_ID
			) B".$infoid." ON B".$infoid.".PEGAWAI_ID = B.PEGAWAI_ID AND B".$infoid.".ujian_id = B.ujian_id
			";
		}
	...
	function getCountByParamsMonitoringMSDT($paramsArray=array(), $jadwaltesid, $statement="")
	{
		$str = "
		SELECT COUNT(1) AS ROWCOUNT
				from cat.ujian_pegawai_daftar a 
			WHERE 1=1 and ujian_id =".$jadwaltesid; 
  ...
  function selectByParamsMonitoringMSDT($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='',$statementdetil='', $sorder=" ORDER BY ja.NOMOR_URUT ASC")
	{
		$str="
		select ja.NOMOR_URUT, xx.pegawai_id, nama, hasil_test, nip
		,hasil1 ,hasil2 ,hasil3 ,hasil4
		,hasil5 ,hasil6 ,hasil7 ,hasil8
		,f33 ,j33 ,n33
		,case when hasil_test='DESERTER' or hasil_test='COMPROMISER' or hasil_test='MISSIONARY' then 'KURANG EFEKTIF'
		else 'LEBIH EFEKTIF' end status
		from(
			select x.pegawai_id, x.nama, x.nip,f33,j33,n33,
			case 
			when f33>=34 and j33>=34 and n33>=34 then 'EXCECUTIVE'
			when f33>=34 and j33>=34 and n33<=34 then 'COMPROMISER'
			when f33>=34 and j33<=34 and n33>=34 then 'BENEVOLENT AUTOCRAT'
			when f33>=34 and j33<=34 and n33<=34 then 'AUTOCRAT'
			when f33<=34 and j33>=34 and n33>=34 then 'DEVELOPER'
			when f33<=34 and j33>=34 and n33<=34 then 'MISSIONARY'
			when f33<=34 and j33<=34 and n33>=34 then 'BUREAUCRAT'
			when f33<=34 and j33<=34 and n33<=34 then 'DESERTER'
			end hasil_test
			,hasil1 ,hasil2 ,hasil3 ,hasil4
			,hasil5 ,hasil6 ,hasil7 ,hasil8
			from (
				select a.pegawai_id, p.nama, p.nip
				,(nilai.hasil3+1)+(nilai.hasil4+0)+(nilai.hasil7+0)+(nilai.hasil8-4) f33
				,(nilai.hasil2+2)+(nilai.hasil4+0)+(nilai.hasil7-1)+(nilai.hasil8-4) j33
				,(nilai.hasil5+3)+(nilai.hasil6-1)+(nilai.hasil7+0)+(nilai.hasil8-4) n33
				,hasil1 ,hasil2 ,hasil3 ,hasil4
				,hasil5 ,hasil6 ,hasil7 ,hasil8
				from cat.ujian_pegawai_daftar a 
				...
				left join (
					select pegawai_id, 	
					sum(case when a.bank_soal_id between 1 and 8 then 1 else 0 end) hasil1,
					sum(case when a.bank_soal_id between 9 and 16 then 1 else 0 end) hasil2,
					sum(case when a.bank_soal_id between 17 and 24 then 1 else 0 end) hasil3,
					sum(case when a.bank_soal_id between 25 and 32 then 1 else 0 end) hasil4,
					sum(case when a.bank_soal_id between 33 and 40 then 1 else 0 end) hasil5,
					sum(case when a.bank_soal_id between 41 and 48 then 1 else 0 end) hasil6,
					sum(case when a.bank_soal_id between 49 and 56 then 1 else 0 end) hasil7,
					sum(case when a.bank_soal_id between 57 and 64 then 1 else 0 end) hasil8
					from cat_pegawai.ujian_pegawai_".$jadwaltesid." A
					left join cat.msdt_pilihan wp on a.bank_soal_pilihan_id = wp.msdt_pilihan_id
					where grade_a=1
					group by pegawai_id
				) nilai on a. pegawai_id = nilai. pegawai_id 
				where ujian_id=".$jadwaltesid."
			)x
		)xx
		LEFT JOIN
		(
			...
			INNER JOIN jadwal_tes B ON JADWAL_AWAL_TES_SIMULASI_ID = JADWAL_TES_ID
			WHERE JADWAL_AWAL_TES_SIMULASI_ID = ".$jadwaltesid."
		) JA ON JA.PEGAWAI_ID = xx.PEGAWAI_ID ";
	...
	function selectByParamsJawabanmsdt($paramsArray=array(), $limit=-1, $from=-1, $jadwaltesid, $statement='', $sorder="")
	{
		$str.="
			select case when GRADE_A = 1 then 'A' else 'B' end nilai
			from cat_pegawai.ujian_pegawai_".$jadwaltesid." A
			left join cat.msdt_pilihan wp on a.bank_soal_pilihan_id = wp.msdt_pilihan_id
			where 1=1 
		".$statement;
