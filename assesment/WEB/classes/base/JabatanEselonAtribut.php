  ...
  class JabatanEselonAtribut extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		...
		$this->setField("JABATAN_ESELON_ATRIBUT_ID", $this->getNextId("JABATAN_ESELON_ATRIBUT_ID","jabatan_eselon_atribut")); 

		$str = "INSERT INTO jabatan_eselon_atribut (
	...
	function delete()
	{
        $str = "DELETE FROM jabatan_eselon_atribut
                WHERE 
    ...
    **/ 
	function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY SATUAN_KERJA_ID ASC")
	{
		...
		FROM jabatan_eselon_atribut A WHERE 1=1 "; 
	...
	function selectByParamsPenggalianPenilaianJabatanAtributCombo($paramsArray=array(),$limit=-1,$from=-1, $reqTahun= "", $reqAspekId="", $statement='', $sOrder="ORDER BY A.ID_ROW ASC")
	{
		...
		$str = "
				...
				(
					...
					THEN ''
					ELSE
					link_penilaian_jabatan(A.JABATAN_ESELON_ATRIBUT_ID, A.TAHUN, A.ATRIBUT_ID, ' - ')
					END  LINK_URL
					FROM jabatan_eselon_atribut A
					INNER JOIN atribut B ON A.ATRIBUT_ID = B.ATRIBUT_ID AND A.TAHUN = B.TAHUN AND A.ASPEK_ID = B.ASPEK_ID
	...
	function selectByParamsPenggalianJabatanAtributCombo($paramsArray=array(),$limit=-1,$from=-1, $reqTahun= "", $reqAspekId="", $reqId="", $statement='', $sOrder="ORDER BY A.ID_ROW ASC")
	{
		...
		$str = "
				...
				(
					...
					CONCAT('<a style=\"display:none\" id=\"reqInfoSimpan',A.JABATAN_ESELON_ATRIBUT_ID,'\" onClick=\"pilihatribut(''',A.JABATAN_ESELON_ATRIBUT_ID,''', ''simpan'')\" style=\"cursor:pointer\" title=\"Pilih\"><img src=\"../WEB/images/icon_pilih.png\" width=\"15px\" heigth=\"15px\"></a><a id=\"reqInfoHapus',A.JABATAN_ESELON_ATRIBUT_ID,'\" onClick=\"pilihatribut(''',A.JABATAN_ESELON_ATRIBUT_ID,''', ''hapus'')\" style=\"cursor:pointer\" title=\"Pilih\"><img src=\"../WEB/images/icn_delete.png\" width=\"15px\" heigth=\"15px\"></a>')
					ELSE
					CONCAT('<a id=\"reqInfoSimpan',A.JABATAN_ESELON_ATRIBUT_ID,'\" onClick=\"pilihatribut(''',A.JABATAN_ESELON_ATRIBUT_ID,''', ''simpan'')\" style=\"cursor:pointer\" title=\"Pilih\"><img src=\"../WEB/images/icon_pilih.png\" width=\"15px\" heigth=\"15px\"></a><a style=\"display:none\" id=\"reqInfoHapus',A.JABATAN_ESELON_ATRIBUT_ID,'\" onClick=\"pilihatribut(''',A.JABATAN_ESELON_ATRIBUT_ID,''', ''hapus'')\" style=\"cursor:pointer\" title=\"Pilih\"><img src=\"../WEB/images/icn_delete.png\" width=\"15px\" heigth=\"15px\"></a>')
					END
					END  LINK_URL
					FROM jabatan_eselon_atribut A
					INNER JOIN atribut B ON A.ATRIBUT_ID = B.ATRIBUT_ID AND A.TAHUN = B.TAHUN AND A.ASPEK_ID = B.ASPEK_ID
					LEFT JOIN
					(
						SELECT PN.JABATAN_ESELON_ATRIBUT_ID, CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END STATUS
						FROM penggalian_penilaian PN 
						INNER JOIN penggalian P ON PN.PENGGALIAN_ID = P.PENGGALIAN_ID
						WHERE 1=1
						AND PN.PENGGALIAN_ID = 1
						GROUP BY PN.JABATAN_ESELON_ATRIBUT_ID
					) C ON A.JABATAN_ESELON_ATRIBUT_ID = C.JABATAN_ESELON_ATRIBUT_ID
					WHERE 1=1 ".$statement_aspek."
				) A
				WHERE 1=1
				"; 
	...
	function selectByParamsJabatanAtributLookup($paramsArray=array(),$limit=-1,$from=-1, $reqEselonId, $reqSatuanKerjaId, $statement='', $sOrder="")
	{
		$str = "
				...
				CASE WHEN (SELECT X.ATRIBUT_ID FROM jabatan_eselon_atribut X WHERE X.ESELON_ID = '".$reqEselonId."' AND X.SATUAN_KERJA_ID = '".$reqSatuanKerjaId."' AND A.ATRIBUT_ID = X.ATRIBUT_ID) = A.ATRIBUT_ID
				THEN '' 
				ELSE 
