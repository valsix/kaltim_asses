    ...
    **/ 
    ...
    function selectByParamsPenilaianAsesor($statement='', $sOrder="ORDER BY A.ASPEK_ID, A.ATRIBUT_ID")
	{
		...
		SELECT
			...
			, A.LEVEL, A.LEVEL_KETERANGAN, A.JADWAL_TES_ID, A.PEGAWAI_ID, A.ASPEK_ID, A.CATATAN_STRENGTH,A.CATATAN_WEAKNES,A.KESIMPULAN,A.SARAN_PENGEMBANGAN,A.SARAN_PENEMPATAN
		FROM 
		(
			...
			, B.PERMEN_ID, A.JADWAL_TES_ID, A.PEGAWAI_ID, A.ASPEK_ID,A.CATATAN_STRENGTH,A.CATATAN_WEAKNES,A.KESIMPULAN,A.SARAN_PENGEMBANGAN,A.SARAN_PENEMPATAN
			FROM penilaian A
			...
			, B.PROSENTASE, A.PERMEN_ID, B.JADWAL_TES_ID, B.PEGAWAI_ID, B.ASPEK_ID,B.CATATAN_STRENGTH,B.CATATAN_WEAKNES,B.KESIMPULAN,B.SARAN_PENGEMBANGAN,B.SARAN_PENEMPATAN
			FROM atribut A
			LEFT JOIN
			(
				SELECT B.PENILAIAN_ID, SUBSTR(B.ATRIBUT_ID, 1, 2) ATRIBUT_ID, COUNT(1) JUMLAH_PENILAIAN_DETIL
				, ROUND((SUM(B.NILAI) / SUM(B1.NILAI_STANDAR)) * 100,2) PROSENTASE, PERMEN_ID, B2.JADWAL_TES_ID, B2.PEGAWAI_ID, B2.ASPEK_ID,B2.CATATAN_STRENGTH,B2.CATATAN_WEAKNES,B2.KESIMPULAN,B2.SARAN_PENGEMBANGAN,B2.SARAN_PENEMPATAN
				FROM penilaian_detil B
				INNER JOIN formula_atribut B1 ON B1.FORMULA_ATRIBUT_ID = B.FORMULA_ATRIBUT_ID
				INNER JOIN penilaian B2 ON B.PENILAIAN_ID = B2.PENILAIAN_ID
				WHERE 1=1
				GROUP BY B.PENILAIAN_ID, SUBSTR(B.ATRIBUT_ID, 1, 2), PERMEN_ID, B2.JADWAL_TES_ID, B2.PEGAWAI_ID, B2.ASPEK_ID,B2.CATATAN_STRENGTH,B2.CATATAN_WEAKNES,B2.KESIMPULAN,B2.SARAN_PENGEMBANGAN,B2.SARAN_PENEMPATAN
    ...
    function updatePenilaianText()
	{
		...
		UPDATE penilaian
		SET
			CATATAN_STRENGTH= '".$this->getField("CATATAN_STRENGTH")."',
			CATATAN_WEAKNES= '".$this->getField("CATATAN_WEAKNES")."',
			KESIMPULAN= '".$this->getField("KESIMPULAN")."',
			SARAN_PENGEMBANGAN= '".$this->getField("SARAN_PENGEMBANGAN")."',
			SARAN_PENEMPATAN= '".$this->getField("SARAN_PENEMPATAN")."'
    ...
    function selectByParamsPenilaianLain($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
	{
		$str = "
		SELECT
			A.CATATAN_STRENGTH, A.CATATAN_WEAKNES, A.KESIMPULAN, 
			A.SARAN_PENGEMBANGAN, A.SARAN_PENEMPATAN 
		FROM penilaian A
		WHERE ASPEK_ID = 1
		".$statement;
		// AND A.ASESOR_ID = 25 AND A.JADWAL_TES_ID = 25
    ...
    function selectByParamsPotensiText($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
    {
    	$str = "
    	SELECT A.PENILAIAN_ID,A.PEGAWAI_ID,A.JADWAL_TES_ID,A.CATATAN_STRENGTH,A.CATATAN_WEAKNES,A.KESIMPULAN,A.SARAN_PENGEMBANGAN,A.SARAN_PENEMPATAN,B.BUKTI
    	FROM penilaian A
    	...
    	$str .= $statement." ORDER BY A.PENILAIAN_ID  ".$sOrder;
    	$this->query = $str;
