    ...
    **/
		...
		$str = "
				SELECT 
					  ...
					  CONCAT_WS(' ',
						...
						ALAMAT_KANTOR, TELP, 
						ESELON,  NAMA_JABATAN
				FROM dbsimpeg.rb_ref_unker A
				LEFT JOIN 
				(SELECT COUNT(1) JUMLAH_SUB, PARENT_TREE ID_TREE FROM dbsimpeg.rb_ref_unker GROUP BY PARENT_TREE) B ON A.ID_TREE = B.ID_TREE
				WHERE 1=1
				"; 
	...
	function selectByParamsMonitoringPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='')
	{
		$str = "
			SELECT IDPEG, NIP_LAMA, NIP_BARU, A.NAMA, GELAR_DEPAN, GELAR_BELAKANG, '' JENIS_KELAMIN, TEMPAT_LAHIR, CASE WHEN TO_CHAR(A.TGL_LAHIR, 'YYYY') > 0 THEN TGL_LAHIR ELSE '' END TGL_LAHIR, CASE STATUS_PEG WHEN 0 THEN 'PNS' ELSE 'CPNS' END STATUS,
				 ...
				 CASE WHEN substring_index(C.NAMA_ESELON,'.', 1) = 'I' THEN 1
				 WHEN substring_index(C.NAMA_ESELON,'.', 1) = 'II' THEN 2
				 WHEN substring_index(C.NAMA_ESELON,'.', 1) = 'III' THEN 3
				 WHEN substring_index(C.NAMA_ESELON,'.', 1) = 'IV' THEN 4
				 ELSE NULL END ESELON_PENILAIAN
			...
			WHERE A.KODE_UNKER = D.KODE_UNKER AND A.STATUS_PEG IN ('0', 'k')
	 		"; 
	...
	function selectByParamsMonitoringAnalisaPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='ORDER BY TO_CHAR(A.TANGGAL_TES, 'YYYY'), D.ATRIBUT_ID ASC')
	{
		$str = "
			...
			FROM penilaian A
			INNER JOIN dbsimpeg.rb_data_pegawai X1 ON A.PEGAWAI_ID = X1.IDPEG
			LEFT JOIN dbsimpeg.rb_ref_gol X2 ON X1.KODE_GOL_AKHIR = X2.KODE_GOL
			LEFT JOIN dbsimpeg.rb_ref_eselon X3 ON X1.KODE_ESELON = X3.KODE_ESELON
			INNER JOIN dbsimpeg.rb_ref_unker X4 ON X1.KODE_UNKER = X4.KODE_UNKER
	...
	function selectByParamsJabatan($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='ORDER BY TANGGAL_SK ASC')
	{
		$str = "
				...
				LEFT JOIN
				(
				SELECT NIP, NAMA_JAB INFO_JABATAN, B.NAMA_ESELON INFO_ESELON, UNIT_KERJA, NOMOR_SK, TANGGAL_SK FROM dbsimpeg.DATAJENJABATAN A
				LEFT JOIN dbsimpeg.rb_ref_eselon B ON B.KODE_ESELON = A.ESELON
				UNION ALL
				SELECT NIP, B.NAMA_FUNG INFO_JABATAN, '' INFO_ESELON, UNIT_KERJA, NOMOR_SK, TANGGAL_SK FROM dbsimpeg.DATAJENJABFUNG A
				LEFT JOIN dbsimpeg.RB_REF_FUNG B ON B.KODE_FUNG = A.KODE_FUNG
	...
	function getCountByParamsAnalisaDiklatKompetensiBendel($paramsArray=array(), $statement="", $tahun='', $prosentase='0', $orderby='')
	{
		...
		FROM standar_kompetensi A , standar_kompetensi_detil B , penilaian_tna_detil C , penilaian_tna D , dbsimpeg.rb_data_pegawai E
		WHERE 1=1 AND A.STANDAR_KOMPETENSI_ID = B.STANDAR_KOMPETENSI_ID 
		";
