...
elseif($reqTipeUjianId == 71 && $reqTipeUjianId == 72 && $reqTipeUjianId == 74 )
{
	// echo "Dadada"; exit;
	...
	if($reqTipeUjianId==72){
		$cekvalue="count(b.ujian_pegawai_daftar_id)";
		$statementdetil= "";
	}
	else{
		$cekvalue="COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)";
		$statementdetil.= " AND HSL.TIPE_UJIAN_ID = ".$reqTipeUjianId." AND HSL.JADWAL_TES_ID = ".$reqId;
	}
	...
	if($reqTipeUjianId==70){
		$norma=",case 
		when ".$cekvalue.">=34 and ".$cekvalue."<=40 then '20'
		when ".$cekvalue."=33 then '19'
		when ".$cekvalue.">=31 and ".$cekvalue."<=32 then '18'
		when ".$cekvalue.">=29 and ".$cekvalue."<=30 then '17'
		when ".$cekvalue.">=27 and ".$cekvalue."<=28 then '16'
		when ".$cekvalue.">=25 and ".$cekvalue."<=26 then '15'
		when ".$cekvalue."=24 then '14'
		when ".$cekvalue.">=22 and ".$cekvalue."<=23 then '13'
		when ".$cekvalue.">=20 and ".$cekvalue."<=21 then '12'
		when ".$cekvalue.">=18 and ".$cekvalue."<=19 then '11'
		when ".$cekvalue."=17 then '10'
		when ".$cekvalue.">=15 and ".$cekvalue."<=16 then '9'
		when ".$cekvalue.">=13 and ".$cekvalue."<=14 then '8'
		when ".$cekvalue.">=11 and ".$cekvalue."<=12 then '7'
		when ".$cekvalue.">=9 and ".$cekvalue."<=10 then '6'
		when ".$cekvalue."=8 then '5'
		when ".$cekvalue.">=6 and ".$cekvalue."<=7 then '4'
		when ".$cekvalue.">=4 and ".$cekvalue."<=5 then '3'
		when ".$cekvalue.">=2 and ".$cekvalue."<=3 then '2'
		when ".$cekvalue."=1 then '1'
		when ".$cekvalue."=0 then '0'
	else '-' end NILAI_HASIL";
	}
	else if($reqTipeUjianId==71){
		$norma=",case 
		when ".$cekvalue."=20 then '16'
		when ".$cekvalue."=19 then '15'
		when ".$cekvalue."=18 then '14'
		when ".$cekvalue."=17 then '13'
		when ".$cekvalue."=16 then '12'
		when ".$cekvalue."=15 then '11'
		when ".$cekvalue."=14 then '10'
		when ".$cekvalue."=13 then '9'
		when ".$cekvalue."=12 then '7'
		when ".$cekvalue."=11 then '7'
		when ".$cekvalue."=10 then '6'
		when ".$cekvalue."=9 then '5'
		when ".$cekvalue."=8 then '4'
		when ".$cekvalue."=7 then '3'
		when ".$cekvalue."=6 then '2'
		when ".$cekvalue."=5 then '1'
		when ".$cekvalue.">=0 and ".$cekvalue."<=4 then '0'
	else '-' end NILAI_HASIL";
	}
	else if($reqTipeUjianId==72){
		$norma=",case 
		when ".$cekvalue."=20 then '19'
		when ".$cekvalue."=19 then '18'
		when ".$cekvalue."=18 then '17'
		when ".$cekvalue."=17 then '16'
		when ".$cekvalue."=16 then '15'
		when ".$cekvalue."=15 then '14'
		when ".$cekvalue."=14 then '13'
		when ".$cekvalue."=13 then '12'
		when ".$cekvalue."=12 then '11'
		when ".$cekvalue."=11 then '10'
		when ".$cekvalue."=10 then '9'
		when ".$cekvalue."=9 then '8'
		when ".$cekvalue."=8 then '7'
		when ".$cekvalue."=7 then '6'
		when ".$cekvalue."=6 then '5'
		when ".$cekvalue."=5 then '4'
		when ".$cekvalue."=4 then '3'
		when ".$cekvalue."=3 then '2'
		when ".$cekvalue."=2 then '1'
		when ".$cekvalue."=1 then '0'
	else '-' end NILAI_HASIL";
	}
	else if($reqTipeUjianId==74){
		$norma=",case 
		when ".$cekvalue.">=78 and ".$cekvalue."<=80 then '20'
		when ".$cekvalue.">=75 and ".$cekvalue."<=77 then '19'
		when ".$cekvalue.">=72 and ".$cekvalue."<=74 then '18'
		when ".$cekvalue.">=69 and ".$cekvalue."<=71 then '17'
		when ".$cekvalue.">=66 and ".$cekvalue."<=68 then '16'
		when ".$cekvalue.">=63 and ".$cekvalue."<=65 then '15'
		when ".$cekvalue.">=59 and ".$cekvalue."<=62 then '14'
		when ".$cekvalue.">=56 and ".$cekvalue."<=58 then '13'
		when ".$cekvalue.">=53 and ".$cekvalue."<=55 then '12'
		when ".$cekvalue.">=50 and ".$cekvalue."<=52 then '11'
		when ".$cekvalue.">=47 and ".$cekvalue."<=49 then '10'
		when ".$cekvalue.">=44 and ".$cekvalue."<=46 then '9'
		when ".$cekvalue.">=41 and ".$cekvalue."<=43 then '8'
		when ".$cekvalue.">=37 and ".$cekvalue."<=40 then '7'
		when ".$cekvalue.">=34 and ".$cekvalue."<=36 then '6'
		when ".$cekvalue.">=31 and ".$cekvalue."<=33 then '5'
		when ".$cekvalue.">=28 and ".$cekvalue."<=30 then '4'
		when ".$cekvalue.">=25 and ".$cekvalue."<=27 then '3'
		when ".$cekvalue.">=24 and ".$cekvalue."<=22 then '2'
		when ".$cekvalue.">=19 and ".$cekvalue."<=21 then '1'
		when ".$cekvalue.">=0 and ".$cekvalue."<=18 then '0'
	else '-' end NILAI_HASIL";
	}
	// echo $allRecord; exit;
	// if($_GET['sSearch'] == "")
	...
	if ($reqTipeUjianId==72){
		...
		$set->selectByParamsMonitoringRekapLain(array(), -1,-1, $reqId, $statement.$searchJson, $statementdetil, $sOrder,$norma, $reqTipeUjianId);
	}
	// echo $set->query;exit;
}
elseif($reqTipeUjianId == 73)
{
	$sOrder='order by ja.NOMOR_URUT_GENERATE asc';
	$set = new RekapSehat();
	$statement= "";
	$statementdetil.= " AND HSL.TIPE_UJIAN_ID = ".$reqTipeUjianId." AND HSL.JADWAL_TES_ID = ".$reqId;
	
	$cekvalue='coalesce(xx.JUMLAH_BENAR,0)';

	if($reqTipeUjianId==73){
		$norma=",case 
		when ".$cekvalue.">=23 and ".$cekvalue."<=25 then '18'
		when ".$cekvalue."=22 then '17'
		when ".$cekvalue.">=20 and ".$cekvalue."<=21 then '16'
		when ".$cekvalue."=19 then '15'
		when ".$cekvalue.">=16 and ".$cekvalue."<=18 then '14'
		when ".$cekvalue."=15 then '13'
		when ".$cekvalue.">=13 and ".$cekvalue."<=14 then '12'
		when ".$cekvalue."=12 then '11'
		when ".$cekvalue.">=10 and ".$cekvalue."<=11 then '10'
		when ".$cekvalue."=9 then '9'
		when ".$cekvalue."=8 then '8'
		when ".$cekvalue.">=6 and ".$cekvalue."<=7 then '7'
		when ".$cekvalue."=5 then '6'
		when ".$cekvalue.">=3 and ".$cekvalue."<=4 then '5'
		when ".$cekvalue."=2 then '4'
		when ".$cekvalue.">=0 and ".$cekvalue."<=1 then '3'
	else '-' end NILAI_HASIL";
	}
		$set->selectByParamsJawabanPegawaiTKD6RMonitoring(array(), -1,-1, $reqId, ' AND (B.TIPE_UJIAN_ID IN (4,46) OR B.TIPE_UJIAN_ID > 49)  ',$sOrder, $norma);
	// echo $set->query;exit;
}
else
{
