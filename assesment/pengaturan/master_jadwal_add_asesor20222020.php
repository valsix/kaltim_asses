...
$tempPenggalianKodeId= $jadwal_acara->getField('PENGGALIAN_KODE_ID');
// echo $tempPenggalianKodeId;exit();
...
else
{
	...
	while($set_detil->nextRow())
	{
		...
		$arrJadwalAsesor[$index_loop]["BATAS_PEGAWAI"]= $set_detil->getField("BATAS_PEGAWAI");
		$arrJadwalAsesor[$index_loop]["TOTAL_JAM_ASESOR"]= $set_detil->getField("TOTAL_JAM_ASESOR");
		$arrJadwalAsesor[$index_loop]["PENGGALIAN_ID"]= $set_detil->getField("PENGGALIAN_ID");
		$index_loop++;
	}
}
// echo $index_loop; exit;
...
if($tempJadwalKelompokRuangId == "")
{
	
	$statement= " AND A.JADWAL_ACARA_ID = ".$reqRowId;
	$set_check= new JadwalKelompokRuangan();
	$set_check->selectByParamsMonitoring(array(),-1,-1, $statement);
	$set_check->firstRow();
	$tempJadwalKelompokRuangId= $set_check->getField("JADWAL_KELOMPOK_RUANGAN_ID");
	//echo $set_check->query;exit;
	if($tempJadwalKelompokRuangId == "")
	{
		$set_detil= new Kelompok();
		$set_detil->selectByParams();
		$set_detil->firstRow();
		$reqKelompokId= $set_detil->getField("KELOMPOK_ID");
		
		$set_detil= new Ruangan();
		$set_detil->selectByParams();
		$set_detil->firstRow();
		$reqRuanganId= $set_detil->getField("RUANGAN_ID");
		
		//echo $reqRuanganId."--".$reqKelompokId;exit;
		
		$set_detil= new JadwalKelompokRuangan();
		$set_detil->setField('JADWAL_TES_ID', $tempJadwalTesId);
		$set_detil->setField('JADWAL_ACARA_ID', $reqRowId);
		$set_detil->setField('KELOMPOK_ID', $reqKelompokId);
		$set_detil->setField('RUANGAN_ID', $reqRuanganId);
		$set_detil->setField("LAST_CREATE_USER", $userLogin->idUser);
		$set_detil->setField("LAST_CREATE_DATE", "CURRENT_DATE");
		$set_detil->insert();
		$tempJadwalKelompokRuangId= $set_detil->id;
	}
}
...
<script type="text/javascript">	
	function setcopascbi()
	{
		$.messager.confirm('Konfirmasi', "Apakah Anda yakin untuk copy data dari data CBI ?",function(r){
			if (r){

				var win = $.messager.progress({title:'Proses data', msg:'Proses data...'});
				urlAjax= "../json-pengaturan/master_jadwal_add_asesor_copy.php?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>&reqPenggalianId=<?=$tempPenggalianKodeId?>";
	...
	function addAsesorRow()
	{
		...
		<?php /*?>cell = document.createElement("TD");
		...
		row.appendChild(cell);<?php */?>
		
		cell = document.createElement("TD");
		var button = document.createElement('label');
		button.innerHTML = '<input type="text" name="reqBatasPegawai[]" id="reqBatasPegawai'+rownum+'" style="width:100%" /> ';
		cell.appendChild(button);
		row.appendChild(cell);
		...
		+'<input type="hidden" name="reqJadwalKelompokRuanganId[]" id="reqJadwalKelompokRuanganId'+rownum+'" value="<?=$tempJadwalKelompokRuangId?>" />'
		...
		$('input[id^="reqBatasPegawai"]').bind('keyup paste', function(){
	...
	function downloaddata(tempJadwalAsesorId)
	{
		newWindow = window.open("form_observasi_penggalian_asesor_pegawai.php?reqJadwalAsesorId="+tempJadwalAsesorId, 'Cetak');
		newWindow.focus();
	}
...
<div id="content" style="height:auto; width:100%">
    ...
    <table class="table_list" cellspacing="1" width="100%">
        ...
        <tr>
            <td colspan="3">
                ...
                <?
                if($tempPenggalianKodeId != "" && $jumlah_asesor == 0)
                {
                ?>
                <input type="button" onclick="setcopascbi()" value="Copas Data dari CBI" />
                <?
                }
                ?>
            </td>
    ...
    <tr>
        ...
        <?php /*?><th scope="col" style="text-align:center; width:20%">Kelompok & Ruangan</th><?php */?>
        <th scope="col" style="text-align:center; width:3%">Batas</th>
        <th scope="col" style="text-align:center; width:40%">Keterangan</th>
	...
	for($checkbox_index=0;$checkbox_index < $jumlah_asesor;$checkbox_index++)
	{
		...
		$reqBatasPegawai= $arrJadwalAsesor[$checkbox_index]["BATAS_PEGAWAI"];
    ...
    <tr>
        <td>
        	...
        	<!-- <?
        	if(!empty($tempJadwalAsesorId) && $arrJadwalAsesor[$checkbox_index]["PENGGALIAN_ID"] > 0)
        	{
        	?>
        	<img src="../WEB/images/icon-excel.png" width="15px" height="15px" onClick="downloaddata(<?=$tempJadwalAsesorId?>)" style="cursor:pointer" />
        	<?
        	}
        	?> -->
        </td>
        ...
        <td>
            <input type="text" name="reqBatasPegawai[]" id="reqBatasPegawai<?=$checkbox_index?>" style="width:100%" value="<?=$reqBatasPegawai?>" /> 
        </td>
        <td>
...
<script type="text/javascript">
// $('#reqHp1,#reqHp2,#reqTelepon').bind('keyup paste', function(){
$('input[id^="reqBatasPegawai"]').bind('keyup paste', function(){
	this.value = this.value.replace(/[^0-9]/g, '');
});
</script>
