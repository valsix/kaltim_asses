...
<script type="text/javascript">
	$(function(){
		$("#reqAspekId,#reqTahun").change(function() {
			reloadMe();
		});
	});
	...
	function reloadMe()
	{
		$('#tt').treegrid({url:'../json-pengaturan/master_penggalian_penilaian_tree.php?reqAspekId='+$("#reqAspekId").val()+'&reqTahun='+$("#reqTahun").val()});
	}
	
	
	function pilihatribut(reqSatkerEselonAtributId, reqTahun, reqAtributId, reqPenggalianId, reqmode)
	{
		var info= '';
		if(reqmode == 'hapus')
		...
		//alert(reqSatkerEselonAtributId); return false;
		//$("#reqInfo"+reqSatkerEselonAtributId).hide();
		//return false;
		if(confirm(info) == false)
			return "";
		try {
			var jqxhr = $.get( "../json-pengaturan/master_penggalian_add_penilaian.php?reqMode="+reqmode+"&reqSatkerEselonAtributId="+reqSatkerEselonAtributId+"&reqTahun="+reqTahun+"&reqAtributId="+reqAtributId+"&reqPenggalianId="+reqPenggalianId, function() {
				arrChecked = [];
			...
			.done(function(info) {
				if(info == "")
				{
					if(reqmode == 'hapus')
					{
						$("#reqInfoSimpan"+reqSatkerEselonAtributId+"-"+reqTahun+"-"+reqAtributId+"-"+reqPenggalianId).show();
						$("#reqInfoHapus"+reqSatkerEselonAtributId+"-"+reqTahun+"-"+reqAtributId+"-"+reqPenggalianId).hide();
					}
					else
					{
						$("#reqInfoSimpan"+reqSatkerEselonAtributId+"-"+reqTahun+"-"+reqAtributId+"-"+reqPenggalianId).hide();
						$("#reqInfoHapus"+reqSatkerEselonAtributId+"-"+reqTahun+"-"+reqAtributId+"-"+reqPenggalianId).show();
					}
				}
				else
				alert(info);
...
<div style="width:100%;">
    ...
    <table id="tt" title="Jabatan Atribut" class="easyui-treegrid" style="width:100%;height:600px"
        data-options="
        url: '../json-pengaturan/master_penggalian_penilaian_tree.php?reqAspekId=&reqTahun='+$('#reqTahun').val(),
        rownumbers: false,
        pagination: true,
        pageSize: 2,
        pageList: [5,10,20],
