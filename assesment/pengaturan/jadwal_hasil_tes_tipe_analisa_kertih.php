...
$statement_detil= " AND B.TIPE_UJIAN_ID IN (48)";
...
$set->selectByParamsSoalKertih(array(), -1,-1, $reqLowonganId, $statement);
// echo $set->query;exit;
while($set->nextRow())
{
...
$arrDataJawaban="";
/*$statement= " AND A.PEGAWAI_ID = ".$reqId.$statement_detil;
$set->selectByParamsJawabanKertih(array(), -1,-1, $reqLowonganId, $statement);
// echo $set->query;exit;
while($set->nextRow())
{
...
$statement= " AND A.PEGAWAI_ID = ".$reqId.$statement_detil;
$set->selectByParamsJawabanPegawaiKertih(array(), -1,-1, $reqLowonganId, $statement);
// echo $set->query;exit;
while($set->nextRow())
{
	...
	$arrDataJawabanPegawai[$index_data]["JAWABAN_KONVERSI"]= $set->getField("JAWABAN_KONVERSI");
            	...
            	<?
						...
						else
						{
							for($i_detil=0; $i_detil < count($arrayKey); $i_detil++)
							{
								...
								$reqBankSoalPathGambar= $arrDataJawabanPegawai[$index_data_detil]["PATH_GAMBAR"];
								$reqBankSoalPathSoal= $arrDataJawabanPegawai[$index_data_detil]["PATH_SOAL"];
								$reqBankSoalJawaban= $arrDataJawabanPegawai[$index_data_detil]["JAWABAN_KONVERSI"];
