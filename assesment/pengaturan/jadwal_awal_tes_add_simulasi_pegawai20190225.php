...
$index_loop= 0;
$arrJadwalAsesor="";
$statement= " AND A.JADWAL_AWAL_TES_ID = ".$reqId." AND A.JADWAL_AWAL_TES_SIMULASI_ID = ".$reqRowId;
$set_detil= new JadwalAwalTesSimulasiPegawai();
...
<script type="text/javascript">	
	...
	function deleteRowDrawTable(tableID, id) {
		...
		for(var i=0; i<=rowCount; i++) {
			if(id == i) {
				...
				reqInfoTotalPeserta= parseInt(rowCount) - 2;
				$("#reqInfoTotalPeserta").text(reqInfoTotalPeserta);
			}
		}
		}catch(e) {
	...
	function deleteRowDrawTablePhp(tableID, id, rowId, tempId, mode) {
		...
		for(var i=0; i<=rowCount; i++) {
			if(id == i) 
			{
				...
				$.getJSON('../json-pengaturan/delete.php?reqMode='+mode+'&id='+valRowId+'&reqRowDetilId=<?=$reqId?>', function (data) 
				{
				});
	...
	function addKaryawanPopUp()
	{
		...
		if(rownum > 0)
		{
			for(var i=0; i < rownum; i++)
			{
				...
				if(typeof anSelectedId == "undefined")
				{}
				else
				{
					if(tempPegawaiId == "")
		...
		// parent.OpenDHTML('pegawai_awal_simulasi_pencarian.php?reqId=<?=$reqId?>&reqRowId='+rownum+'&reqPegawaiId='+tempPegawaiId, 'Pencarian Pegawai', 780, 500)
		parent.OpenDHTML('pegawai_awal_simulasi_pencarian.php?reqId=<?=$reqId?>&reqRowId='+rownum+'&reqPegawaiId='+tempPegawaiId, 'Pencarian Pegawai', 780, 500)
	}
	...
	function openPencarianKaryawan(rowid)
	{
		...
		parent.OpenDHTML('pegawai_awal_simulasi_pencarian.php?reqId=<?=$reqId?>&reqCall=1&reqRowId='+rowid+'&reqPegawaiId='+tempPegawaiId, 'Pencarian Pegawai', 780, 500)
	}
...
<div id="content" style="height:auto; width:100%">
    ...
    <tr>
		...
		?>
        <td>
        	<center><a style="cursor:pointer" onclick="deleteRowDrawTablePhp('tableAsesor', this, '<?=$checkbox_index?>', 'reqPegawaiId', 'jadwal_awal_tes_simulasi_pegawai')"><img src="../WEB/images/delete-icon.png" width="15" height="15" border="0" /></a></center>
