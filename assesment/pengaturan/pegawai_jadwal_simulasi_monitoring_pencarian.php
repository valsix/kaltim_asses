...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-pengaturan/pegawai_jadwal_simulasi_monitoring_pencarian_json.php?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>",
		...
		/* Click event handler */
		  ...
		  $('#btnCari').on('click', function () {
			...
			reqPegawaiId= $("#reqPegawaiId").val();
		  	...
		  	oTable.fnReloadAjax("../json-pengaturan/pegawai_jadwal_simulasi_monitoring_pencarian_json.php?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>&reqCheckId="+reqPegawaiId+"&reqCari="+reqPencarian);
		  });
		  ...
		  $('#btnHapusData').on('click', function () {
			  	...
			  	$.messager.confirm('Konfirmasi',"Apakah anda yakin simpan, data terpilih?",function(r){
			  		if (r)
			  		{
			  			$.getJSON("../json-pengaturan/jadwal_awal_tes_add_simulasi_pegawai_add.php?reqId=<?=$reqRowId?>&reqPegawaiId="+reqPegawaiId,
		  				function(data){
		  					// setCariInfo();
		  					parent.setCariInfo();
			  				parent.divwin.close();
