...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-pengaturan/bank_soal_json.php",
		...
		/* Click event handler */
		  ...
		  $('#btnTambah,#btnUbahData').on('click', function () {
			  ...
			  opUrl= 'bank_soal_add.php?reqId='+anSelectedRowId;
			  window.top.OpenDHTMLDetil(opUrl, "Master Peraturan", '1000', '615');
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						var jqxhr = $.get( "../json-pengaturan/delete.php?reqMode=formula_assesment&id="+anSelectedRowId, function() {
						...
						.done(function() {
							...
							oTable.fnReloadAjax("../json-pengaturan/formula_assesment_json.php");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
        ...
        // $(".fg-toolbar").show();
        var reqTipeSoal= reqTipeUjian= reqCariFilter= "";
        reqTipeSoal= $("#reqTipeSoal").val();
        reqTipeUjian= $("#reqTipeUjian").val();
        // $("#reqStatusPegawaiId").val();
        oTable.fnReloadAjax("../json-pengaturan/bank_soal_json.php?reqTipeSoal="+reqTipeSoal+"&reqTipeUjian="+reqTipeUjian);
       
      });

    

      $("#reqTipeSoal,#reqTipeUjian").change(function() { 
      	//alert();
        setCariInfo();
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <div id="header-tna">Bank <span>Soal</span></div>
    ...
    <div style="position: absolute; margin-top:7px; margin-left:178px; z-index:9999;">
        
        &nbsp;Tipe Ujian : 
        <select name="reqTipeUjian" id="reqTipeUjian">
        	<option value="" selected>Semua</option>
			<?
            while($tipe_ujian->nextRow())
            {
            ?>
            <option value="<?=$tipe_ujian->getField("TIPE_UJIAN_ID")?>"><?=$tipe_ujian->getField("TIPE")?></option>
            <?
            }
            ?>
        </select>
        ...
        <select name="reqTipeSoal" id="reqTipeSoal">
        	<option value="" selected>Semua</option>
            <option value="1">Soal Text</option>
            <option value="2">Soal Gambar</option>
            <option value="3">Gambar lebih dari Satu Jawaban</option>
            <option value="4">Soal text jawaban gambar</option>
            <option value="5">Soal gambar jawaban text</option>
        </select>
    ...
    <div class="vmenu">
    	<input type="hidden" id="reqTipeUjian" value="10" />
