...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-pengaturan/pegawai_awal_jadwal_pencarian_json.php?reqId=<?=$reqId?>&reqPegawaiId="+reqPegawaiId,
		...
		/* Click event handler */
		  ...
		  $('#btnUbahData').on('click', function () {
					...
					$.messager.confirm('Konfirmasi', "Apakah Anda Yakin, pilih semua data yg tampil ?",function(r){
						if (r){
							...
							for(var i=0; i<reqArrData.length; i++) 
							{
								...
								{
								?>
								window.parent.addAsesorRow();
								<?
								}
								?>
								window.parent.OptionSetMain(anSelectedId, reqRowId);
							...
							arrChecked= ""
							arrChecked= [];
							oTable.fnReloadAjax("../json-pengaturan/pegawai_awal_jadwal_pencarian_json.php?reqId=<?=$reqId?>&reqPegawaiId="+reqPegawaiId+"&reqCari="+reqPencarian);
						}
					});
			  }
			  else
			  {
				...
				//alert(reqPegawaiId);return false;
				reqRowId= parseInt(reqRowId) + 1;
				oTable.fnReloadAjax("../json-pengaturan/pegawai_awal_jadwal_pencarian_json.php?reqId=<?=$reqId?>&reqPegawaiId="+reqPegawaiId+"&reqCari="+reqPencarian);<?php */?>
