...
$tempAsesorNama= $jadwal_asesor->getField('NAMA');
...
<script type="text/javascript">	
	$(function(){
		...
		$('input[id^="reqPegawai"]').autocomplete({ 
			source:function(request, response){
				...
				if (id.indexOf('reqPegawai') !== -1)
				{
					...
					urlAjax= "../json-pengaturan/pegawai_asesor_auto_combo_json.php";
				}
	...
	function addAsesorRow()
	{
		...
		+'<input type="text" class="easyui-validatebox" style="width:98%" id="reqPegawai'+rownum+'" data-options="validType:[\'sameAutoLoder[\'reqPegawai'+rownum+'\', \'\']\']" />';
...
<div id="content" style="height:auto; width:100%">
    ...
    <tr>
		...
		{
        ?>
        <a style="cursor:pointer" title="Tambah" onclick="addAsesorRow()"><img src="../WEB/images/icn_add.gif" width="16" height="16" border="0" /></a>
        <?
		}
        ?>
        </th>
	...
	for($checkbox_index=0;$checkbox_index < $jumlah_asesor;$checkbox_index++)
	{
		...
		$tempAsesorId= $arrJadwalAsesor[$checkbox_index]["PEGAWAI_ID"];
		$tempAsesor= "NIP: ".$arrJadwalAsesor[$checkbox_index]["PEGAWAI_NIP"].", Nama: ".$arrJadwalAsesor[$checkbox_index]["PEGAWAI_NAMA"];
		$tempKeteranganAsesor= $arrJadwalAsesor[$checkbox_index]["KETERANGAN_JADWAL"];
    ?>
    <tr>
        <td>
            <input type="hidden" name="reqPegawaiId[]" id="reqPegawaiId<?=$checkbox_index?>" value="<?=$tempAsesorId?>" /> 
            <input type="text" class="easyui-validatebox" style="width:98%" id="reqPegawai<?=$checkbox_index?>" 
            data-options="validType:['sameAutoLoder[\'reqPegawai\', \'\']']"
            value="<?=$tempAsesor?>" />
        </td>
        <td>
