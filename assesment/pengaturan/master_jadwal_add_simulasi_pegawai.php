...
while($set_detil->nextRow())
{
	...
	$arrJadwalAsesor[$index_loop]["PEGAWAI_SATKER"]= $set_detil->getField("SATKER");
	...
	$arrJadwalAsesor[$index_loop]["ASELON_ID"]= $set_detil->getField("eselon_id");
...
<script type="text/javascript">	
	...
	function CetakData(pegawaiId)
	{
		...
		newWindow = window.open('cetak_data_pribadi_pdf.php?reqPegawaiId='+IdPegawai+'&reqId=<?=$reqId?>', 'Cetak');
		newWindow.focus();
	}
	...
	function CetakKuisioner(pegawaiId)
	{
		...
		newWindow = window.open('cetak_kuisioner_pdf.php?reqPegawaiId='+IdPegawai+'&reqId=<?=$reqId?>', 'Cetak');
		newWindow.focus();
	}
	...
	function CetakDataPeserta(pegawaiId,mode)
	{
		...
		newWindow = window.open('cetak_data_pribadi_pdf.php?reqPegawaiId='+IdPegawai+'&reqId=<?=$reqId?>&reqMode='+Mode, 'Cetak');
		newWindow.focus();
	}
...
<div id="content" style="height:auto; width:100%">
    ...
    <tr>
        ...
        <th scope="col" >Data Pribadi Peserta</th>
        <?
        if ($arrJadwalAsesor[0]["ASELON_ID"]!=99){?>
        <th scope="col" >Critical Incident</th>
        <th scope="col" >Q Inta</th>
        <th scope="col" >Q Kompetensi Eselon</th>
        <?} else{?>
        <th scope="col" >Q Kompetensi Pelaksana</th>
        <?}?>
	...
	for($checkbox_index=0;$checkbox_index < $jumlah_asesor;$checkbox_index++)
	{
		...
		$peserta= new Peserta();
		$peserta->selectByParamsDataPribadi(array(), -1,-1, " AND A.PEGAWAI_ID = ".$tempPegawaiId);
		$peserta->firstRow();
		...
		$tempPegawaiSatker= $arrJadwalAsesor[$checkbox_index]["PEGAWAI_SATKER"];
		$tempFile1= $urlkarpeg.$arrJadwalAsesor[$checkbox_index]["LINK_FILE1"];
		$tempFile1x= $arrJadwalAsesor[$checkbox_index]["LINK_FILE1"];
		// print_r($arrJadwalAsesor[$checkbox_index]);exit;
		$tempFile2= $urlkarpeg.$arrJadwalAsesor[$checkbox_index]["LINK_FILE2"];
		$tempFile2x= $arrJadwalAsesor[$checkbox_index]["LINK_FILE2"];
		$tempFoto= $urlkarpeg.$arrJadwalAsesor[$checkbox_index]["LINK_FOTO"];
		...
		$urlPendidikan = 'https://api-simpeg.kaltimbkd.info//pns/riwayat-pendidikan/'.$tempPegawaiId.'/?api_token=f5a46b71f13fe1fd00f8747806f3b8fa';
		...
		{
			$imgdrh = "gambardurung";
			// echo $riwayatpendidikanapi; exit;
		}
		// print_r($imgdrh);

		$sudah = new JadwalTes();
		$statementsoal= " AND B.PEGAWAI_ID = ".$tempPegawaiId."";
		$statementjawaban= " AND A.PEGAWAI_ID = ".$tempPegawaiId."";
		$statementqinta= " AND C.PEGAWAI_ID = ".$tempPegawaiId." AND C.TIPE_FORMULIR_ID = 2";
		$statementeselon= " AND C.PEGAWAI_ID = ".$tempPegawaiId." AND C.TIPE_FORMULIR_ID = 3";
    ...
    <tr>
        ...
        <td>
			...
			{
			?>
        	<img  src="../WEB/images/<?=$imgdrh?>.png" onClick="CetakData(<?=$tempPegawaiId?>)" style="cursor:pointer;text-align: center" class="gambar" />
        	<?
        	}
        	else
        	{
        ...
        <?if ($arrJadwalAsesor[$checkbox_index]["ASELON_ID"]!=99){?>
        <td>
        	<?if($img == "down_icon")
			{
			?>
        	<img  src="../WEB/images/<?=$img?>.png" onClick="CetakDataPeserta(<?=$tempPegawaiId?>,'CI')" style="cursor:pointer;text-align: center" class="gambar" />
        	<?
        	}
        	else
        	{
        ...
        <td>
			...
			{
			?>
        	<img  src="../WEB/images/<?=$imginta?>.png" onClick="CetakDataPeserta(<?=$tempPegawaiId?>,'QI')" style="cursor:pointer;text-align: center" class="gambar" />
        	<?
        	}
        	else
        	{
        ...
        <td>
			...
			{
			?>
        	<img  src="../WEB/images/<?=$imgeselon?>.png" onClick="CetakDataPeserta(<?=$tempPegawaiId?>,'QK_Eselon')" style="cursor:pointer;text-align: center" class="gambar" />
        	<?
        	}
        	else
        	{
        ...
        else
        {
        	...
        	<td>
        		...
        		{
        			?>
        			<img  src="../WEB/images/<?=$imgpel?>.png" onClick="CetakDataPeserta(<?=$tempPegawaiId?>,'QK_Pelaksana')" style="cursor:pointer;text-align: center" class="gambar" />
        			<?
        		}
        		else
        		{
      ...
      <!--   <?
        ...
        {
        ?>
        	<td><center><a href="<?=$tempFile2?>" target="_blank">File Pendukung</a></center></td>
        <?
    	}
        else
        {
    	...
    	?>
        	<td><img  src="../WEB/images/down_icon.png" onClick="CetakKuisioner(<?=$tempPegawaiId?>)" style="cursor:pointer;text-align: center" class="gambar" /></td>
