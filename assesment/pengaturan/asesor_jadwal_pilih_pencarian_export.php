...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-pengaturan/asesor_jadwal_pilih_pencarian_export_json.php?reqAsesorId=<?=$reqAsesorId?>",
		...
		/* Click event handler */
		  ...
		  $('#btnUbahData').on('click', function () {
			  ...
			  if($("#reqCheckAll").prop('checked')) 
			  {
					$.messager.confirm('Konfirmasi', "Apakah Anda Yakin, pilih semua data yg tampil ?",function(r){
						if (r){
							var data= "";
							console.log(reqRowId);
							data= getChecked();
							window.parent.OptionSetAsesorDetil(data, reqRowId);
							window.parent.DwinClose();
							// var data= "";
							// data= getChecked();
							...
							// 	window.parent.OptionSetAsesorDetil(anSelectedId, reqRowId);
							...
							arrChecked= ""
							arrChecked= [];
							oTable.fnReloadAjax("../json-pengaturan/asesor_jadwal_pilih_pencarian_export_json.php?reqId=<?=$reqId?>&reqAsesorId="+reqPegawaiId+"&reqCari="+reqPencarian);
						}
					});
			  }
			  else
			  {
					...
					$.messager.confirm('Konfirmasi', "Apakah Anda Yakin, pilih semua data yang terpilih ?",function(r){
						if (r){
							...
							// {
							// 	anSelectedId= reqArrData[i];
							// 	// window.parent.addAsesorExportRowDetil();
							// 	window.parent.OptionSetAsesorDetil(anSelectedId, reqRowId);
		  ...
		  $("#reqPencarian").keyup(function(e) { 
			...
			if(code==13)
			{
				  ...
				  reqPencarian= $("#reqPencarian").val();
				  oTable.fnReloadAjax("../json-pengaturan/asesor_jadwal_pilih_pencarian_export_json.php?reqAsesorId=<?=$reqAsesorId?>"+"&reqCari="+reqPencarian);
			}
		  });
