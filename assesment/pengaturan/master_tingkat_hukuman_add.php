...
else
{
	...
	$tempPeraturanId 	= $set->getField('PERATURAN_ID');
...
<script type="text/javascript">	
	...
	$(function(){
		...
		$('#ff').form({
			url:'../json-pengaturan/master_tingkat_hukuman_add.php',
			onSubmit:function(){
...
<div id="content" style="height:auto; margin-top:-4px; width:100%">
    	...
    	<table class="table_list" cellspacing="1" width="100%">
            <tr>
                <td colspan="6">
                <div id="header-tna-detil">Tambah <span>Tingkat Hukuman</span></div>	                    
                </td>			
            </tr>
            <tr>
                ...
                <td width="10px">:</td>
                <td>        
                    <input type="text" name="reqNama" style="width:350px" value="<?=$tempNama?>"/>			
               </td>
            </tr>
            <tr>
                <td>Peraturan</td>
                <td>:</td>
                <td>
                    <select name="reqPeraturanId">
                    <? 
                        $set->selectByParamsStatus(array(),-1,-1);
                        while($set->nextRow())
                        { 
                    ?>
                            <option value="<?=$set->getField('PERATURAN_ID')?>"<? if($tempPeraturanId == $set->getField("PERATURAN_ID")) { ?> selected="selected" <? } ?>>
                                <?=$set->getField('NAMA')?>
                            </option>
                    <? 
                        } 
