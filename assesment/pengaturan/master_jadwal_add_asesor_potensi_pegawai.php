...
$jadwal_asesor= new JadwalAsesorPotensi();
$jadwal_asesor->selectByParamsMonitoring(array("A.JADWAL_ASESOR_POTENSI_ID"=> $reqRowId),-1,-1,'');
$jadwal_asesor->firstRow();
//echo $jadwal_asesor->query;exit;
$tempJadwalTesId= $jadwal_asesor->getField('JADWAL_TES_ID');
$tempJadwalAcaraId= $jadwal_asesor->getField('JADWAL_ACARA_ID');
$tempAsesorId= $jadwal_asesor->getField('ASESOR_ID');
...
$tempKeterangan= $jadwal_asesor->getField('KETERANGAN_JADWAL');
...
else
{
	...
	$statement= " AND A.JADWAL_ASESOR_POTENSI_ID = ".$reqRowId;
	$set_detil= new JadwalAsesorPotensiPegawai();
	$set_detil->selectByParamsPegawai(array(), -1,-1, $statement);
	//echo $set_detil->query;exit;
	while($set_detil->nextRow())
	{
		$arrJadwalAsesorPotensi[$index_loop]["SATKER_TES_ID"]= $set_detil->getField("SATKER_TES_ID");
		$arrJadwalAsesorPotensi[$index_loop]["TANGGAL_TES"]= dateToPageCheck($set_detil->getField("TANGGAL_TES"));
		
		$arrJadwalAsesorPotensi[$index_loop]["JADWAL_ASESOR_POTENSI_PEGAWAI_ID"]= $set_detil->getField("JADWAL_ASESOR_POTENSI_PEGAWAI_ID");
		$arrJadwalAsesorPotensi[$index_loop]["PEGAWAI_ID"]= $set_detil->getField("PEGAWAI_ID");
		$arrJadwalAsesorPotensi[$index_loop]["PEGAWAI_NAMA"]= $set_detil->getField("PEGAWAI_NAMA");
		$arrJadwalAsesorPotensi[$index_loop]["PEGAWAI_NIP"]= $set_detil->getField("PEGAWAI_NIP");
		$arrJadwalAsesorPotensi[$index_loop]["PEGAWAI_GOL"]= $set_detil->getField("PEGAWAI_GOL");
		$arrJadwalAsesorPotensi[$index_loop]["PEGAWAI_ESELON"]= $set_detil->getField("PEGAWAI_ESELON");
		$arrJadwalAsesorPotensi[$index_loop]["PEGAWAI_JAB_STRUKTURAL"]= $set_detil->getField("PEGAWAI_JAB_STRUKTURAL");
...
<script type="text/javascript">	
	$(function(){
		$('#ff').form({
			url:'../json-pengaturan/master_jadwal_add_asesor_potensi_pegawai.php',
			onSubmit:function(){
			...
			success:function(data){
				...
				parent.frames['mainFrame'].location.href = 'master_jadwal_add_asesor_potensi_pegawai_monitoring.php?reqId=<?=$reqId?>';
				parent.frames['mainFrameDetil'].location.href = 'master_jadwal_add_asesor_potensi_pegawai.php?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>';
			}
		});
	});
	...
	function addAsesorRow()
	{
		...
		cell = document.createElement("TD");
		var button = document.createElement('label');
		button.innerHTML = '<input type="hidden" name="reqJadwalAsesorPotensiPegawaiId[]" id="reqJadwalAsesorPotensiPegawaiId'+rownum+'" />'
	...
	function addKaryawanPopUp()
	{
		...
		parent.OpenDHTML('pegawai_simulasi_detil_pencarian.php?reqId=<?=$reqId?>&reqJadwalTesId=<?=$tempJadwalTesId?>&reqRowId='+rownum+'&reqPegawaiId='+tempPegawaiId, 'Pencarian Pegawai', 780, 500)
	}
...
<div id="content" style="height:auto; width:100%">
    ...
    <table class="table_list" cellspacing="1" width="100%">
        ...
        <tr>
            <td>
                ...
                <input type="hidden" name="reqJadwalTesId" value="<?=$tempJadwalTesId?>">
                <input type="hidden" name="reqJadwalAcaraId" value="<?=$tempJadwalAcaraId?>">
                <input type="hidden" name="reqAsesorId" value="<?=$tempAsesorId?>">
    ...
    <tr>
        ...
        <th style="display: none;" scope="col" style="text-align:center; width:50px">Aksi</th>
    </tr>
    </thead>
    <tbody>
    <?
	...
	{
		$tempJadwalAsesorPotensiId= $arrJadwalAsesorPotensi[$checkbox_index]["JADWAL_ASESOR_POTENSI_PEGAWAI_ID"];
		$tempPegawaiId= $arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_ID"];
		//$tempPegawai= "NIP: ".$arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_NIP"].", Nama: ".$arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_NAMA"];
		
		$tempPegawaiSatkerTesId= $arrJadwalAsesorPotensi[$checkbox_index]["SATKER_TES_ID"];
		$tempPegawaiTanggalTes= $arrJadwalAsesorPotensi[$checkbox_index]["TANGGAL_TES"];
		
		$tempPegawai= $arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_NAMA"];
		$tempPegawaiNip= $arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_NIP"];
		$tempPegawaiGol= $arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_GOL"];
		$tempPegawaiEselon= $arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_ESELON"];
		$tempPegawaiJabatan= $arrJadwalAsesorPotensi[$checkbox_index]["PEGAWAI_JAB_STRUKTURAL"];
    ...
    <tr>
        <td>
        	...
        	<img src="../WEB/images/icn_search.png" onClick="openPencarianKaryawan(<?=$checkbox_index?>)" style="cursor:pointer; display: none;" />
        </td>
        ...
        <td style="display: none;">
        	<center><a style="cursor:pointer" onclick="deleteRowDrawTablePhp('tableAsesor', this, '<?=$checkbox_index?>', 'reqJadwalAsesorPotensiPegawaiId', 'jadwal_asesor_potensi_pegawai')"><img src="../WEB/images/delete-icon.png" width="15" height="15" border="0" /></a></center>
            <input type="hidden" name="reqJadwalAsesorPotensiPegawaiId[]" id="reqJadwalAsesorPotensiPegawaiId<?=$checkbox_index?>" value="<?=$tempJadwalAsesorPotensiId?>" />
        </td>
    </tr>
    <?
	}
