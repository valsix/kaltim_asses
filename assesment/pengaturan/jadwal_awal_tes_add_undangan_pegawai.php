...
$index_loop= $set_detil->getCountByParamsMonitoringPegawai(array(), $statement);
...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-pengaturan/jadwal_awal_tes_add_undangan_pegawai_json.php?reqId=<?=$reqId?>",
		...
		/* Click event handler */
		  ...
		  $('#btnCari').on('click', function () {
                ...
                reqCariFilter= $("#reqCariFilter").val();
			  	...
			  	oTable.fnReloadAjax("../json-pengaturan/jadwal_awal_tes_add_undangan_pegawai_json.php?reqId=<?=$reqId?>&reqCheckId="+reqPegawaiId+"&sSearch="+reqCariFilter);
		  });
		  ...
		  $('#btnTambahData').on('click', function () {
		  		parent.OpenDHTML('pegawai_awal_simulasi_monitoring_pencarian.php?reqId=<?=$reqId?>', 'Pencarian Pegawai', 780, 500);
		  });
		  ...
		  $('#btnHapusData').on('click', function () {
			  	...
			  	$.messager.confirm('Konfirmasi',"Apakah anda yakin hapus, data terpilih?",function(r){
			  		if (r)
			  		{
			  			$.getJSON("../json-pengaturan/jadwal_awal_tes_add_undangan_pegawai_hapus.php?reqId=<?=$reqId?>&reqPegawaiId="+reqPegawaiId,
		  				function(data){
		  					setCariInfo();
		  					$("#reqPegawaiId").val("");
		  				});
