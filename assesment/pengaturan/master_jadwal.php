...
if($reqJenis == "1" || $reqJenis == "2")
{
	$statementdetil= " AND CURRENT_DATE = TO_DATE(TO_CHAR(TANGGAL_TES, 'YYYY-MM-DD'), 'YYYY-MM-DD')";
	// $statementdetil= " AND CURRENT_DATE + 6 = TO_DATE(TO_CHAR(TANGGAL_TES, 'YYYY-MM-DD'), 'YYYY-MM-DD')";
	$reqJadwalTesId= $separator= "";
	$set= new JadwalTes();
	$set->selectByParams(array(),-1,-1, $statementdetil);
	while($set->nextRow())
	{
		// $set->firstRow();
		// echo $set->query;exit();
		...
		$reqJadwalTesId= $reqJadwalTesId.$separator.$set->getField("JADWAL_TES_ID");
	}
	// echo $reqJadwalTesId;exit();

	$link= "master_jadwal_add_absen";
	if($reqJenis == "2")
		$link= "master_jadwal_add_multi_absen";

	echo '<script language="javascript">';
	echo "window.location.href = '".$link.".php?reqJenis=".$reqJenis."&reqId=".$reqJadwalTesId."';";
	echo '</script>';
}
?>
...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-pengaturan/master_jadwal_json.php",
		...
		/* Click event handler */
		  ...
		  $('#btnTambah,#btnUbahData').on('click', function () {
			  ...
			  {
			  ?>
			  opUrl= 'master_jadwal_add_absen.php?reqJenis=<?=$reqJenis?>&reqId='+anSelectedRowId;
			  ...
			  {
			  ?>
			  opUrl= 'master_jadwal_add.php?reqJenis=<?=$reqJenis?>&reqId='+anSelectedRowId;
			  window.location.href = opUrl;
			  <?
			  }
			  else
			  {
			  ?>
			  opUrl= 'master_jadwal_add.php?reqId='+anSelectedRowId;
			  window.top.OpenDHTML(opUrl, "Master Peraturan", '680', '325');
			  <?
			  }
			  ?>
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						var jqxhr = $.get( "../json-pengaturan/delete.php?reqMode=master_jadwal&id="+anSelectedRowId, function() {
						...
						.done(function() {
							...
							oTable.fnReloadAjax("../json-pengaturan/master_jadwal_json.php");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
...
<body id="index" class="grid_2_3" style="overflow:hidden">
	...
	{
	?>
    <div id="header-tna">Absensi <span>Jadwal</span></div>
    <?
	}
	else
	{
	?>
	<div id="header-tna">Master <span>Jadwal</span></div>
	<?
	}
    ?>
    ...
    <ul>
    	...
    	{
    	?>
    	<li><a href="#" title="Absen" id="btnUbahData"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;Absen</a></li>
    	<?
    	}
    	elseif($reqJenis == "2")
    	{
    	?>
    	<li><a href="#" title="Hasil CFIT" id="btnUbahData"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;Hasil CFIT</a></li>
    	<?
    	}
    	else
    	{
