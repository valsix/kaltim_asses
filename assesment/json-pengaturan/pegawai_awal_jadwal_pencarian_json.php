...
else
{
	...
	= 
	(
	SELECT FE.ESELON_ID FROM jadwal_awal_tes JT INNER JOIN formula_eselon FE ON JT.FORMULA_ESELON_ID = FE.FORMULA_ESELON_ID WHERE 1=1 AND JT.JADWAL_AWAL_TES_ID = ".$reqId."
	)
...
{
	$statement.= " AND A.PEGAWAI_ID IN (SELECT A.PEGAWAI_ID FROM jadwal_awal_tes_simulasi_pegawai A WHERE A.JADWAL_AWAL_TES_ID = ".$reqJadwalTesId.")";
}
