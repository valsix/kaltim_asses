...
if ($id == 0)
{
	...
	$result["total"] = $set->getCountByParamsAtributCombo(array("PARENT_ID" => 0), $statement);	
	$set->selectByParamsAtributCombo(array("PARENT_ID" => 0), $rows, $offset, $statement);
	// echo $set->query;exit;
...
else 
{
	...
	$set->selectByParamsAtributCombo(array("PARENT_ID" => $id), -1, -1, $statement);
	//echo $set->query;exit;
	$i=0;
	while($set->nextRow())
	{
...
{
	$child = new Atribut();
	$child->selectByParamsAtributCombo(array("PARENT_ID" => $id), -1, -1, $statement);
	$child->firstRow();
