...
$aColumns = array("user_app_id","CHECK", "user_login", "nama");
$aColumnsAlias = array("user_app_id","CHECK", "user_login", "nama");
...
$searchJson= " AND (UPPER(NAMA) LIKE UPPER('%".strtoupper($reqCari)."%') OR UPPER(user_login) LIKE UPPER('%".strtoupper($reqCari)."%')) ";


$sOrder="ORDER BY user_app_id ASC";
$set->selectByParamsMonitoringAdminKegiatan(array(), $dsplyRange, $dsplyStart, $statement.$searchJson, $sOrder, $reqId);
...
while($set->nextRow())
{
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if($aColumns[$i] == 'CHECK')
		{
			$row[] = "<input type='checkbox' class='editor-active' id='reqPilihCheck' value='".$set->getField("IDPEG")."'>";
		}
