...
include_once("../WEB/classes/base/FormulaAssesmentUjianTahapBankSoal.php");
...
for($i=0; $i<count($reqTipeUjian); $i++)
{
	...
	else
	{
		...
		$ujian_tahap = new FormulaAssesmentUjianTahap();
		$ujian_tahap->setField("FORMULA_ASSESMENT_ID", $reqId);
		...
		if($reqRowId[$i] == "")
		{
			...
			if($reqStatusAnak[$i] == "1")
			{
				...
				while($set->nextRow())
				{
					$set_detil= new FormulaAssesmentUjianTahap();
					$set_detil->setField("FORMULA_ASSESMENT_ID", $reqId);
		...
		else
		{
			...
			$ujian_tahap->setField("FORMULA_ASSESMENT_UJIAN_TAHAP_ID", $reqRowId[$i]);
...
$set_detil= new FormulaAssesmentUjianTahapBankSoal();
$set_detil->setField("FORMULA_ASSESMENT_ID", $reqId);
$set_detil->setField("LAST_CREATE_USER", $userLogin->nama);
