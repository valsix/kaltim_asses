...
if ($id == 0)
{
	...
	$result["total"] = $set->getCountByParamsIndikatorAtributCombo(array("PARENT_ID" => 0), $statement);	
	$set->selectByParamsIndikatorAtributCombo(array("PARENT_ID" => 0), $rows, $offset, $statement);
	//echo $set->query;exit;
...
else 
{
	...
	$set->selectByParamsIndikatorAtributCombo(array("PARENT_ID" => $id), -1, -1, $statement);
	//echo $set->query;exit;
	$i=0;
	while($set->nextRow())
	{
...
{
	$child = new Atribut();
	$child->selectByParamsIndikatorAtributCombo(array("PARENT_ID" => $id), -1, -1, $statement);
	$child->firstRow();
