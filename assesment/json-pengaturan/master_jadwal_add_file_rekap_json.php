...
$aColumns= array("NIP_BARU", "NAMA", "LAST_JABATAN", "SATKER", "BUKTI_PENDUKUNG");
...
//if($reqTipe == "1")
//	$statement.= " AND A.STATUS_PEGAWAI_ID IS NULL";
//else
//	$statement.= " AND A.STATUS_PEGAWAI_ID IS NOT NULL";

$statement.= " AND JP.JADWAL_AWAL_TES_SIMULASI_ID = ".$reqId;

$searchJson= " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') ";

$allRecord = $set->getCountByParamsCari(array(), $reqId, $statement);
if($_GET['sSearch'] == "")
	$allRecordFilter = $allRecord;
else
	$allRecordFilter = $set->getCountByParamsCari(array(), $reqId, $statement.$searchJson);
// echo $set->query;exit;

$set->selectByParamsCari(array(), $dsplyRange, $dsplyStart, $reqId, $statement.$searchJson, $sOrder);
// echo $set->query;exit;
