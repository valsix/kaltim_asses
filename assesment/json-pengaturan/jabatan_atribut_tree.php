...
if ($id == 0)
{
	...
	$set->selectByParamsJabatanAtributCombo(array("PARENT_ID" => 0), $rows, $offset, $reqTahun, $reqAspekId, $statement);
	//echo $set->query;exit;
...
{
	$set->selectByParamsJabatanAtributCombo(array("PARENT_ID" => $id), -1, -1, $reqTahun, $reqAspekId, $statement);
	//echo $set->query;exit;
	$i=0;
	while($set->nextRow())
	{
		...
		else
		{
			...
			$set_detil->selectByParamsJabatanAtributCombo(array("PARENT_ID" => $result[$i]['ID']), -1, -1, $reqTahun, $reqAspekId, $statement);
			$set_detil->firstRow();
