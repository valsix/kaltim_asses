...
if ($id == 0)
{
	...
	$set->selectByParamsMonitoringTree(array("PARENT_ID" => 0), $rows, $offset, $statement);
	//echo $set->query;exit;
...
else 
{
	...
	$set->selectByParamsMonitoringTree(array(), -1, -1, $statement);
	//echo $set->query;exit;
	$i=0;
	while($set->nextRow())
	{
		...
		else
		{
			...
			//$set_detil->selectByParamsMonitoringTree(array("PARENT_ID" => $result[$i]['ID']), -1, -1, $reqTahun, $reqAspekId, $statement);
			$set_detil->selectByParamsMonitoringTree(array(), -1, -1, $statement);
			$set_detil->firstRow();
