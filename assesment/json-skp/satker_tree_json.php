...
if ($id == 0)
{
	...
	{
		$result["total"] = $set->getCountByParamsSatuanKerja(array("PARENT_ID" => 0), $statement);	
		$set->selectByParamsSatuanKerja(array("PARENT_ID" => 0), $rows, $offset, $statement);
		//echo $satker->query;exit;
	}
	else
	{
		$result["total"] = $set->getCountByParamsSatuanKerja(array("A.ID" => $userSatkerEvaluasiKinerja), $statement);	
		$set->selectByParamsSatuanKerja(array("A.ID" => $userSatkerEvaluasiKinerja), $rows, $offset, $statement);
	}	
	//echo $set->query;exit;
...
{
	$set->selectByParamsSatuanKerja(array("PARENT_ID" => $id), -1, -1, $statement);
	$i=0;
	while($set->nextRow())
	{
...
{
	$child = new Kelautan();
	$jumlah = $child->getCountByParamsSatuanKerja(array("PARENT_ID" => $id), $stat);
