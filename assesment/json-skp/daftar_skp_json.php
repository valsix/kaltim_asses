...
$pegawai_penilai->selectByParamsMonitoringDaftar($reqTahun, array(), $dsplyRange, $dsplyStart, $statement.$searchJson, $sOrder);  
...
while($pegawai_penilai->nextRow())
{
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if($aColumns[$i] == "TANGGAL")
			...
			$row[] = "<img src=\"../WEB/images/".$pegawai_penilai->getField($aColumns[$i]).".png\" onClick=\"openValidasi('".$pegawai_penilai->getField("PEGAWAI_ID")."', '".str_replace("BL", "", $aColumns[$i])."', '".$pegawai_penilai->getField($aColumns[$i])."')\">";
		else
