...
$setSatker=setPegawai();
// print_r($setSatker); exit;
...
{
	var reqTahun= reqEselonId= reqKdOrganisasiId= reqJenisJabatan= "";
	...
	var opUrl= "tabel_nine_box_talent.php?reqId=<?=$reqId?>&reqTahun="+reqTahun+"&reqEselonId="+reqEselonId+"&reqKuadranId="+idKuadran+"&reqFormulaId="+reqFormulaId;
	...
	request.done(function(dataJson)
	{
		...
		else
		{
			...
			chart: {
				...
				title: {
					text:'<span style="font-size:14px">TALENT POOL KOMPETENSI KINERJA<span><br><span style="font-size:14px"><?=$tempNamaSatuanKerja?></span>'
				},
				series: [
				{
					type: 'line',
...
setGrafik("../json-ikk/grafik_nine_box_talent_skp_grafik_json.php?reqId=<?=$reqId?>&reqTahun=<?=$reqTahun?>");
setModal("tableGrafik", "../json-ikk/grafik_nine_box_talent_skp_table_json.php?reqId=<?=$reqId?>&reqTahun=<?=$reqTahun?>");

$(function () {

	$('#reqId').change(function() {
		reqId= $("#reqId").val();
		document.location.href= "grafik_nine_box_talent.php?reqId="+reqId;
	});
	...
	$.messager.confirm('Konfirmasi', "Apakah Anda Yakin, proses data ?",function(r){
		if (r){
			...
			$.ajax({'url': s_url,'success': function(msg) {
				...
				{
					setGrafik("../json-ikk/grafik_nine_box_talent_skp_grafik_json.php?reqId=<?=$reqId?>&reqTahun="+reqTahun+"&reqKdOrganisasiId="+reqKdOrganisasiId+"&reqJenisJabatan="+reqJenisJabatan+"&reqEselonId="+reqEselonId+"&reqPencarian="+reqPencarian+"&reqFormulaId="+reqFormulaId);
					setModal("tableGrafik", "../json-ikk/grafik_nine_box_talent_skp_table_json.php?reqId=<?=$reqId?>&reqTahun="+reqTahun+"&reqKdOrganisasiId="+reqKdOrganisasiId+"&reqJenisJabatan="+reqJenisJabatan+"&reqEselonId="+reqEselonId+"&reqPencarian="+reqPencarian+"&reqFormulaId="+reqFormulaId);
				}
			}});
		}
	});
	        ...
	        <div class="bar-status" style="width:100% !important; border:1px; padding-left: 20px">
		        <table border="0" style="width:100%">
		        	...
		        	<tr style="padding-top: 3px;">
		        		<td colspan="3">
		        			<button style=" background-color: #4CAF50; color: white;padding: 5px;text-align: center;margin: 4px 2px;cursor: pointer;border-radius: 8px;" onclick="setProsesToleransi()">Proses Data</button>
		        			<a style=" background-color: #4CAF50; color: white;padding: 5px;text-align: center;margin: 4px 2px;cursor: pointer;border-radius: 8px;"  onclick="updateSkp(0)">Update SKP</a>
		        		</td>
		        	</tr>
		        </table>
...
<!-- <script>
	...
	{
		var win = $.messager.progress({title:'Proses Data', msg:'Proses Data...'});
		var s_url= "../json-ikk/update_skp.php";
		//alert(s_url);return false;
		$.ajax({'url': s_url,'success': function(msg) {
			if(msg == '')
			{
...
<script type="text/javascript">
	function updateSkp(val)
	{
		...
		pegawaiNIP=satker[0];
		pegawaiID=satker[1];
		document.getElementById("myModalIsi").innerHTML=val+'/'+satkerFunc.length+'<br>proses menyimpan data pegawai';
		// var s_url= "../json-ikk/update_pegawai.php?reqId="+satkerid;
		var s_url= "../json-ikk/update_skp.php?reqNip="+pegawaiNIP+"&reqPegawaiId="+pegawaiID;
		//alert(s_url);return false;
		$.ajax({'url': s_url,'success': function(msg) {
