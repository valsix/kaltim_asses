...
$set_detil->selectByParamsMonitoringPenilaian(array(), -1, -1, " AND B.ASPEK_ID = ".$tempAspekId." AND A.SATKER_ID = '".$tempSatkerTesId."'", $reqRowId);
...
while($set_detil->nextRow())
{
	...
	if($index_detil == 0 || $tempAtributIdParent == "0")
		$arrDetil[$index_detil]["JUMLAH_PENILAIAN_DETIL"] = $set_detil->getField("JUMLAH_PENILAIAN_DETIL");
	else
	{
		$tempIndexDetil= $index_detil-1;
		$arrDetil[$index_detil]["JUMLAH_PENILAIAN_DETIL"] = $arrDetil[$tempIndexDetil]["JUMLAH_PENILAIAN_DETIL"];
	}
...
$tempTotalIkk= round($tempTotalIkk/$tempJumlahAtribut,2);
$tempTotalJpm= round($tempTotalJpm/$tempJumlahAtribut,2);
            ...
            <tr>
			      <td colspan="6">
                  		<ol id="toc"> 
                            ...
                            <li><a href="#" onclick="parent.setLoad('general_ikk_add_penilaian_atribut_spider.php?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>', '1');"><span>Grafik<!--Spider Plot--></span></a></li>
                        </ol>
