...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",
		"sAjaxSource": "../json-ikk/tabel_nine_box_talent_json.php?reqId=<?=$reqId?>&reqKuadranId=<?=$reqKuadranId?>&reqTahun=<?=$reqTahun?>&reqFormulaId=<?=$reqFormulaId?>",
		...
		/* Click event handler */
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						var jqxhr = $.get( "delete.php?reqMode=tabel_nine_box_talent&id="+anSelectedPegawaiId, function() {
							$.messager.progress('close');
						})
						.done(function() {
							$.messager.progress('close');
							oTable.fnReloadAjax("../json/tabel_nine_box_talent_json.php?reqSearch=" + $("#reqStatus").val() + "&reqId=<?=$reqId?>");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
		  ...
		  $("#reqPencarian").keyup(function(e) { 
			...
			if(code==13)
			{
				  var reqStatusPeg= reqPencarian= reqFormulaId= "";
			  
				  reqStatusPeg= $("#reqStatusPeg").val();
				  reqPencarian= $("#reqPencarian").val();
				  reqFormulaId= $("#reqFormulaId").val();

				  oTable.fnReloadAjax("../json-ikk/tabel_nine_box_talent_json.php?reqStatusPeg="+reqStatusPeg+"&reqFormulaId="+reqFormulaId+"&reqId=<?=$reqId?>&sSearch="+reqPencarian);
			}
		  });
		  ...
		  $("#reqTahun, #reqKuadranId, #reqFormulaId").change(function() { 
			  ...
			  oTable.fnReloadAjax("../json-ikk/tabel_nine_box_talent_json.php?reqTahun="+reqTahun+"&reqKuadranId="+reqKuadranId+"&reqFormulaId="+reqFormulaId+"&reqId=<?=$reqId?>&sSearch="+reqPencarian);
		  });
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <select id="reqKuadranId" name="reqKuadranId">
        ...
        <option value="11">I. Kinerja dibawah ekspektasi dan potensial rendah</option>
        <option value="12">II. Kinerja sesuai ekspektasi dan potensial rendah</option>
        <option value="21">III. Kinerja dibawah ekspektasi dan potensial menengah</option>
        <option value="13">IV. Kinerja diatas ekspektasi dan potensial rendah</option>
        <option value="22">V. Kinerja sesuai ekspektasi dan potensial menengah</option>
        <option value="31">VI. Kinerja dibawah ekspektasi dan potensial tinggi</option>
        <option value="23">VII. Kinerja diatas ekspektasi dan potensial menengah</option>
        <option value="32">VIII. Kinerja sesuai ekspektasi dan potensial tinggi</option>
        <option value="33">IX. Kinerja diatas ekspektasi dan potensial tinggi</option>
    </select>
    <?
	}
	else
