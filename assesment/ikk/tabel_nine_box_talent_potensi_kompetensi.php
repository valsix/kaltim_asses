...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ "iDisplayLength": 250,bJQueryUI: true,
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",
		"sAjaxSource": "../json-ikk/tabel_nine_box_talent_potensi_kompetensi_json.php?reqId=<?=$reqId?>&reqKuadranId=<?=$reqKuadranId?>&reqTahun=<?=$reqTahun?>&reqKdOrganisasiId=<?=$reqKdOrganisasiId?>&reqJenisJabatan=<?=$reqJenisJabatan?>&reqEselonId=<?=$reqEselonId?>",
		...
		/* Click event handler */
		  ...
		  $('#btnUbahData').on('click', function () {
			  ...
			  opUrl= 'pegawai_edit.php?reqId='+anSelectedPegawaiId;
			  newWindow = window.open(opUrl, 'INFO');
			  newWindow.focus();
		  ...
		  $('#btnExcel').on('click', function () {
			  ...
			  newWindow = window.open("tabel_nine_box_talent_potensi_kompetensi_excel.php?reqId=<?=$reqId?>&reqKuadranId="+reqKuadranId+"&reqTahun="+reqTahun);
			  newWindow.focus();				  
		  });
		  ...
		  $('#btnMutasi').on('click', function () {
			  ...
			  opWidth= 1050;
			  opHeight= 550;
			  opUrl= 'mutasi_tabel_nine_box_talent_potensi_kompetensi.php?reqPegawaiId='+anSelectedPegawaiId+'&reqId='+anSelectedSatkerId;
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						var jqxhr = $.get( "delete.php?reqMode=tabel_nine_box_talent_potensi_kompetensi&id="+anSelectedPegawaiId, function() {
							$.messager.progress('close');
						})
						.done(function() {
							$.messager.progress('close');
							oTable.fnReloadAjax("../json/tabel_nine_box_talent_potensi_kompetensi_json.php?reqSearch=" + $("#reqStatus").val() + "&reqId=<?=$reqId?>");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
		  ...
		  $("#reqPencarian").keyup(function(e) { 
			...
			if(code==13)
			{
				  ...
				  oTable.fnReloadAjax("../json-ikk/tabel_nine_box_talent_potensi_kompetensi_json.php?reqTahun="+reqTahun+"&reqKuadranId="+reqKuadranId+"&reqFormulaId="+reqFormulaId+ "&reqId=<?=$reqId?>&reqKdOrganisasiId=<?=$reqKdOrganisasiId?>&reqJenisJabatan=<?=$reqJenisJabatan?>&sSearch="+reqPencarian);
			}
		  });
		  
		  $("#reqTahun, #reqKuadranId, #reqEselonId, #reqFormulaId").change(function() { 
		  	  var reqTahun= reqKuadranId= reqPencarian= reqEselonId= reqFormulaId="";
			  ...
			  oTable.fnReloadAjax("../json-ikk/tabel_nine_box_talent_potensi_kompetensi_json.php?reqTahun="+reqTahun+"&reqEselonId="+reqEselonId+"&reqKuadranId="+reqKuadranId+"&reqFormulaId="+reqFormulaId+"&reqId=<?=$reqId?>&reqKdOrganisasiId=<?=$reqKdOrganisasiId?>&reqJenisJabatan=<?=$reqJenisJabatan?>&sSearch="+reqPencarian);
		  });
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <ul>
          <li>
          	<a style="display: none;" href="#" title="Detail Data" id="btnUbahData"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;Detail Data</a>
    ...
    <select id="reqKuadranId" name="reqKuadranId">
    	...
    	<option value="11">I. Tingkatkan Kompetensi</option>
        <option value="12">II. Tingkatkan Peran Saat Ini</option>
        <option value="21">III. Tingkatkan Peran Saat Ini</option>
        <option value="13">IV. Tingkatkan Peran Saat Ini</option>
        <option value="22">V. Siap Untuk Peran Masa Depan Dengan Pengembangan</option>
        <option value="31">VI. Pertimbangkan (mutasi)</option>
        <option value="23">VII. Siap Untuk Peran Masa Depan Dengan Pengembangan</option>
        <option value="32">VIII. Siap Untuk Peran Masa Depan Dengan Pengembangan</option>
        <option value="33">IX. Siap Untuk Peran Di Masa Depan</option>
    </select>
    ...
    <input name="reqEselonId" id="reqEselonId" value="<?=$reqEselonId?>" type="hidden" />
    ...
    <select id="reqEselonId" name="reqEselonId">
        ...
        <option value="9">Staff / Jabatan Fungsional</option>
    </select> -->
	...
	{
    ?>
    <input name="reqKuadranId" id="reqKuadranId" value="<?=$reqKuadranId?>" type="hidden" />
	...
	else
	{
	    ...
	    elseif($reqEselonId == "2") 
	    {
	        echo "II";
	    } 
	    elseif($reqEselonId == "3") 
	    {
	        echo "III";
	    } 
	    elseif($reqEselonId == "4") 
	    {
	        echo "IV";
	    } 
	    else 
	    {
	    	echo "Staff / Jabatan Fungsional";
	    }
	}
    ?>
    ...
    <thead>
        ...
        <tr>
          <th style="width:50px;"><div align="center">Potensi</div></th>
          <th style="width:50px;"><div align="center">Kompetensi</div></th>
