...
function openInfo(idKuadran, kodeKuadran)
{
	...
	var opUrl= "tabel_nine_box_talent_potensi_kompetensi.php?reqId=<?=$reqId?>&reqTahun="+reqTahun+"&reqEselonId="+reqEselonId+"&reqKuadranId="+idKuadran;
	...
	request.done(function(dataJson)
	{
		...
		else
		{
			...
			chart: {
				...
				title: {
					text:'<span style="font-size:14px">TALENT POOL POTENSI KOMPETENSI<span><br><span style="font-size:14px"><?=$tempNamaSatuanKerja?></span>'
				},
				series: [
				{
					type: 'line',
...
setGrafik("../json-ikk/grafik_nine_box_talent_potensi_kompetensi_grafik_json.php?reqId=<?=$reqId?>&reqTahun=<?=$reqTahun?>");
setModal("tableGrafik", "../json-ikk/grafik_nine_box_talent_potensi_kompetensi_table_json.php?reqId=<?=$reqId?>&reqTahun=<?=$reqTahun?>");

$(function () {

	$('#reqId').change(function() {
		reqId= $("#reqId").val();
		document.location.href= "grafik_nine_box_talent_potensi_kompetensi.php?reqId="+reqId;
	});
...
function setProsesToleransi()
{
	$.messager.confirm('Konfirmasi', "Apakah Anda Yakin, proses data ?",function(r){
		if (r){
			var reqTahun= reqEselonId= reqToleransiY= reqToleransiX= reqKdOrganisasiId= reqJenisJabatan= reqFormulaId = "";
			...
			$.ajax({'url': s_url,'success': function(msg) {
				...
				{
					setGrafik("../json-ikk/grafik_nine_box_talent_potensi_kompetensi_grafik_json.php?reqId=<?=$reqId?>&reqTahun="+reqTahun+"&reqKdOrganisasiId="+reqKdOrganisasiId+"&reqJenisJabatan="+reqJenisJabatan+"&reqEselonId="+reqEselonId+"&reqFormulaId="+reqFormulaId+"&reqPencarian="+reqPencarian);
					setModal("tableGrafik", "../json-ikk/grafik_nine_box_talent_potensi_kompetensi_table_json.php?reqId=<?=$reqId?>&reqTahun="+reqTahun+"&reqKdOrganisasiId="+reqKdOrganisasiId+"&reqJenisJabatan="+reqJenisJabatan+"&reqEselonId="+reqEselonId+"&reqFormulaId="+reqFormulaId+"&reqPencarian="+reqPencarian);
				}
			}});
		}
	});
...
<body style="background-color: white !important">
    ...
    <div style="width:45%;" class="konten-table" id="tableGrafik"></div>
    
	<!-- <div style="width:calc(50% - 40px); float:right; clear:left; position:absolute; right:20px; top:300px;"> -->
	<div style="width:45%; float:right; position:relative; top:260px;">
	    <!--<div id="container" style="height: 300px; clear:both; border:1px solid cyan !important;"></div>-->
...
$tempSkpY1= $set->getField("SKP_Y1");
$tempGmX1= $set->getField("GM_X1");
$tempGmY0= $set->getField("GM_Y1");
$tempSkpX2= $set->getField("SKP_X2");
$tempSkpY2= $set->getField("SKP_Y2");
$tempGmX2= $set->getField("GM_X2");
$tempGmY0= $set->getField("GM_Y2");-->
