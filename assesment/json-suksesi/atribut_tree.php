...
if ($id == 0)
{
	...
	$result["total"] = $set->getCountByParamsUnsurPenilaianCombo(array("PARENT_ID" => 0), $statement);	
	$set->selectByParamsUnsurPenilaianCombo(array("PARENT_ID" => 0), $rows, $offset, $statement);
	// echo $set->query;exit;
...
else 
{
	...
	$set->selectByParamsUnsurPenilaianCombo(array("PARENT_ID" => $id), -1, -1, $statement);
	//echo $set->query;exit;
	$i=0;
	while($set->nextRow())
	{
...
{
	$child = new UnsurPenilaian();
	$child->selectByParamsUnsurPenilaianCombo(array("PARENT_ID" => $id), -1, -1, $statement);
	$child->firstRow();
