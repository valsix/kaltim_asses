...
if ($id == 0)
{
	...
	$result["total"] = $set->getCountByParamsMasterJabatanCombo(array("PARENT_ID" => 0), $statement);	
	$set->selectByParamsMasterJabatanCombo(array("PARENT_ID" => 0), $rows, $offset, $statement);
	// echo $set->query;exit;
	...
	while($set->nextRow())
	{
		...
		$items[$i]['NAMA_JABATAN'] = $set->getField("NAMA_JABATAN");
...
{	
	...
	while($set->nextRow())
	{
		...
		$result[$i]['NAMA_JABATAN'] = $set->getField("NAMA_JABATAN");
