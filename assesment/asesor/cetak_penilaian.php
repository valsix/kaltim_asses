...
$reqJadwalTesId= $setCetak->getField("JADWAL_TES_ID");
...
while($set->nextRow())
{
  ...
  $arrAsesor[$set->getField("PENGGALIAN_KODE")]['nama']= $set->getField("nama_asesor");
  $arrAsesor[$set->getField("PENGGALIAN_KODE")]['nip']= $set->getField("nip_asesor");
  $index_loop++;
...
if($reqTipe=='CBI'||$reqTipe=='ITR'||$reqTipe=='LGD'||$reqTipe=='Psikotes'||$reqTipe=='PW'||$reqTipe=='AK'||$reqTipe=='PTR'||$reqTipe=='PT'){
    $html.= file_get_contents($urlLink."asesor/cetak_cbi.php?reqTipe=".$reqTipe."&reqJadwalTesId=".$reqJadwalTesId."&reqPegawaiId=".$reqPegawaiId."&reqTanggalTes=".$reqTanggalTes);
}
else{
...
if($reqTipe=='Rekap'){
    $ttdnama=$arrAsesor['CBI']['nama'];
    $ttdnip=$arrAsesor['CBI']['nip'];
}
else{
    $ttdnama=$arrAsesor[$reqTipe]['nama'];
    $ttdnip=$arrAsesor[$reqTipe]['nip'];
}
...
$mpdf->Output($reqTipe.'-'. $reqJadwalPegawaiNip .'-'.$nama.').pdf','I');
//exit;
?>