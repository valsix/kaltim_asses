...
$set_eselon->selectByParamsMonitoringEselon(array(), -1,-1, " AND ESELON_ID NOT IN (99)");
...
<style type="text/css" media="screen">
	...
	.sudahKandidat { background-color:#F9F; }
</style>
...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",
		"sAjaxSource": "../json-suksesi/rencana_suksesi_json.php?reqId=<?=$reqId?>&reqCari=<?=$reqCari?>&reqTahun=<?=$reqTahun?>",
		"sPaginationType": "full_numbers",
		"fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
			var valueStyle= maxLoop= "";
			maxLoop= 18;
			valueStyle= nRow % 2;
			//oddWarna;evenWarna;sudahKandidat
			if(valueStyle == "1") 
			{
				var i=0;
				for (i=0;i<=maxLoop;i++)
				{
					jQuery('td:eq('+i+')', nRow).addClass('oddWarna');
				}
			}
			else
			{
				...
				for (i=0;i<=maxLoop;i++)
				{
					jQuery('td:eq('+i+')', nRow).addClass('evenWarna');
				}
			}
			...
			if( aData[19] == "1")
			{
				...
				for (i=0;i<=maxLoop;i++)
				{
					jQuery('td:eq('+i+')', nRow).addClass('sudahKandidat');
				}
			}
		...
		/* Click event handler */
		  ...
		  $('#btnUbahData').on('click', function () {
			  ...
			  opUrl    = 'rencana_suksesi_kandidat.php?reqPensiunId='+anSelectedPegawaiId;
			  ...
			  if(anSelectedData == "")
				...
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						...
						.done(function() {
							$.messager.progress('close');
							oTable.fnReloadAjax("../json/rencana_suksesi_json.php?reqSearch=" + $("#reqStatus").val() + "&reqId=<?=$reqId?>");
						})
						.fail(function() {
							alert( "error" );
							$.messager.progress('close');
		  ...
		  $("#reqPencarian").keyup(function(e) { 
			...
			if(code==13)
			{
				  ...
				  oTable.fnReloadAjax("../json-suksesi/rencana_suksesi_json.php?reqStatusPeg="+reqStatusPeg+ "&reqId=<?=$reqId?>&sSearch="+reqPencarian);
			}
		  });
		  
		  $("#btnLaporanRekapMonitoring").on("click", function () {
			  //alert('-'+val.length);
			  $.messager.confirm('Konfirmasi', "Apakah Anda Yakin, Cetak Rekap Monitoring dengan data yang tampil ?",function(r){
					if (r){
						var reqStatusPeg= reqTahun= reqEselonId= reqTipe= reqPencarian= "";
						...
						newWindow = window.open("rencana_suksesi_laporan_rekap_monitoring.php?reqTahun="+reqTahun+"&reqEselonId="+reqEselonId+"&reqTipe="+reqTipe+"&reqId=<?=$reqId?>&sSearch="+reqPencarian, 'Cetak');
						newWindow.focus();
					}
			  });
		  });
			  
		  $("#reqStatusPeg,#reqTahun,#reqEselonId,#reqTipe").change(function() { 
		  	  var reqStatusPeg= reqTahun= reqEselonId= reqTipe= reqPencarian= "";
			  ...
			  reqTipe= $("#reqTipe").val();
			  reqPencarian= $("#reqPencarian").val();
			  oTable.fnReloadAjax("../json-suksesi/rencana_suksesi_json.php?reqTahun="+reqTahun+"&reqEselonId="+reqEselonId+"&reqTipe="+reqTipe+"&reqId=<?=$reqId?>&sSearch="+reqPencarian);
		  });
...
<body id="index" class="grid_2_3" style="overflow:hidden">
    ...
    <ul>
          <li>
          	<a href="#" title="Kandidat" id="btnUbahData"><img src="../WEB/images/pegawai-edit.png"/>&nbsp;Kandidat</a>
            <a href="#" title="Rekap Monitoring" id="btnLaporanRekapMonitoring" style="width:300px"><img src="../WEB/images/icon-excel.png" width="15" height="15"/>&nbsp;Rekap Monitoring</a>
    ...
    <select id="reqTipe" name="reqTipe">
    	<option selected="selected" value="1">Pensiun</option>
        <option value="2">Masa Jabatan</option>
        <option value="3">Pilih pegawai manual</option>
    </select>
    Eselon
