...
$reqUnsurPenilaianParentId= httpFilterGet("reqUnsurPenilaianParentId");
$reqUnsurPenilaianId= httpFilterGet("reqUnsurPenilaianId");
$reqTahun= httpFilterGet("reqTahun");

if($reqUnsurPenilaianId == "")
{
	$reqMode = "insert";
	$statement= " AND A.UNSUR_ID = '".$reqUnsurPenilaianParentId."'".$statement_tahun;
	...
	$statement= " AND EXISTS (SELECT 1 FROM (SELECT PERMEN_ID AKTIF_PERMENT FROM permen WHERE STATUS = '1') X WHERE AKTIF_PERMENT = PERMEN_ID)";
	$set_parent= new UnsurPenilaian();
	$set_parent->selectByParams(array('A.UNSUR_ID'=>$reqUnsurPenilaianParentId), -1, -1, $statement);
	$set_parent->firstRow();
	$tempUnsurPenilaianParent= $set_parent->getField("NAMA");
}
else
{
	$reqMode = "update";
	...
	$set->selectByParams(array('A.UNSUR_ID'=>$reqUnsurPenilaianId), -1, -1, $statement.$statement_tahun);
	// echo $set->query;exit;
	$set->firstRow();
	
	$reqTahun= $set->getField("TAHUN");
	$reqUnsurPenilaianParentId= $set->getField("UNSUR_ID_PARENT");
	$tempAspekId= $set->getField("ASPEK_ID");
	$tempAspekNama= $set->getField("ASPEK_NAMA");
	...
	$set_parent= new UnsurPenilaian();
	$set_parent->selectByParams(array('A.UNSUR_ID'=>substr($reqUnsurPenilaianParentId, 0, 2)), -1, -1, $statement_tahun);
	$set_parent->firstRow();
	$tempUnsurPenilaianParent= $set_parent->getField("NAMA");
}
...
<script type="text/javascript">	
	$(function(){
		$('#ff').form({
			url:'../json-suksesi/unsur_add.php',
			onSubmit:function(){
...
<div id="content" style="height:auto; margin-top:-4px; width:100%">
    	...
    	<table class="table_list" cellspacing="1" width="100%">
            <tr>
                <td colspan="6">
                <div id="header-tna-detil">Master <span>Unsur Penilaian</span></div>
                </td>			
            </tr>
            <!-- <tr>
                ...
                <td>
                    <?
					if($reqUnsurPenilaianParentId == "0")
					{
					?>
            ...
            <?
			if($tempUnsurPenilaianParent == ""){}
			else
			{
            ?>
            <tr>
                <td>Unsur Penilaian </td><td>:</td>
                <td><?=$tempUnsurPenilaianParent?></td>
            </tr>
            <?
			}
            ?>
            ...
            <tr>
                <td>
                    ...
                    <input type="hidden" name="reqUnsurPenilaianParentId" value="<?=$reqUnsurPenilaianParentId?>">
                    <input type="hidden" name="reqUnsurPenilaianId" value="<?=$reqUnsurPenilaianId?>">
