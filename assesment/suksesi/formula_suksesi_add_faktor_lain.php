...
else
{
	...
	$reqAssessmentId= $setcheck->getField('ASSESSMENT_ID');
	$tempKuadran= $setcheck->getField('KUADRAN_NAMA');
	$reqFormulaFaktorId= $setcheck->getField('FORMULA_FAKTOR_ID');
	
}
$setment= new FormulaAssesment();
$setment->selectByParams(array());
...
<script type="text/javascript">	
	...
	$(function(){
		$('#ff').form({
			url:'../json-suksesi/formula_suksesi_add_faktor_lain.php',
			onSubmit:function(){
			...
			success:function(data){
				...
				document.location.href = 'formula_suksesi_add_faktor_lain.php?reqId='+reqId;
			}
		});
...
<div id="content" style="height:auto; margin-top:-4px; width:100%">
		<form id="ff" method="post" novalidate>
			<table class="table_list" cellspacing="1" width="100%">
				<tr>
					<td colspan="6">
					<div id="header-tna-detil">Kriteria <span>Lainnya</span></div>
					</td>			
				</tr>
				<!-- <tr>
					<td style="width: 20%">
				...
				<tr>
					<td><input type="checkbox" name="checkbox0" class="checkbox0" id="checkbox0" value="<?=$reqAssessmentId?>" /> Formula Assessment</td>
					<td>:</td>        
					<td>
						<select name ="reqAssessmentId" id="reqAssessmentId" >
							<option value="">Semua</option>
							<?
							while ($setment->nextRow()) 
							{
								$infoid= $setment->getField("FORMULA_ID");
								$infonama= $setment->getField("FORMULA");
							?>
							<option value="<?=$infoid?>" <? if($infoid == $reqAssessmentId) echo 'selected';?> ><?=$infonama?></option>
				...
				<tr>
					<td><input type="checkbox" name="checkbox1" class="checkbox1" id="checkbox1" value="<?=$reqGrafikId?>" /> Grafik Assessment</td>
					<td>:</td>        
					<td>
    ...
    <script>
        ...
        $(function () {
        	...
        	reqAssessmentId= $("#reqAssessmentId").val();
        	if(reqAssessmentId == "")
        	{
        		$("#checkbox0").attr('checked', '');
        		$("#checkbox0").prop('checked', false);
        		$("#reqAssessmentId").hide();
        	}
	        ...
	        $('input[name="checkbox0"]').on('click', function () {
	        	if ($(this).prop('checked')) {
	        		$('#reqAssessmentId').fadeIn();
	        	} else {
	        		$("#reqAssessmentId").val("");
	        		$('#reqAssessmentId').hide();
	        	}
	        ...
	        // 	}
	        // });
	        $('.checkbox0').each(function(e){
	        	let checkval = $(this).val();
	        	// console.log(checkval);
	        	if(checkval){
	        		$(this).attr("checked", "checked");
	        		$('#reqAssessmentId').show();
	        	}
	        });
