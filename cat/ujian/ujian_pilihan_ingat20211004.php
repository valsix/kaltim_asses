...
<script language="javascript">
	...
	function settime(id)
	{
		...
		var end;
		// var endlog;
		if(localStorage.getItem("ingatend")) {
			end = new Date(localStorage.getItem("ingatend"));
		...
		var counter = function () {
			...
			if(now >= end) {
				...
				localStorage.setItem("ingatend", end);
...
{
	// console.log("setKondisiInfoFinish");
	<?
	if($tempCheckDataIngat == "0")
	{
		// set mulai pernah ingat
		$statement= " AND B.TIPE_UJIAN_ID = 27 AND A.UJIAN_ID = ".$tempUjianId." AND A.PEGAWAI_ID = ".$tempPegawaiId;
		$set= new UjianTahapStatusUjianIngat();
...
$set= new TipeUjian();
$set->selectByParams(array(), -1,-1, " AND TIPE_UJIAN_ID = ".$tempTipeUjianId);
