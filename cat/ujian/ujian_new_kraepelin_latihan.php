...
include_once("../WEB/classes/base-cat/UjianTahap.php");
include_once("../WEB/classes/base-cat/NKraepelinSoal.php");
...
(
	...
	FROM cat_pegawai.UJIAN_KRAEPELIN_N_LATIHAN_".$tempUjianPegawaiLowonganId."
...
$set= new NKraepelinSoal();
$set->selectByXbarisYbarisParamsLatihan(array(), -1,-1, $statement);
$set->firstRow();
// echo $set->query;exit();
...
if($x_bawah_batas == "")
{
	...
	if($tempPegawaiId == "")
	{
		...
		if($set->insertLatihan())
		{
			...
			// $set_detil->selectByParams(array(), -1,-1, " AND TIPE_UJIAN_ID = ".$reqTipeUjianId." AND UJIAN_ID = ".$ujianPegawaiUjianId);
			// $set_detil->firstRow();
			// $reqUjianTahapId= $set_detil->getField("UJIAN_TAHAP_ID");

			$statement= " AND A.UJIAN_ID = ".$ujianPegawaiUjianId." AND A.PEGAWAI_ID = ".$tempPegawaiId." AND A.TIPE_UJIAN_ID = ".$reqTipeUjianId;
			...
			echo 'top.location.href = "index.php?pg=ujian_new_kraepelin&reqUjianTahapId='.$reqUjianTahapId.'";';
			echo '</script>';
			exit;
		}
		//echo $set->query;exit;
...
$arrJawabanSoal=array();
$index_data= 0;
$set= new NKraepelinSoal();
$set->selectByParamsJawabanLatihan(array(), -1, -1, $statement);
//echo $set->errorMsg;exit;
// echo $set->query;exit;
while($set->nextRow())
{
	$arrJawabanSoal[$index_data]["KOORDINAT"]= $set->getField("X_DATA")."-".$set->getField("Y_DATA");
	$arrJawabanSoal[$index_data]["X_DATA"]= $set->getField("X_DATA");
	$arrJawabanSoal[$index_data]["Y_DATA"]= $set->getField("Y_DATA");
	$arrJawabanSoal[$index_data]["NILAI"]= $set->getField("NILAI");
	$index_data++;
}
unset($set);
$jumlah_jawaban_soal= $index_data;
// print_r($arrJawabanSoal);exit();
...
<script type="text/javascript">
	...
	function setSimpan()
	{
		$.messager.confirm('Konfirmasi', "Apakah Anda yakin untuk simpan data, dan melanjutkan untuk ujian ?",function(r){
			if (r){
				$('#ff').submit();
	...
	$(function(){
		$('#ff').form({
			url:'../json-ujian/ujian_new_kraepelin_latihan.php',
			onSubmit:function(){
			...
			success:function(data){
				...
				document.location.href = '?pg=ujian_new_kraepelin_latihan&reqUjianTahapId=<?=$reqUjianTahapId?>';
			}
		});
                		...
                		<tr>
                			...
                			<td>
	                			...
	                			for($y=$y_batas; $y >= 1; $y--)
	                			{
									...
									$arrayKey= $reqJawabSoalNilai= '';
  									$arrayKey= in_array_column($koordinat, "KOORDINAT", $arrJawabanSoal);
  									//print_r($arrayKey);exit;
  									if($arrayKey == ''){}
									else
									{
										$index_row= $arrayKey[0];
										$reqJawabSoalNilai= $arrJawabanSoal[$index_row]["NILAI"];
									}
	                			?>
		                			<tr>
								        ...
								        <td> </td>
								        <td>
								        	<span style="display: none; color: red" id='reqJawabSoalRow-<?=$x."-".$y_jawab?>'><?=$reqJawabSoalNilai?></span>
								        </td>
								    </tr>
							    <?
								    if($y_jawab > 0)
