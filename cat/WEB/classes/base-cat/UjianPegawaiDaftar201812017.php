  ...
  class UjianPegawaiDaftar extends Entity{ 
    ...
    **/
	...
	function insertUser($statement="")
	{
		...
		INNER JOIN PELAMAR B ON A.PEGAWAI_ID=B.PELAMAR_ID
    ...
    **/ 
	...
	{
		$str = "SELECT UJIAN_PEGAWAI_DAFTAR_ID, NO_KTP NIP_BARU, B.NAMA NAMA_PEGAWAI, A.UJIAN_ID, A.PEGAWAI_ID
				  ...
				  , TEMPAT_LAHIR, TANGGAL_LAHIR, EMAIL1 EMAIL, B.NO_HP TELEPON
				  FROM cat.UJIAN_PEGAWAI_DAFTAR A
				  LEFT JOIN PELAMAR B ON A.PEGAWAI_ID=B.PELAMAR_ID
				WHERE 1=1"; 
	...
	{
		$str = "SELECT DISTINCT A.UJIAN_ID,  A.PEGAWAI_ID, A.UJIAN_TAHAP_ID, COALESCE(JUMLAH_SUDAH,0)JUMLAH_SUDAH, COALESCE(JUMLAH_BELUM,0)JUMLAH_BELUM 
				FROM CAT.UJIAN_PEGAWAI A
				LEFT JOIN 
				(
					SELECT UJIAN_ID,  PEGAWAI_ID, UJIAN_TAHAP_ID, COUNT(1) JUMLAH_SUDAH
					FROM CAT.UJIAN_PEGAWAI
					WHERE BANK_SOAL_PILIHAN_ID IS NOT NULL
					GROUP BY UJIAN_ID,  PEGAWAI_ID, UJIAN_TAHAP_ID
				) B ON A.UJIAN_ID = B.UJIAN_ID AND A.PEGAWAI_ID = B.PEGAWAI_ID AND A.UJIAN_TAHAP_ID = B.UJIAN_TAHAP_ID
				LEFT JOIN 
				(
					SELECT UJIAN_ID,  PEGAWAI_ID, UJIAN_TAHAP_ID, COUNT(1) JUMLAH_BELUM
					FROM CAT.UJIAN_PEGAWAI
					WHERE BANK_SOAL_PILIHAN_ID IS NULL
					GROUP BY UJIAN_ID,  PEGAWAI_ID, UJIAN_TAHAP_ID
				) C ON A.UJIAN_ID = C.UJIAN_ID AND A.PEGAWAI_ID = C.PEGAWAI_ID AND A.UJIAN_TAHAP_ID = C.UJIAN_TAHAP_ID
				WHERE 1=1"; 
    ...
    function selectByParamsMonitoringHasil($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sorder="")
	{
		$str = "SELECT UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, A.PELAMAR_ID PEGAWAI_ID
				...
				, STATUS_LOGIN, STATUS_SETUJU, STATUS_UJIAN, STATUS_SELESAI, NILAI_HASIL, JUMLAH_BENAR, NILAI_LULUS
				...
				LEFT JOIN PELAMAR A ON B.PEGAWAI_ID=A.PELAMAR_ID
	...
	function selectByParamsMonitoringCfitHasilDetil($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sorder="")
	{
		$str = "
		SELECT
			UJIAN_PEGAWAI_DAFTAR_ID, B.UJIAN_ID, A.PELAMAR_ID PEGAWAI_ID
			, A.NAMA NAMA_PEGAWAI, A.KTP_NO NIP_BARU
			, CAST(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0) AS NUMERIC) JUMLAH_BENAR, HSL.UJIAN_TAHAP_ID
			, cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) NILAI_HASIL
			, CASE
			WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 130 THEN 'Sangat Superior'
			WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 120 AND CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) < 130 THEN 'Superior'
			WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 110 AND CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) < 120 THEN 'Diatas Rata - Rata'
			WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 90 AND CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) < 110 THEN 'Rata - Rata'
			WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 80 AND CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) < 90 THEN 'Dibawah Rata - Rata'
			WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 70 AND CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) < 80 THEN 'Borderline'
			WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) <= 69 THEN 'Intellectual Deficient'
			END KESIMPULAN
		FROM cat.UJIAN_PEGAWAI_DAFTAR B
		LEFT JOIN PELAMAR A ON B.PEGAWAI_ID=A.PELAMAR_ID
		LEFT JOIN cat.UJIAN D ON B.UJIAN_ID = D.UJIAN_ID
		LEFT JOIN
		(
			SELECT A.PEGAWAI_ID, A.UJIAN_ID, A.ID, COALESCE(A.JUMLAH_BENAR,0) JUMLAH_BENAR_PEGAWAI
			, C.UJIAN_TAHAP_ID
			FROM
			(
				SELECT 
				A.PEGAWAI_ID, A.UJIAN_ID, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
				FROM
				(
				SELECT a.*
				FROM
				(
				SELECT 
				A.PEGAWAI_ID, A.UJIAN_ID, B.TIPE_UJIAN_ID, TU.ID, A.BANK_SOAL_ID
				FROM cat.UJIAN_PEGAWAI A
				...
				GROUP BY A.PEGAWAI_ID, A.UJIAN_ID, B.TIPE_UJIAN_ID, TU.ID, A.BANK_SOAL_ID
				) A
				) A
				INNER JOIN cat.UJIAN_TAHAP B ON A.UJIAN_ID = B.UJIAN_ID AND A.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
				WHERE 1=1
				GROUP BY A.PEGAWAI_ID, A.UJIAN_ID, A.ID
			) A
			INNER JOIN
			(
				...
				WHERE 1=1
			) C ON A.UJIAN_ID = A.UJIAN_ID AND A.ID = C.ID
	...
	function selectByParamsMonitoringCfitHasil($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sorder="")
	{
		...
		(
			...
			(
				...
				(
				SELECT 
				A.PEGAWAI_ID, A.UJIAN_ID, B.TIPE_UJIAN_ID, SUBSTR(TU.ID,1,2) ID, A.BANK_SOAL_ID
				FROM cat.UJIAN_PEGAWAI A
				INNER JOIN cat.BANK_SOAL B ON A.BANK_SOAL_ID = B.BANK_SOAL_ID
				INNER JOIN cat.TIPE_UJIAN TU ON TU.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
				INNER JOIN 
				...
				GROUP BY A.PEGAWAI_ID, A.UJIAN_ID, B.TIPE_UJIAN_ID, SUBSTR(TU.ID,1,2), A.BANK_SOAL_ID
				) A
				) A
				INNER JOIN cat.UJIAN_TAHAP B ON A.UJIAN_ID = B.UJIAN_ID AND A.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
				WHERE 1=1
	...
	function selectByParamsMonitoringHasilCetak($limit=-1,$from=-1, $statement='', $sorder="")
	{
		...
		(
			SELECT
				DISTINCT A.NAMA NAMA_PEGAWAI, A.KTP_NO NIP_BARU, NRP, 
				cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) NILAI_HASIL, 
				CASE
				WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 130 THEN 'Sangat Superior'
				WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 120 AND CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) < 130 THEN 'Superior'
				...
				WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) >= 70 AND CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) < 80 THEN 'Borderline'
				WHEN CAST(cat.AMBIL_IQ_NILAI(COALESCE(HSL.JUMLAH_BENAR_PEGAWAI,0)) AS NUMERIC) <= 69 THEN 'Intellectual Deficient'
				END KESIMPULAN, TIPE, E.TIPE_UJIAN_ID
			FROM cat.UJIAN_PEGAWAI_DAFTAR B
			LEFT JOIN PELAMAR A ON B.PEGAWAI_ID=A.PELAMAR_ID
			LEFT JOIN cat.UJIAN D ON B.UJIAN_ID = D.UJIAN_ID
			LEFT JOIN
			(
				SELECT A.PEGAWAI_ID, A.UJIAN_ID, A.ID, COALESCE(A.JUMLAH_BENAR,0) JUMLAH_BENAR_PEGAWAI
				, C.UJIAN_TAHAP_ID, TIPE_UJIAN_ID, PARENT_ID
				FROM
				(
					SELECT 
					A.PEGAWAI_ID, A.UJIAN_ID, A.ID, COUNT(A.PEGAWAI_ID) JUMLAH_BENAR
				...
				INNER JOIN
				(
					SELECT A.UJIAN_TAHAP_ID, B.ID, A.UJIAN_ID, A.TIPE_UJIAN_ID, PARENT_ID
					FROM cat.UJIAN_TAHAP A
					INNER JOIN cat.TIPE_UJIAN B ON A.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
					WHERE 1=1
					AND PARENT_ID = '0'
			...
			LEFT JOIN cat.TIPE_UJIAN E ON HSL.TIPE_UJIAN_ID = E.TIPE_UJIAN_ID 
			WHERE E.TIPE_UJIAN_ID IN (1,2) AND NOT E.TIPE_UJIAN_ID = 7 AND E.PARENT_ID = '0' ".$statement."
			UNION ALL
			SELECT 
				A.NAMA NAMA_PEGAWAI, A.KTP_NO NIP_BARU, NRP, CAST(NILAI_HASIL AS TEXT) NILAI_HASIL, '' KESIMPULAN, TIPE, D.TIPE_UJIAN_ID
			FROM CAT.UJIAN_PEGAWAI_DAFTAR A1
			INNER JOIN CAT.UJIAN_PEGAWAI_HASIL_TIPE B ON A1.UJIAN_ID = B.UJIAN_ID AND A1.PEGAWAI_ID = B.PEGAWAI_ID
			LEFT JOIN PELAMAR A ON B.PEGAWAI_ID=A.PELAMAR_ID
	...
	function selectByParamsMonitoringHasilCetakBak($limit=-1,$from=-1, $statement='', $sorder="")
	{
		$str = "
		SELECT *
		FROM
		(
			...
			FROM CAT.UJIAN_PEGAWAI_DAFTAR A1
			INNER JOIN CAT.UJIAN_PEGAWAI_HASIL_CFIT B ON A1.UJIAN_ID = B.UJIAN_ID AND A1.PEGAWAI_ID = B.PEGAWAI_ID
			LEFT JOIN PELAMAR A ON B.PEGAWAI_ID=A.PELAMAR_ID
			LEFT JOIN cat.UJIAN_TAHAP C ON B.UJIAN_TAHAP_ID = C.UJIAN_TAHAP_ID
			LEFT JOIN cat.TIPE_UJIAN D ON C.TIPE_UJIAN_ID = D.TIPE_UJIAN_ID 
	...
	function selectByParamsSoalRevisi($paramsArray=array(),$limit=-1,$from=-1,$statement="", $statementujian="", $order="")
	{
		$str = "
		SELECT
			A.UJIAN_ID, UP.UJIAN_BANK_SOAL_ID, UP.BANK_SOAL_ID, C.KEMAMPUAN, C.KATEGORI, C.PERTANYAAN
			, UP.BANK_SOAL_PILIHAN_ID, A.PEGAWAI_ID
			, A.STATUS_SETUJU, A.UJIAN_PEGAWAI_DAFTAR_ID, UP.UJIAN_PEGAWAI_ID
			, C.TIPE_SOAL, C.PATH_GAMBAR, C.PATH_SOAL
			, CASE WHEN COALESCE(UPX.JUMLAH_DATA,0) > 0 THEN 1 ELSE 0 END JUMLAH_DATA
			, UP.UJIAN_TAHAP_ID, URUT
		FROM cat.UJIAN_PEGAWAI_DAFTAR A
		INNER JOIN 
		(
			SELECT A.UJIAN_PEGAWAI_DAFTAR_ID, UP.BANK_SOAL_PILIHAN_ID
			, UP.UJIAN_PEGAWAI_ID, URUT, B.UJIAN_TAHAP_ID
			, B.UJIAN_BANK_SOAL_ID, B.BANK_SOAL_ID, B.UJIAN_ID
			FROM cat.UJIAN_PEGAWAI_DAFTAR A
			INNER JOIN cat.UJIAN_BANK_SOAL B ON A.UJIAN_ID = B.UJIAN_ID
			LEFT JOIN cat.UJIAN_PEGAWAI UP ON UP.PEGAWAI_ID = A.PEGAWAI_ID AND A.UJIAN_ID = UP.UJIAN_ID AND UP.UJIAN_BANK_SOAL_ID = B.UJIAN_BANK_SOAL_ID
			WHERE 1=1 ".$statementujian."
		) UP ON A.UJIAN_PEGAWAI_DAFTAR_ID = UP.UJIAN_PEGAWAI_DAFTAR_ID AND A.UJIAN_ID = UP.UJIAN_ID
		INNER JOIN cat.BANK_SOAL C ON UP.BANK_SOAL_ID = C.BANK_SOAL_ID
		INNER JOIN cat.UJIAN UJ ON UJ.UJIAN_ID = A.UJIAN_ID
		LEFT JOIN
		(
			SELECT COUNT(1) JUMLAH_DATA, PEGAWAI_ID, UJIAN_ID, UJIAN_BANK_SOAL_ID
			...
			WHERE BANK_SOAL_PILIHAN_ID IS NOT NULL
			GROUP BY PEGAWAI_ID, UJIAN_ID, UJIAN_BANK_SOAL_ID
		) UPX ON UPX.PEGAWAI_ID = A.PEGAWAI_ID AND A.UJIAN_ID = UPX.UJIAN_ID AND UPX.UJIAN_BANK_SOAL_ID = UP.UJIAN_BANK_SOAL_ID
		WHERE 1=1
		"; 
	...
	function selectByParamsSoalCheckList($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order="")
	{
		$str = "SELECT
					A.UJIAN_ID, B.UJIAN_BANK_SOAL_ID, B.BANK_SOAL_ID
					, UP.BANK_SOAL_PILIHAN_ID, A.PEGAWAI_ID, UP.UJIAN_PEGAWAI_ID
					, A.UJIAN_PEGAWAI_DAFTAR_ID, UP.UJIAN_PEGAWAI_ID
				FROM cat.ujian_pegawai_daftar A
				INNER JOIN cat.ujian_bank_soal B ON A.UJIAN_ID = B.UJIAN_ID
				INNER JOIN cat.bank_soal C ON B.BANK_SOAL_ID = C.BANK_SOAL_ID
				INNER JOIN cat.ujian UJ ON UJ.UJIAN_ID = A.UJIAN_ID
				INNER JOIN cat.ujian_pegawai UP ON UP.PEGAWAI_ID = A.PEGAWAI_ID AND A.UJIAN_ID = UP.UJIAN_ID AND UP.UJIAN_BANK_SOAL_ID = B.UJIAN_BANK_SOAL_ID
				WHERE 1=1
			"; 
	...
	function selectByParamsCheck($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order="")
	{
		$str = "
				SELECT
					UP.UJIAN_PEGAWAI_ID, A.UJIAN_ID, A.PEGAWAI_ID, UT.TIPE_UJIAN_ID, URUT
				FROM cat.ujian_pegawai_daftar A
				INNER JOIN cat.ujian_bank_soal B ON A.UJIAN_ID = B.UJIAN_ID
				INNER JOIN cat.bank_soal C ON B.BANK_SOAL_ID = C.BANK_SOAL_ID
				INNER JOIN cat.ujian UJ ON UJ.UJIAN_ID = A.UJIAN_ID
				INNER JOIN cat.ujian_tahap UT ON B.UJIAN_TAHAP_ID = UT.UJIAN_TAHAP_ID AND A.UJIAN_ID = UT.UJIAN_ID
				LEFT JOIN cat.ujian_pegawai UP ON UP.PEGAWAI_ID = A.PEGAWAI_ID AND A.UJIAN_ID = UP.UJIAN_ID AND UP.UJIAN_BANK_SOAL_ID = B.UJIAN_BANK_SOAL_ID
				WHERE 1=1
			"; 
	...
	function selectByParamsSoalFinish($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order="")
	{
		$str = "SELECT
					A.UJIAN_ID, B.UJIAN_BANK_SOAL_ID, B.BANK_SOAL_ID, C.KEMAMPUAN, C.KATEGORI, C.PERTANYAAN
					, UP.BANK_SOAL_PILIHAN_ID, A.PEGAWAI_ID, UP.UJIAN_PEGAWAI_ID
				...
				LEFT JOIN
				(
					SELECT COUNT(1) JUMLAH_DATA, UJIAN_ID, UJIAN_BANK_SOAL_ID, PEGAWAI_ID, BANK_SOAL_ID, UJIAN_TAHAP_ID FROM
					(
						SELECT DISTINCT UJIAN_ID, UJIAN_BANK_SOAL_ID, PEGAWAI_ID, BANK_SOAL_ID, UJIAN_TAHAP_ID
						FROM cat.ujian_pegawai A
						WHERE 1=1 AND BANK_SOAL_PILIHAN_ID IS NOT NULL
					) A
					GROUP BY UJIAN_ID, UJIAN_BANK_SOAL_ID, PEGAWAI_ID, BANK_SOAL_ID, UJIAN_TAHAP_ID
				) UPX ON UPX.PEGAWAI_ID = A.PEGAWAI_ID AND A.UJIAN_ID = UPX.UJIAN_ID AND UPX.UJIAN_BANK_SOAL_ID = B.UJIAN_BANK_SOAL_ID
				WHERE 1=1
			"; 
	...
	function selectByParamsSoalFinishRevisi($paramsArray=array(),$limit=-1,$from=-1,$statement="", $statementujian="", $order="")
	{
		$str = "
		SELECT
			A.UJIAN_ID, UP.UJIAN_BANK_SOAL_ID, UP.BANK_SOAL_ID, C.KEMAMPUAN, C.KATEGORI, C.PERTANYAAN
	...
	function selectByParamsSoalPapiCheckList($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order="")
	{
		$str = "SELECT
					A.UJIAN_ID, B.UJIAN_BANK_SOAL_ID, B.BANK_SOAL_ID, '' KEMAMPUAN
					, UP.BANK_SOAL_PILIHAN_ID, A.PEGAWAI_ID, UP.UJIAN_PEGAWAI_ID
	...
	function selectByParamsSoalPapiFinishData($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order="")
	{
		$str = "SELECT
					A.UJIAN_ID, B.UJIAN_BANK_SOAL_ID, B.BANK_SOAL_ID, '' KEMAMPUAN
					, '' KATEGORI, C.PERTANYAAN
					,A.PEGAWAI_ID
					, A.STATUS_SETUJU, A.UJIAN_PEGAWAI_DAFTAR_ID
					, '' TIPE_SOAL, '' PATH_GAMBAR, '' PATH_SOAL
					, CASE WHEN COALESCE(UPX.JUMLAH_DATA,0) > 0 THEN 1 ELSE 0 END JUMLAH_DATA
					, B.UJIAN_TAHAP_ID
				FROM cat.ujian_pegawai_daftar A
	...
	function selectByParamsSoalPapiFinish($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order="")
	{
		$str = "SELECT
					A.UJIAN_ID, B.UJIAN_BANK_SOAL_ID, B.BANK_SOAL_ID, '' KEMAMPUAN
					, '' KATEGORI, C.PERTANYAAN
    ...
    **/ 
	  ...
	  function getCountByParamsMonitoringCfitHasil($paramsArray=array(),$statement="")
	{
		...
		LEFT JOIN
		(
			SELECT A.PEGAWAI_ID, A.UJIAN_ID, A.ID, ROUND(CAST(COALESCE(A.JUMLAH_BENAR,0) AS NUMERIC) / CAST(COALESCE(A.JUMLAH_SOAL_UJIAN,0) AS NUMERIC) * 100) JUMLAH_BENAR
			, C.UJIAN_TAHAP_ID
			FROM
			(
				SELECT 
				A.PEGAWAI_ID, A.UJIAN_ID, A.ID, SUM(A.JUMLAH_BENAR) JUMLAH_BENAR, SUM(B.JUMLAH_SOAL_UJIAN_TAHAP) JUMLAH_SOAL_UJIAN
				FROM
				(
				SELECT A.PEGAWAI_ID, A.UJIAN_ID, B.TIPE_UJIAN_ID, SUBSTR(TU.ID,1,2) ID, COUNT(A.PEGAWAI_ID) AS JUMLAH_BENAR
				FROM cat.UJIAN_PEGAWAI A
				INNER JOIN cat.BANK_SOAL B ON A.BANK_SOAL_ID = B.BANK_SOAL_ID
				INNER JOIN cat.TIPE_UJIAN TU ON TU.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
				INNER JOIN 
				...
				GROUP BY A.PEGAWAI_ID, A.UJIAN_ID, B.TIPE_UJIAN_ID, SUBSTR(TU.ID,1,2)
				) A
				INNER JOIN cat.UJIAN_TAHAP B ON A.UJIAN_ID = B.UJIAN_ID AND A.TIPE_UJIAN_ID = B.TIPE_UJIAN_ID
				WHERE 1=1
				GROUP BY A.PEGAWAI_ID, A.UJIAN_ID, A.ID
