  ...
  ***/
  ...
  class UjianTahapStatusUjian extends Entity{ 
    ...
    **/
	...
	{
		/*Auto-generate primary key(s) by next max value (integer) */
		$this->setField("UJIAN_TAHAP_STATUS_UJIAN_ID", $this->getNextId("UJIAN_TAHAP_STATUS_UJIAN_ID","cat.ujian_tahap_status_ujian_latihan")); 
		$str = "INSERT INTO cat.ujian_tahap_status_ujian_latihan (
				   UJIAN_TAHAP_STATUS_UJIAN_ID, UJIAN_PEGAWAI_DAFTAR_ID,
				   JADWAL_TES_ID, FORMULA_ASSESMENT_ID, FORMULA_ESELON_ID, UJIAN_ID,
				   UJIAN_TAHAP_ID, TIPE_UJIAN_ID,
				   PEGAWAI_ID, STATUS,
	...
	function selectByParamsLatihan($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "
		SELECT UJIAN_TAHAP_STATUS_UJIAN_ID, UJIAN_ID, UJIAN_TAHAP_ID, PEGAWAI_ID, STATUS
		FROM cat.ujian_tahap_status_ujian_latihan
		WHERE 1=1
		"; 
    ...
    function getPegawaiUjianTahapSelesaiLatihan($statement="")
	{
		$str = "
		SELECT COUNT(1) AS ROWCOUNT
		FROM cat.ujian_tahap_status_ujian_latihan A
		WHERE 1=1 ".$statement;
		$this->select($str); 
		$this->query = $str;		
		if($this->firstRow()) 
