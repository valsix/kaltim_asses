...
***************************************************************************************************** */
  ...
  ***/
  ...
  class NKraepelinSoal extends Entity{ 

	var $query;
    /**
    * Class constructor.
    **/
    function NKraepelinSoal()
	{
      $this->Entity(); 
    }
	...
	{
		$str = "
		INSERT INTO cat.N_KRAEPELIN_SOAL (
    ...
    function update()
	{
		...
		$str = "
		UPDATE cat.N_KRAEPELIN_SOAL SET
	...
	{
		$str = "
		DELETE FROM cat.N_KRAEPELIN_SOAL
		WHERE PAKAI_KRAEPELIN_ID= ".$this->getField("PAKAI_KRAEPELIN_ID")."
		AND X_DATA= ".$this->getField("X_DATA").",
		AND Y_DATA= ".$this->getField("Y_DATA")."
		"; 
    ...
    function selectByXbarisYbarisParamsLatihan($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sorder="")
	{
		...
		SELECT A.PAKAI_KRAEPELIN_ID, MAX(A.X_DATA) X_DATA, MAX(A.Y_DATA) Y_DATA
		, MIN(A.X_DATA) MIN_X_DATA, MAX(A.X_DATA) - MIN(A.X_DATA) CHECK_MIN_X_DATA
		FROM cat.N_KRAEPELIN_SOAL_LATIHAN A
		WHERE 1=1 "; 
    ...
    function selectByParamsJawabanLatihan($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sorder="ORDER BY X_DATA, Y_DATA DESC")
	{
		$str = "
		SELECT A.PAKAI_KRAEPELIN_ID, A.X_DATA, A.Y_DATA, A.NILAI
		FROM cat.N_KRAEPELIN_JAWAB_LATIHAN A
		WHERE 1=1 "; 
	...
	{
		$str = "SELECT COUNT(1) AS ROWCOUNT FROM cat.N_KRAEPELIN_SOAL A WHERE 1=1 ".$statement; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = '$val' ";
		}
