...
/* *******************************************************************************************************
MODUL NAME 			: SIMKeu
FILE NAME 			: PageNumberJquery.php
AUTHOR				: Wawan
VERSION				: 1.0
MODIFICATION DOC	:
...
//include_once("../../kejagung/WEB/classes/WEB/functions/math.func.php");

class PageNumberJquery
{
	var $rowCount; #jumlah seluruh record
    var $numPage; #jumlah halaman
	
	var $pageId; #current page
	...
	function initialize($rowCount,$limit,$reqPage,$fileName,$pageId)
	{   
		$this->pageId = $pageId;
	...
	function drawPageFlex()
	{
		...
		*/
		...
		{ 
			echo "
				<a href='#' onclick=\"setModal('".$this->pageId."', '".$this->fileName."&reqPage=1&reqFlag=".$this->anyflag."');\">First</a>
				&nbsp;<a href='#' onclick=\"setModal('".$this->pageId."', '".$this->fileName."&reqPage=".($this->page-1)."&reqFlag=".$this->anyflag."');\">&lt;</a>&nbsp;
			";
		} 
		else 
		{ 
		...
		if($awal !== 1)
			echo "
							<a href='#' onclick=\"setModal('".$this->pageId."', '".$this->fileName."&reqPage=1&reqFlag=".$this->anyflag."');\">1...</a>
			";
		...
		for ($i = $awal; $i <= $showHal; $i++)
		{
			...
			if($this->page == $i)
				...
				echo"<a href='#' onclick=\"setModal('".$this->pageId."', '".$this->fileName."&reqPage=$i&reqFlag=".$this->anyflag."');\">$i</a>&nbsp;";
			echo "";
		}
				
		if ($i <= $this->numPage)
			echo "
				<a href='#' onclick=\"setModal('".$this->pageId."', '".$this->fileName."&reqPage=".$this->numPage."&reqFlag=".$this->anyflag."');\">...".$this->numPage."</a>
			";
		//------------------------------------------//
		...
		*/
		...
		{ 
			echo "
				&nbsp;<a href='#' onclick=\"setModal('".$this->pageId."', '".$this->fileName."&reqPage=".($this->page + 1)."&reqFlag=".$this->anyflag."');\">&gt;</a>&nbsp;
				<a href='#' onclick=\"setModal('".$this->pageId."', '".$this->fileName."&reqPage=".$this->numPage."&reqFlag=".$this->anyflag."');\">Last</a>
			";
		} 
		else 
		{ 
...
$pageNumber = new PageNumberJquery();
