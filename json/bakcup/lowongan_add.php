...
$reqTahapanTesId = $_POST["reqTahapanTesId"];
...
$reqStatusPegawaiProyek= httpFilterPost("reqStatusPegawaiProyek");
$reqTesFisik = httpFilterPost("reqTesFisik");
$reqTesKesemaptaan = httpFilterPost("reqTesKesemaptaan");

$jabatan->selectByParams(array("A.JABATAN_ID" => $reqJabatanId));
$jabatan->firstRow();
...
$set_nomor = new Lowongan();
$reqNomorGenerate = $set_nomor->getCountByParamsGenerateNomor($tahun, $statement); 

$reqKode = $reqNomorGenerate."-".$kode_jabatan.'-'.$kode_cabang;
$reqLinkFile= $_FILES['reqLinkFile'];
$reqLinkFileTemp = httpFilterPost("reqLinkFileTemp");

$lowongan->setField('LOWONGAN_ID', $reqId);
$lowongan->setField('KODE', $reqKode);
$lowongan->setField('NOMOR_GENERATE', $reqNomorGenerate);
...
$lowongan->setField('JABATAN_ID', $reqJabatanId);
$lowongan->setField('JUMLAH', $reqJumlah);
$lowongan->setField('MANUAL', $reqManual);
$lowongan->setField('IS_FISIK', $reqTesFisik);
$lowongan->setField('IS_KESEMAPTAAN', $reqTesKesemaptaan);
$lowongan->setField('KETERANGAN', $reqKeterangan);
$lowongan->setField("STATUS_PEGAWAI_PROYEK", ValToNullDB($reqStatusPegawaiProyek));
...
$lowongan->setField("PERSYARATAN", setQuote($InsertPersyaratan, 0));	
/* END */
	...
	if($lowongan->insert())
	{
		...
		for($i=0;$i<count($reqDokumen);$i++)
		{
			...
			else
			{
				...
				$lowongan_dokumen->setField("LOWONGAN_ID", $reqDetilId);
				$lowongan_dokumen->setField("DOKUMEN_ID", ValToNullDB($reqDokumenId[$i]));
				$lowongan_dokumen->setField("NAMA", $reqDokumen[$i]);
				$lowongan_dokumen->setField("WAJIB", $reqDokumenWajib[$i]);
				$lowongan_dokumen->setField("FORMAT", $reqFormat[$i]);
		...
		for($i=0;$i<count($reqTahapan);$i++)
		{
			...
			else
			{
				...
				$lowongan_tahapan->setField("TAHAPAN_TES_ID", $reqTahapanTesId[$i]);
	...
	if($lowongan->update())
	{
		...
		for($i=0;$i<count($reqDokumen);$i++)
		{
			...
			else
			{
				...
				{
					$lowongan_dokumen->setField("LOWONGAN_DOKUMEN_ID", $reqRowId[$i]);
					$lowongan_dokumen->update();			
				}
				
				//echo $lowongan_dokumen->query;
