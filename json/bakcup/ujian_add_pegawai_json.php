...
$aColumns = array("CHECK", "NIP_BARU", "NAMA_PEGAWAI", "JENIS_KELAMIN_KET", "AGAMA","EMAIL","TEMPAT_LAHIR", "TANGGAL_LAHIR", "PEGAWAI_ID");
$aColumnsAlias = array("CHECK", "NIP_BARU", "NAMA_PEGAWAI", "JENIS_KELAMIN_KET", "AGAMA","EMAIL","TEMPAT_LAHIR", "TANGGAL_LAHIR", "PEGAWAI_ID");
...
$searchJson= " AND (UPPER(B.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(EMAIL) LIKE '%".strtoupper($_GET['sSearch'])."%')";
...
while($set->nextRow())
{
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if($aColumns[$i] == 'CHECK')
		{
			$row[] = "<input type='checkbox' class='editor-active' id='reqPilihCheck".$no_urut."' value='".$set->getField("PEGAWAI_ID")."'>";
		}
