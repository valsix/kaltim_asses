...
$aColumns = array("PELAMAR_ID", "CHECK", "SHORTLIST", "NAMA", "NAMA_KOTA", "AGAMA", "JENIS_KELAMIN", "TELEPON", "TEMPAT_LAHIR", "TANGGAL_LAHIR", "UMUR", "TINGGI", "BERAT_BADAN", "STATUS_KAWIN", "PENDIDIKAN", "DURASI", "SERTIFIKAT", "LAMARAN_LAIN", "LAMPIRAN_FOTO");
$aColumnsAlias = array("PELAMAR_ID", "CHECK", "(SELECT COUNT(1) FROM pds_rekrutmen.PELAMAR_LOWONGAN_SHORTLIST X WHERE X.PELAMAR_ID = A.PELAMAR_ID AND X.LOWONGAN_ID = B.LOWONGAN_ID)", "A.NAMA", "F.NAMA", "A.AGAMA_ID", "JENIS_KELAMIN", "TELEPON", "TEMPAT_LAHIR", "TANGGAL_LAHIR", "pds_rekrutmen.AMBIL_MASA_KERJA(TANGGAL_LAHIR, TO_DATE('".$reqCutoff."', 'DD-MM-YYYY'))", "TINGGI", "BERAT_BADAN", "STATUS_KAWIN", 
						"C.PENDIDIKAN_ID", "COALESCE(BULAN, 0)", "pds_rekrutmen.AMBIL_PELAMAR_SERTIFIKAT(A.PELAMAR_ID)", "(SELECT COUNT(1) FROM pds_rekrutmen.PELAMAR_LOWONGAN X WHERE X.PELAMAR_ID = A.PELAMAR_ID AND NOT X.LOWONGAN_ID = B.LOWONGAN_ID)", "LAMPIRAN_FOTO");
...
$statement .= " AND NOT EXISTS(SELECT 1 FROM pds_rekrutmen.PELAMAR_LOWONGAN X WHERE X.LOWONGAN_ID = '".$reqId."' AND X.PELAMAR_ID = A.PELAMAR_ID)";

$allRecord = $pelamar_lowongan->getCountByParamsDaftarAddPelamarCutoff(array(), $statement);
if($_GET['sSearch'] == "")
	$allRecordFilter = $allRecord;
else	
	$allRecordFilter = $pelamar_lowongan->getCountByParamsDaftarAddPelamarCutoff(array(), $statement." AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

$pelamar_lowongan->selectByParamsDaftarAddPelamarCutoff($reqCutoff, array(), $dsplyRange, $dsplyStart, $statement." AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
