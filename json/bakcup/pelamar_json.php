...
$sSearch= " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(A.KTP_NO) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(EMAIL) LIKE '%".strtoupper($_GET['sSearch'])."%')";
$allRecord = $pelamar->getCountByParams(array(), $statement);
if($_GET['sSearch'] == "")
	$allRecordFilter = $allRecord;
else	
	$allRecordFilter = $pelamar->getCountByParams(array(), $sSearch.$statement);
	
$pelamar->selectByParamsCutOff($reqCutoff, array(), $dsplyRange, $dsplyStart, $sSearch.$statement, $sOrder); 
//echo $pelamar->query;exit;
//echo $pelamar->errorMsg;
