...
$aColumns = array("BANK_SOAL_ID", "NO", "PERTANYAAN", "TIPE", "TIPE_SOAL_INFO");
$aColumnsAlias = array("BANK_SOAL_ID", "NO", "PERTANYAAN", "TIPE", "TIPE_SOAL_INFO");
...
if ( isset( $_GET['iSortCol_0'] ) )
{
	...
	if ( trim($sOrder) == "ORDER BY BANK_SOAL_ID asc" )
	{
		...
		*/
		...
		//$sOrder = " ORDER BY A.BANK_SOAL_ID DESC";
...
if($_GET['sSearch'] == "")
	...
	$allRecordFilter = $bank_soal->getCountByParams(array(), $searchJson.$statement);

$bank_soal->selectByParamsMonitoring(array(), $dsplyRange, $dsplyStart, $searchJson.$statement, $sOrder);
//echo $bank_soal->query;exit;
//echo $bank_soal->errorMsg;exit;
...
while($bank_soal->nextRow())
{
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		...
		else if($aColumns[$i] == "PERTANYAAN")
		{
			...
			if($tempTipeSoal == "1" || $tempTipeSoal == "4"){}
			else
			{
				$tempUrlGambar= $bank_soal->getField("PATH_GAMBAR").$bank_soal->getField("PATH_SOAL");
				if(file_exists($tempUrlGambar))
				{
					$tempUrlGambar= '  <img src="'.$tempUrlGambar.'" height="80" width="90" onclick="setLookUpImage(\''.$tempUrlGambar.'\');" >';
				}
				else
				$tempUrlGambar="";
			}
