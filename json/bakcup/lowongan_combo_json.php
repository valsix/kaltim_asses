...
$statement = " AND NOT A.STATUS = 2";
$set->selectByParams(array(), -1, -1, $statement." AND UPPER(A.KODE) LIKE '%".strtoupper($search_term)."%' ");
//$set->selectByParams(array(), 5, 0);
//echo $set->query;
while($set->nextRow())
{
	$arr_parent[$j]['id'] = $set->getField("LOWONGAN_ID");
	$arr_parent[$j]['text'] = $set->getField("LOWONGAN_INFO");
